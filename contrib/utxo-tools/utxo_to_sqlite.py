...
def main():
    ...
    for coin_idx in range(1, num_utxos+1):
        ...
        scriptpubkey_bytes = decompress_script(f)
        # TensorCash: Read and skip vExt field (extension data in CTxOut)
        vext_size = read_compactsize(f)
        if vext_size > 0:
            f.read(vext_size)  # Skip the vExt data
        scriptpubkey = scriptpubkey_bytes.hex()
