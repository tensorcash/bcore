...
def calc_hash_str(blk_hdr, hash_header_size):
    data = blk_hdr[:hash_header_size]
    blk_hdr_hash = hashlib.sha256(hashlib.sha256(data).digest()).digest()
...
class BlockDataCopier:
    def __init__(self, settings, blkindex, blkmap):
        ...
        self.header_size = self.settings['header_size']
        self.hash_header_size = self.settings['hash_header_size']
    ...
    def is_padding_header(self, header, file_offset):
        if not header:
            return True
        ...
        if any(self.xor_key):
            ...
            matches_xor_pattern = True
            for i, byte in enumerate(header):
                if byte != self.xor_key[(file_offset + i) % key_len]:
                    matches_xor_pattern = False
                    break

            if matches_xor_pattern and header[:4] != self.settings['netmagic']:
                return True
    ...
    def run(self):
        while self.blkCountOut < len(self.blkindex):
            ...
            file_offset = self.inF.tell()
            inhdr = self.read_xored(self.inF, 8)
            if self.inFn == 0 and self.blkCountOut == 0 and file_offset == 0:
                print("Debug XOR key", self.xor_key.hex())
                print("Debug header", inhdr.hex())
                print("Debug netmagic", self.settings['netmagic'].hex())
                padding = self.is_padding_header(inhdr, file_offset)
                print("is_padding?", padding)
            else:
                padding = self.is_padding_header(inhdr, file_offset)
            if padding:
                print("Detected padding at file %i offset %i (written %i of %i)" % (
                    self.inFn, file_offset, self.blkCountOut, len(self.blkindex)))
                self.inF.close()
                self.inF = None
            ...
            if self.blkCountOut == 0 and self.blkCountIn == 0:
                print("blk_hdr bytes", blk_hdr.hex())
            inExtent = BlockExtent(self.inFn, self.inF.tell(), inhdr, blk_hdr, inLen)

            self.hash_str = calc_hash_str(blk_hdr, self.hash_header_size)
            if self.blkCountOut == 0 and self.blkCountIn == 0:
                print("First block hash", self.hash_str)
                print("In map?", self.hash_str in self.blkmap)
...
if __name__ == '__main__':
    ...
    if 'header_size' not in settings:
        settings['header_size'] = 80
    if 'hash_header_size' not in settings:
        settings['hash_header_size'] = settings['header_size']
    ...
    settings['header_size'] = int(settings['header_size'])
    settings['hash_header_size'] = int(settings['hash_header_size'])
