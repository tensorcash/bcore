...
class InitTest(BitcoinTestFramework):
    ...
    def set_test_params(self):
        ...
        self.rpc_timeout = 300  # Corrupted DB detection can take time
    ...
    def init_stress_test(self):
        ...
        def start_expecting_error(err_fragment):
            self.log.info(f"Starting node expecting error: '{err_fragment}'")
            node.assert_start_raises_init_error(
                extra_args=['-txindex=1', '-blockfilterindex=1', '-coinstatsindex=1', '-checkblocks=200', '-checklevel=4', '-debug=leveldb', '-logsourcelocations'],
        ...
        files_to_perturb = {
            ...
            'indexes/txindex/*.*': 'LevelDB error: Corruption',
        ...
        for file_patt, err_fragment in files_to_perturb.items():
            ...
            # INSTRUMENTATION: Capture pre-corruption state
            if 'blocks/index' in file_patt:
                index_dir = node.chain_path / "blocks" / "index"
                self.log.info(f"=== PRE-CORRUPTION STATE for {file_patt} ===")
                self.log.info(f"All files in blocks/index:")
                for f in sorted(index_dir.iterdir()):
                    self.log.info(f"  {f.name}: {f.stat().st_size} bytes, mtime={f.stat().st_mtime}")

                # Read MANIFEST to understand LSM state
                current_file = index_dir / "CURRENT"
                if current_file.exists():
                    manifest_name = current_file.read_text().strip()
                    self.log.info(f"CURRENT points to: {manifest_name}")
                    manifest_path = index_dir / manifest_name
                    if manifest_path.exists():
                        manifest_hash = hashlib.sha256(manifest_path.read_bytes()).hexdigest()
                        self.log.info(f"MANIFEST hash: {manifest_hash}, size: {manifest_path.stat().st_size}")
            elif 'indexes' in file_patt:
                pattern_path = Path(file_patt)
                parts = pattern_path.parts
                self.log.info(f"=== PRE-CORRUPTION STATE for {file_patt} ===")
                if 'db' in parts:
                    db_idx = parts.index('db')
                    db_dir = node.chain_path.joinpath(*parts[:db_idx + 1])
                    if db_dir.exists():
                    ...
                    base_dir = node.chain_path.joinpath(*parts[:-1])
                    if base_dir.exists():
            ...
            for target_file in target_files:
                ...
                with open(target_file, "r+b") as tf:
                    ...
                    # in blk*.dat is affected. The second block starts at offset 590
                    # with data from 598-1145, so we corrupt at 700 to ensure we're
                    # in the second block's transaction data.
                    # However, LevelDB files can be very small (< 700 bytes) after
                    # compaction. For these, we must corrupt from the beginning to
                    # ensure we hit actual data instead of writing past EOF.
                    file_size = tf.seek(0, 2)  # Get file size
                    if file_size >= 700:
                        tf.seek(700)
                        tf.write(b"1" * 350)
                    else:
                        # Small file: corrupt from the beginning
                        tf.seek(0)
                        tf.write(b"1" * min(350, file_size))

            # INSTRUMENTATION: Enable full LevelDB debugging for startup attempt
            def start_expecting_error_instrumented(err_fragment):
                self.log.info(f"Starting node with FULL leveldb debugging to detect corruption: expecting '{err_fragment}'")
                try:
                    node.assert_start_raises_init_error(
                        extra_args=['-txindex=1', '-blockfilterindex=1', '-coinstatsindex=1', '-checkblocks=200', '-checklevel=4', '-debug=leveldb', '-logsourcelocations'],
                        expected_msg=err_fragment,
                    ...
                    self.log.info(f"Node correctly failed with expected error")
                except Exception as e:
                    self.log.error(f"!!! UNEXPECTED: Node failed to detect corruption !!!")
                    self.log.error(f"Exception: {e}")
                    # Capture post-mortem state
                    if 'blocks/index' in file_patt:
                        index_dir = node.chain_path / "blocks" / "index"
                        self.log.info(f"=== POST-ATTEMPT STATE (FAILED TO DETECT CORRUPTION) ===")
                        self.log.info(f"Files in blocks/index after startup attempt:")
                        for f in sorted(index_dir.iterdir()):
                            self.log.info(f"  {f.name}: {f.stat().st_size} bytes, mtime={f.stat().st_mtime}")
                        current_file = index_dir / "CURRENT"
                        if current_file.exists():
                            manifest_name = current_file.read_text().strip()
                            self.log.info(f"CURRENT now points to: {manifest_name}")
                    raise

            start_expecting_error_instrumented(err_fragment)
