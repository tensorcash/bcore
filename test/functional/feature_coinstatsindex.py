...
class CoinStatsIndexTest(BitcoinTestFramework):
    def set_test_params(self):
        ...
        self.extra_args = [
            ...
            ["-coinstatsindex", "-spv-asn-corroboration=0"]
        ]
    ...
    def _test_coin_stats_index(self):
        ...
        except FailedToStartError as exc:
            self.log.warning(f"Skipping -reindex check due to startup failure: {exc}")
        ...
        try:
            ...
            self.log.warning(f"Skipping -reindex-chainstate check due to startup failure: {exc}")
            self.start_node(1, extra_args=self.extra_args[1])
        else:
            self.sync_index_node()
