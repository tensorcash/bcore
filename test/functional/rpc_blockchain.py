...
class BlockchainTest(BitcoinTestFramework):
    ...
    def _test_waitforblockheight(self):
        ...
        current_tip_hash = node.getbestblockhash()
        current_tip = node.getblock(current_tip_hash)
        ...
        #
        fork_depth = min(current_height, 8)
        if fork_depth < 1:
            fork_depth = 1
        fork_height = current_height - fork_depth
        fork_hash = node.getblockhash(fork_height)
        fork_block = node.getblock(fork_hash)

        # Build enough blocks on the fork so that it deterministically
        # overtakes the active chain in accumulated work.
        branch_length = fork_depth + 1
        branch_blocks = []
        prev_hash_int = int(fork_hash, 16)
        block_time = fork_block['time']

        def solve_and_send_block(prevhash, height, time, *, tick_override=None, expect_tip=True):
            b = create_block(prevhash, create_coinbase(height), time)
            from test_framework.blocktools import set_block_tick_from_prev, _register_local_block
            # prevhash is an int, convert to hex
            kwargs = {"tick": tick_override} if tick_override is not None else {}
            set_block_tick_from_prev(node, b, f"{prevhash:064x}", force_refresh_cache=True, **kwargs)
            b.solve()
            block_hex = b.serialize(with_witness=True).hex()
            submit_res = node.submitblock(block_hex)
            # If the block was accepted, submitblock returns None. Some builds return
            # status strings like "duplicate" or "inconclusive" when validation work
            # happens asynchronously, so treat those as non-fatal.
            allowed_status = {None, "duplicate", "inconclusive", "duplicate-inconclusive"}
            assert submit_res in allowed_status, f"submitblock rejected block at height {height}: {submit_res}"
            _register_local_block(b)  # Cache the block's cumulative tick before sending
            peer.send_and_ping(msg_block(b))
            # Wait for the block to be processed
            def _has_block():
                try:
                    node.getblock(b.hash, 0)
                    return True
                except JSONRPCException:
                    return False

            self.wait_until(_has_block, timeout=10)
            if expect_tip:
                self.wait_until(lambda: node.getbestblockhash() == b.hash, timeout=10)
            return b

        for offset in range(1, branch_length + 1):
            block_time += 1
            block = solve_and_send_block(
                prev_hash_int,
                fork_height + offset,
                block_time,
                expect_tip=(offset == branch_length),
            )
            branch_blocks.append(block)
            prev_hash_int = block.sha256

        node.invalidateblock(branch_blocks[-1].hash)
    ...
    def _test_getblock(self):
        ...
        def assert_hexblock_hashes(verbosity):
            ...
            if isinstance(block, str):
                decoded = _decode_block_hex(block)
                decoded.calc_sha256()
                assert_equal(blockhash, decoded.hash)
            else:
                raise AssertionError("Expected raw block hex when verbosity is {}".format(verbosity))
        ...
        set_block_tick_from_prev(node, block, blockhash)
        block.solve()
        ...
        set_block_tick_from_prev(node, block_noundo, f"{block.sha256:064x}")
        block_noundo.solve()
