...
class AssumeutxoTest(BitcoinTestFramework):
    ...
    def set_test_params(self):
        ...
        self.extra_args = [
            ...
            ["-fastprune", "-prune=1", "-blockfilterindex=1", "-coinstatsindex=1", "-spv-asn-corroboration=0"],
            ["-persistmempool=0","-txindex=1", "-blockfilterindex=1", "-coinstatsindex=1", "-spv-asn-corroboration=0"],
            ["-spv-asn-corroboration=0"]
        ]
    ...
    def test_headers_not_synced(self, valid_snapshot_path):
        expected_hash = self.nodes[0].getblockhash(SNAPSHOT_BASE_HEIGHT)
        for node in self.nodes[1:]:
            msg = (
                "Unable to load UTXO snapshot: The base block header "
                f"({expected_hash}) must appear in the headers chain. Make sure all headers are syncing, and call loadtxoutset again."
            )
    ...
    def test_snapshot_block_invalidated(self, dump_output_path):
        ...
        for height in [SNAPSHOT_BASE_HEIGHT, SNAPSHOT_BASE_HEIGHT - 1]:
            ...
            msg = "Unable to load UTXO snapshot: The base block header (e92bdf2134abaa5e6fcd69608de1a03ba84c27fdab9ad5c7e680a68af06faa3e) is part of an invalid chain."
            assert_raises_rpc_error(-32603, msg, node.loadtxoutset, dump_output_path)
    ...
    def test_snapshot_not_on_most_work_chain(self, dump_output_path):
        ...
        set_block_tick_from_prev(node0, fork_block1, parent_block_hash)
        fork_block1.solve()
        fork_block2 = create_block(fork_block1.sha256, create_coinbase(SNAPSHOT_BASE_HEIGHT + 1), block_time + 1)
        set_block_tick(fork_block2, tick=getattr(fork_block1.pow, 'tick', 1000), prev_block=fork_block1)
        fork_block2.solve()
    ...
    def test_sync_from_assumeutxo_node(self, snapshot):
        ...
        for block_num in range(1, miner.getblockcount() + 1):
            block_hash = miner.getblockhash(block_num)
            block_hex = miner.getblock(block_hash, 0)
            snapshot_node.submitheader(block_hex)
    ...
    def run_test(self):
        ...
        # Submit headers for blocks 200-299 that n0 generated but others haven't seen


        for i in range(200, 300):
            # Get the FULL BLOCK (header + proof blob) for submission
            block_hash = n0.getblockhash(i)
            # getblock with verbose=0 returns the full serialized block including proof blob
            block_hex = n0.getblock(block_hash, 0)
            # Submit the full block data to submitheader
            # The RPC will extract header + proof blob for validation
            n1.submitheader(block_hex)
            n2.submitheader(block_hex)
            n3.submitheader(block_hex)
        ...
        prev_timeout = self.rpc_timeout
        self.rpc_timeout = max(self.rpc_timeout, 900)
        try:
            self.restart_node(0, ["-reindex"])
        finally:
            self.rpc_timeout = prev_timeout
        ...
        # TensorCash blocks include the VDF sidecar, so snapshot-chain files can spill over
        # to an additional blkNNNN.dat even when -fastprune is active. We require one
        # blockfile for pre-snapshot data and at least one for the assumed chainstate.
        blockfiles = sorted(path.stem[3:] for path in n1.blocks_path.glob("blk*.dat"))
        assert blockfiles, "no blockfiles found"
        assert blockfiles[0] == "00000", "normal blockfile missing"
        assumed_files = [num for num in blockfiles if num != "00000"]
        assert assumed_files, "assumed blockfile missing"
        assert len(blockfiles) <= 5, f"unexpected number of blockfiles: {blockfiles}"
