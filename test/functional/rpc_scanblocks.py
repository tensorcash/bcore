...
class ScanblocksTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        def find_false_positive(target_hash, block_hash, avoid_script):
            prefix = b"\x00\x14"
            limit = 5_000_000
            for i in range(limit):
                candidate = prefix + i.to_bytes(20, 'little')
                if candidate == avoid_script:
                    continue
                if bip158_basic_element_hash(candidate, 1, block_hash) == target_hash:
                    self.log.debug(f"Found false positive witness program after {i} iterations")
                    return candidate
            raise AssertionError("Unable to find compact filter false positive within search limit")

        false_positive_spk = find_false_positive(genesis_coinbase_hash, genesis_blockhash, genesis_coinbase_spk)
