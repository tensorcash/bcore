...
class HTTPBasicsTest(BitcoinTestFramework):
    ...
    def conf_setup(self):
        ...
        # Try to get RPCAUTH from config, fallback to default location
        try:
            gen_rpcauth = config['environment']['RPCAUTH']
        except KeyError:
            # Fallback to default location relative to the source directory
            srcdir = config['environment'].get('SRCDIR', '/build/bcore')
            gen_rpcauth = os.path.join(srcdir, 'share', 'rpcauth', 'rpcauth.py')
            if not os.path.exists(gen_rpcauth):
                self.log.warning("RPCAUTH script not found, skipping rpcauth generation tests")
                return
