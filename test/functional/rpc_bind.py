...
class RPCBindTest(BitcoinTestFramework):
    ...
    def _run_loopback_tests(self):
        if self.options.run_ipv4:
            ...
            # Note: IPv6 might not actually bind even if supposedly available
            try:
                self.run_bind_test(None, '127.0.0.1', [],
                    [('127.0.0.1', self.defaultport), ('::1', self.defaultport)])
            except AssertionError:
                # Fallback: IPv6 not actually available, test IPv4 only
                self.stop_nodes()  # Stop node before retry
                self.run_bind_test(None, '127.0.0.1', [],
                    [('127.0.0.1', self.defaultport)])
            ...
            try:
                ...
                # IPv6-only binding may fail even if IPv6 is available
                self.log.info("IPv6-only binding failed, skipping")
            # check both IPv4 and IPv6 localhost (explicit)
            try:
                self.run_bind_test(['127.0.0.1'], '127.0.0.1', ['127.0.0.1', '[::1]'],
                    [('127.0.0.1', self.defaultport), ('::1', self.defaultport)])
            except AssertionError:
                # IPv6 might not bind even when specified with IPv4
                self.log.info("Dual IPv4/IPv6 binding failed, trying IPv4 only")
                self.stop_nodes()
                self.run_bind_test(['127.0.0.1'], '127.0.0.1', ['127.0.0.1'],
                    [('127.0.0.1', self.defaultport)])
