...
class TxConflicts(BitcoinTestFramework):
    ...
    def no_op(self):
        """No-op sync function for selective operations."""
        pass

    def get_utxo_of_value(self, from_tx_id, search_value):
        return next(tx_out["vout"] for tx_out in self.nodes[0].gettransaction(from_tx_id)["details"] if tx_out["amount"] == Decimal(str(search_value)))
    ...
    def test_mempool_conflict(self):
        # Ensure proper test isolation by disconnecting nodes that may be connected from previous test
        try:
            self.disconnect_nodes(0, 2)
        except:
            pass  # Nodes might not be connected
        ...
        # Reconnect and sync to ensure alice receives the confirmed transactions
        self.connect_nodes(0, 2)
        self.sync_blocks([self.nodes[0], self.nodes[2]])
        ...
        self.sync_blocks()  # Ensure all nodes sync the cleanup transaction to prevent test bleeding
    ...
    def test_mempool_and_block_conflicts(self):
        ...
        # Ensure bob's wallet has fully processed the reorg before checking balances
