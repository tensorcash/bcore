...
class AcceptBlockTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        # Refresh cumulative tick cache from nodes to ensure parent blocks are cached
        for i, node in enumerate(self.nodes):
            refresh_cumulative_tick_cache_from_node(node, format(tips[i], '064x'))
        ...
        refresh_cumulative_tick_cache_from_node(self.nodes[0], genesis_hash)
        block_h1f = create_block(int("0x" + genesis_hash, 0), create_coinbase(1), block_time)
        block_time += 1
        block_h1f.solve()
