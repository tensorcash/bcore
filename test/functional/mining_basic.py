...
class MiningTest(BitcoinTestFramework):
    def set_test_params(self):
        ...
        self.extra_args = [
            ...
            ["-spv-asn-corroboration=0"],
            ["-fastprune", "-prune=1", "-spv-asn-corroboration=0"]
        ]
    ...
    def test_timewarp(self):
        ...
        block.nAdjBits = block.nBits  # Initialize Tensor field
        block.nNonce = 0
        block.vtx = [create_coinbase(height=int(tmpl["height"]))]
        set_block_tick_from_prev(node, block, tmpl["previousblockhash"]) 
        block.solve()
        assert_template(node, block, None)

        bad_block = copy.deepcopy(block)
        bad_block.nTime = t
        # Re-initialize VDF fields after modifying time
        set_block_tick_from_prev(node, bad_block, tmpl["previousblockhash"])
        bad_block.solve()
        assert_raises_rpc_error(-25, 'time-timewarp-attack', lambda: node.submitheader(hexdata=bad_block.serialize().hex()))
        ...
        set_block_tick_from_prev(node, bad_block, tmpl["previousblockhash"])
        bad_block.solve()
        node.submitheader(hexdata=bad_block.serialize().hex())
    ...
    def test_block_max_weight(self):
        ...
        LARGE_TXS_COUNT = 10
        available_weight = MAX_BLOCK_WEIGHT - DEFAULT_BLOCK_RESERVED_WEIGHT
        LARGE_VSIZE = int(((MAX_BLOCK_WEIGHT - DEFAULT_BLOCK_RESERVED_WEIGHT) / WITNESS_SCALE_FACTOR) / LARGE_TXS_COUNT)
        large_tx_weight = LARGE_VSIZE * WITNESS_SCALE_FACTOR
        assert_equal(available_weight, LARGE_TXS_COUNT * large_tx_weight)

        NORMAL_VSIZE = int(2000 / WITNESS_SCALE_FACTOR)
        normal_tx_weight = NORMAL_VSIZE * WITNESS_SCALE_FACTOR

        HIGH_FEERATE = Decimal("0.0003")
        self.restart_node(0, extra_args=[f"-datacarriersize={LARGE_VSIZE}", "-acceptnonstdtxn"])

        # Ensure the mempool is empty
        assert_equal(len(self.nodes[0].getrawmempool()), 0)

        # Generate UTXOs and send the high-fee large transactions
        ...
        # Verify the block template includes only the high-fee transactions
        ...
        custom_available_weight = (LARGE_TXS_COUNT - 1) * large_tx_weight + 2 * normal_tx_weight
        custom_block_weight = custom_available_weight + DEFAULT_BLOCK_RESERVED_WEIGHT
        assert custom_block_weight < MAX_BLOCK_WEIGHT
        # Adjust the maximum block weight so that one large transaction must be dropped and replaced with lower-fee ones.
        self.restart_node(0, extra_args=[f"-datacarriersize={LARGE_VSIZE}", "-acceptnonstdtxn", f"-blockmaxweight={custom_block_weight}"])

        self.log.info(
            "Testing the block template with custom -blockmaxweight to include "
            f"{LARGE_TXS_COUNT - 1} large and 2 normal transactions."
        )
        self.verify_block_template(
            expected_tx_count=(LARGE_TXS_COUNT - 1) + 2,
            expected_weight=custom_available_weight,
        )
        ...
        self.restart_node(0, extra_args=[f"-datacarriersize={LARGE_VSIZE}", "-acceptnonstdtxn", f"-blockmaxweight={MAX_BLOCK_WEIGHT}"])
        ...
        large_tx_weight = LARGE_VSIZE * WITNESS_SCALE_FACTOR
        normal_tx_weight = NORMAL_VSIZE * WITNESS_SCALE_FACTOR
        mempool_count = len(self.nodes[0].getrawmempool())
        normal_tx_count = max(0, mempool_count - LARGE_TXS_COUNT)
        expected_weight = LARGE_TXS_COUNT * large_tx_weight + normal_tx_count * normal_tx_weight
        self.log.info(f"Testing that the mempool's weight matches the expected weight: {expected_weight}.")
        assert_equal(mempool_weight, expected_weight)

        self.log.info(f"Testing that the mempool's weight stays within the maximum block weight: {MAX_BLOCK_WEIGHT}.")
        assert_greater_than_or_equal(MAX_BLOCK_WEIGHT, mempool_weight)

        self.log.info(
            f"Testing that the block template includes only {LARGE_TXS_COUNT} transactions and cannot reach full block weight."
        )
        self.verify_block_template(
            expected_tx_count=LARGE_TXS_COUNT,
            expected_weight=available_weight,
        ...
        self.restart_node(0, extra_args=[f"-datacarriersize={LARGE_VSIZE}", "-acceptnonstdtxn", "-blockreservedweight=4000"])
        self.verify_block_template(
    ...
    def run_test(self):
        ...
        # Re-initialize VDF fields for modified block
        set_block_tick_from_prev(node, bad_block, tmpl["previousblockhash"])
        bad_block.solve()
        ...
        # Use header-only serialization to avoid CProofBlob interference
        bad_block_sn = bytearray(block.serialize())
        # Truncate to header + txs (remove CProofBlob + cumulative_tick to ensure clean failure)
        header_and_txs_end = BLOCK_HEADER_SIZE + 1 + len(block.vtx[0].serialize_with_witness())
        bad_block_sn = bad_block_sn[:header_and_txs_end]
        assert_equal(bad_block_sn[BLOCK_HEADER_SIZE], 1)
        bad_block_sn[BLOCK_HEADER_SIZE] = 2  # Corrupt tx count to 2
        assert_raises_rpc_error(-22, "Block decode failed", node.getblocktemplate, {
        ...
        assert_raises_rpc_error(-22, 'Block header decode failed', lambda: node.submitheader(hexdata='00' * 40))  # Too short to be valid header (80 byte minimum)
        assert_raises_rpc_error(-25, 'Must submit previous header', lambda: node.submitheader(hexdata=super(CBlock, bad_block).serialize().hex()))

        block.nTime += 1
        # Re-initialize VDF fields after modifying block time
        # Don't pass prev_hash - let it validate against block.hashPrevBlock
        ...
        def chaintips_contains(expected_tip, *, tips=None):
            """Return True if any chaintip matches all fields in expected_tip."""
            tips = tips or node.getchaintips()
            for tip in tips:
                if all(tip.get(key) == value for key, value in expected_tip.items()):
                    return True
            return False

        assert not chaintips_contains(chain_tip(block.hash))
        node.submitheader(hexdata=block.serialize().hex())
        assert chaintips_contains(chain_tip(block.hash))
        node.submitheader(hexdata=block.serialize().hex())  # Noop with full block data
        assert chaintips_contains(chain_tip(block.hash))

        bad_block_root = copy.deepcopy(block)
        bad_block_root.hashMerkleRoot += 2
        # Re-initialize VDF fields for the copied block
        set_block_tick_from_prev(node, bad_block_root)
        bad_block_root.solve()
        assert not chaintips_contains(chain_tip(bad_block_root.hash))
        node.submitheader(hexdata=bad_block_root.serialize().hex())
        assert chaintips_contains(chain_tip(bad_block_root.hash))
        ...
        # Re-initialize VDF fields after modifying the block
        set_block_tick_from_prev(node, bad_block_lock)
        ...
        # Cannot use set_block_tick_from_prev as bad_block_lock is not on chain
        # Just solve without VDF fields - this should fail anyway
        bad_block2.solve()
        # Submitting a full block whose parent is invalid fails at VDF validation before prev-index checks.
        assert_raises_rpc_error(-25, 'VDF verification failed', lambda: node.submitheader(hexdata=bad_block2.serialize().hex()))

        # Should reject invalid header right away
        bad_block_time = copy.deepcopy(block)
        bad_block_time.nTime = 1
        # Re-initialize VDF fields after modifying time
        set_block_tick_from_prev(node, bad_block_time)
        bad_block_time.solve()
        assert_raises_rpc_error(-25, 'time-too-old', lambda: node.submitheader(hexdata=bad_block_time.serialize().hex()))
        ...
        # Must be active now:
        assert chaintips_contains(chain_tip(block.hash, status='active', branchlen=0))

        # Building a few blocks should give the same results
        self.generatetoaddress(node, 10, node.get_deterministic_priv_key().address)
        assert_raises_rpc_error(-25, 'time-too-old', lambda: node.submitheader(hexdata=bad_block_time.serialize().hex()))
        # Same expectation once additional blocks have advanced the chain tip.
        assert_raises_rpc_error(-25, 'VDF verification failed', lambda: node.submitheader(hexdata=bad_block2.serialize().hex()))
        node.submitheader(hexdata=block.serialize().hex())
        node.submitheader(hexdata=bad_block_root.serialize().hex())
