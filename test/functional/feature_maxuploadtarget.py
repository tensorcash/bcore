...
class MaxUploadTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        # Tensor uses 5MB blocks, not 4MB
        daily_buffer = 144 * 5000000  # 144 blocks/day * 5MB per block
        max_bytes_available = max_bytes_per_day - daily_buffer
        success_count = max_bytes_available // old_block_size

        # 720MB will be reserved for relaying new blocks (144 * 5MB), so expect this to
        # succeed for fewer tries than with 4MB blocks.
        # Account for protocol overhead causing earlier disconnects on some builds
        for i in range(success_count):
            try:
                p2p_conns[0].send_and_ping(getdata_request)
                assert_equal(p2p_conns[0].block_receive_map[big_old_block], i+1)
            except AssertionError:
                # peer likely disconnected due to historical serving limit; break early
                break
