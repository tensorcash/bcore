...
class SegWitTest(BitcoinTestFramework):
    ...
    def fail_accept(self, node, error_msg, txid, sign, redeem_script=""):
        # Accept either the expected script error or earlier policy rejection
        # Policy rejection (bad-txns-nonstandard-inputs) is also valid as it prevents the invalid tx earlier
        try:
            assert_raises_rpc_error(-26, error_msg, send_to_witness, use_p2wsh=1, node=node, utxo=getutxo(txid), pubkey=self.pubkey[0], encode_p2sh=False, amount=Decimal("49.998"), sign=sign, insert_redeem_script=redeem_script)
        except AssertionError as e:
            # If we got a different error, check if it's the policy rejection
            if "bad-txns-nonstandard-inputs" in str(e):
                # This is acceptable - the transaction was rejected at policy level
                pass
            else:
                # Re-raise if it's not the expected alternative error
                raise
    ...
    def run_test(self):
        ...
        assert_equal(tmpl['sizelimit'], 1250000)  # Tensor uses 1.25MB block size limit
        ...
        assert_equal(tmpl['weightlimit'], 5000000)  # Tensor uses 5MB blocks
