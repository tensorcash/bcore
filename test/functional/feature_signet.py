...
class SignetBasicTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        # Generate and capture valid signet blocks for tensorcash
        # We'll use node3 (also default signet) to generate blocks
        import json
        import os

        # First, check if we need to generate blocks (tensorcash has different genesis)
        genesis_hash = self.nodes[2].getblockhash(0)
        tensorcash_genesis = "6e4dc93b60255aff3a1cae5bae7b1135c933f41a4f35581db2f3f5ee56eee6a4"

        if genesis_hash == tensorcash_genesis:
            self.log.info(f"Generating valid tensorcash signet blocks (genesis: {genesis_hash})")

            # Generate 10 blocks on node3 and capture them
            generated_blocks = []
            for i in range(10):
                blockhash = self.generate(self.nodes[3], 1, sync_fun=self.no_op)[0]
                block_hex = self.nodes[3].getblock(blockhash, 0)
                generated_blocks.append(block_hex)
                self.log.info(f"Generated block {i+1}: {blockhash}")

            # Save the blocks to file
            output_file = os.path.join(self.options.tmpdir, "tensorcash_signet_blocks.json")
            with open(output_file, 'w') as f:
                json.dump({
                    "genesis_hash": genesis_hash,
                    "generated_blocks": generated_blocks,
                    "comment": "These blocks are valid for tensorcash signet. Replace signet_blocks array with these."
                }, f, indent=2)

            self.log.info(f"Saved {len(generated_blocks)} valid blocks to: {output_file}")

            # Also save as Python array format for easy copy-paste
            py_output_file = os.path.join(self.options.tmpdir, "tensorcash_signet_blocks.py")
            with open(py_output_file, 'w') as f:
                f.write("# Valid signet blocks for tensorcash\n")
                f.write(f"# Genesis: {genesis_hash}\n")
                f.write("signet_blocks = [\n")
                for block in generated_blocks:
                    f.write(f"    '{block}',\n")
                f.write("]\n")

            self.log.info(f"Also saved as Python array to: {py_output_file}")

            # Now submit these blocks to node2 to verify they work
            self.nodes[2].invalidateblock(self.nodes[2].getbestblockhash())  # Reset to genesis
            height = 0
            for block in generated_blocks:
            ...
            self.log.info(f"Successfully submitted all {height} blocks to node2")

        else:
            # Standard signet - use original blocks
            height = 0
        ...
        # Use the appropriate block for the incompatible test
        if genesis_hash == tensorcash_genesis:
            # Use first generated block for incompatible test
            if generated_blocks:
                assert_equal(self.nodes[4].submitblock(generated_blocks[0]), 'bad-signet-blksig')
        else:
