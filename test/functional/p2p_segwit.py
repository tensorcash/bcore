...
class UTXO():
    ...
    def __init__(self, sha256, n, value, witness_stack=None):
        self.sha256 = sha256
        self.n = n
        self.nValue = value
...
def test_witness_block(node, p2p, block, accepted, with_witness=True, reason=None):
    ...
    reason_list = [reason] if reason else []
    if reason == 'bad-txnmrklroot':
        reason_list.extend([
            'DataStream::read(): end of data',
            'CDataStream::read(): end of data',
        ])
    message = msg_block(block) if with_witness else msg_no_witness_block(block)
    payload_size = len(message.serialize())

    if hasattr(node, 'log'):
        node.log.debug(
            "test_witness_block: payload_size=%d bytes, max_protocol=%d, weight=%d",
            payload_size,
            MAX_PROTOCOL_MESSAGE_LENGTH,
            block.get_weight(),
        )
    ...
    if payload_size > MAX_PROTOCOL_MESSAGE_LENGTH:
        # Fallback to RPC submission when the block payload exceeds the protocol limit.
        prev_best = node.getbestblockhash()
        block_hex = block.serialize().hex()
        with node.assert_debug_log(expected_msgs=reason_list):
            result = node.submitblock(block_hex)
        if accepted:
            assert_equal(result, None)
            node.wait_until(lambda: node.getbestblockhash() == block.hash)
        else:
            if reason_list:
                assert_equal(result, reason_list[0])
            time.sleep(0.1)
    ...
    with node.assert_debug_log(expected_msgs=reason_list):
        p2p.send_and_ping(message)
        # Be tolerant of async tip updates: poll briefly
        if accepted:
            node.wait_until(lambda: node.getbestblockhash() == block.hash)
        else:
            # Give node a moment to reject before checking
            time.sleep(0.1)
...
class SegWitTest(BitcoinTestFramework):
    def set_test_params(self):
        ...
        self.extra_args = [
            ...
            # validation).
            ["-acceptnonstdtxn=1", f"-testactivationheight=segwit@{SEGWIT_HEIGHT}", "-par=1", "-spv-asn-corroboration=0"],
            ["-acceptnonstdtxn=0", f"-testactivationheight=segwit@{SEGWIT_HEIGHT}", "-spv-asn-corroboration=0"],
        ]
    ...
    def build_next_block(self):
        ...
        # Ensure Tensor tick/cumulative fields are consistent with prev block
        from test_framework.blocktools import set_block_tick_from_prev
        set_block_tick_from_prev(self.nodes[0], block, tip)
        block.rehash()
        return block
    ...
    def run_test(self):
        # Connect the nodes to each other so blocks can propagate
    ...
    def test_non_witness_transaction(self):
        ...
        # Wait for the block to be accepted by node0
        self.wait_until(lambda: self.nodes[0].getbestblockhash() == block.hash, timeout=10)
        # Relay the pre-activation block to node1 directly; node0 can defer
        # relay until external validation completes, blocking sync otherwise.
        self.std_node.send_and_ping(msg_no_witness_block(block))
        # Ensure nodes are synced - they should be connected but may need explicit sync
    ...
    def test_witness_block_size(self):
        ...
        def _peer_connected_to_node1():
            for peer in self.nodes[0].getpeerinfo():
                if "testnode1" in peer.get("subver", ""):
                    return True
            return False

        rejoin_after = _peer_connected_to_node1()
        if rejoin_after:
        ...
        res = test_witness_block(self.nodes[0], self.test_node, block, accepted=True)
        if res["used_rpc"] and res["block_hex"]:
            for peer_node in self.nodes[1:]:
                peer_node.submitblock(res["block_hex"])
    ...
    def test_segwit_versions(self):
        ...
        # Prepare Taproot v2 metadata for script-path validation.
        v2_internal_seckey = ECKey()
        v2_internal_seckey.generate()
        v2_internal_key = compute_xonly_pubkey(v2_internal_seckey.get_bytes())[0]
        v2_tap_info = taproot_construct(v2_internal_key, [("v2_leaf", witness_script)])
        v2_leaf = v2_tap_info.leaves["v2_leaf"]
        v2_control_block = bytes([v2_leaf.version | v2_tap_info.negflag]) + v2_tap_info.internal_pubkey + v2_leaf.merklebranch
        v2_leaf_bytes = bytes(v2_leaf.script)
        v2_script_pubkey = CScript([CScriptOp(OP_2), v2_tap_info.output_pubkey])
        v2_utxo_index = None
        ...
        for version in list(range(OP_1, OP_16 + 1)) + [OP_0]:
            ...
            if version == OP_1:
                ...
                script_pubkey = v2_script_pubkey
            else:
            ...
            if version == OP_2:
                v2_utxo_index = len(temp_utxo) - 1

        self.generate(self.nodes[0], 1)  # Mine all the transactions
        assert len(self.nodes[0].getrawmempool()) == 0

        # Witness v2 script-path spends remain valid.
        assert v2_utxo_index is not None
        script_pubkey = v2_script_pubkey
        v2_utxo = temp_utxo[v2_utxo_index]

        tx2 = CTransaction()
        tx2.vin = [CTxIn(COutPoint(v2_utxo.sha256, v2_utxo.n), b"")]
        tx2.vout = [CTxOut(v2_utxo.nValue - 1000, script_pubkey)]
        tx2.wit.vtxinwit.append(CTxInWitness())
        tx2.wit.vtxinwit[0].scriptWitness.stack = [v2_leaf_bytes, v2_control_block]
        tx2.rehash()
        test_transaction_acceptance(self.nodes[0], self.test_node, tx2, with_witness=True, accepted=True)
        test_transaction_acceptance(self.nodes[1], self.std_node, tx2, with_witness=True, accepted=True)
        temp_utxo[v2_utxo_index] = UTXO(tx2.sha256, 0, tx2.vout[0].nValue, witness_stack=[v2_leaf_bytes, v2_control_block])

        # Spend everything in temp_utxo into a segwit v1 output.
        tx3 = CTransaction()
        total_value = 0
        for idx, entry in enumerate(temp_utxo):
            tx3.vin.append(CTxIn(COutPoint(entry.sha256, entry.n), b""))
            tx3.wit.vtxinwit.append(CTxInWitness())
            tx3.wit.vtxinwit[idx].scriptWitness.stack = [witness_script]
            total_value += entry.nValue
        tx3.wit.vtxinwit[v2_utxo_index].scriptWitness.stack = [v2_leaf_bytes, v2_control_block]
        ...
        self.utxo.append(UTXO(tx3.sha256, 0, tx3.vout[0].nValue, witness_stack=[v2_leaf_bytes, v2_control_block]))
    ...
    def test_uncompressed_pubkey(self):
        ...
        if utxo.witness_stack is not None:
            tx.wit.vtxinwit.append(CTxInWitness())
            tx.wit.vtxinwit[0].scriptWitness.stack = list(utxo.witness_stack)
