...
# Local policy in this tree relays transaction versions up to
# Consensus::MODEL_CHALLENGE_COMMIT_TX_VERSION (see src/policy/policy.h).
TX_MAX_STANDARD_VERSION = 9
...
ERR_SCHNORR_SIG_HASHTYPE = {"err_msg": ("Invalid Schnorr signature hash type", "Taproot key-path spend disabled for this output")}
ERR_SCHNORR_SIG = {"err_msg": "Invalid Schnorr signature"}
ERR_OP_RETURN = {"err_msg": ("OP_RETURN was encountered", "Taproot key-path spend disabled for this output")}
...
ERR_BAD_OPCODE_OR_KEYPATH = {"err_msg": ("Opcode missing or not understood", "Taproot key-path spend disabled for this output")}
ERR_TAPROOT_KEYPATH_DISABLED = {"err_msg": "Taproot key-path spend disabled for this output"}
...
def spenders_taproot_active():
    ...
    for leafver in range(0, 0x100, 2):
        ...
        add_spender(spenders, "unkver/undecodable", standard=False, tap=tap, leaf="undecodable_unkver", failure={"leaf": "undecodable_c0"}, **ERR_BAD_OPCODE_OR_KEYPATH)
    ...
    for opval in range(76, 0x100):
        ...
        add_spender(spenders, "opsuccess/undecodable", standard=False, tap=tap, leaf="undecodable_success", failure={"leaf": "undecodable_nop"}, **ERR_BAD_OPCODE_OR_KEYPATH)
        add_spender(spenders, "opsuccess/undecodable_bypass", standard=False, tap=tap, leaf="undecodable_success", failure={"leaf": "undecodable_bypassed_success"}, **ERR_BAD_OPCODE_OR_KEYPATH)
...
class TaprootTest(BitcoinTestFramework):
    ...
    def block_submit(self, node, txs, msg, err_msg, cb_pubkey=None, fees=0, sigops_weight=0, witness=False, accept=False):
        ...
        set_block_tick_from_prev(self.nodes[0], block, f"{self.tip:064x}")
        witness and add_witness_commitment(block)
        block.solve()
        block_response = node.submitblock(block.serialize().hex())
        if err_msg is not None:
            expected_errors = []
            if isinstance(err_msg, (list, tuple, set)):
                expected_errors = list(err_msg)
            else:
                expected_errors = [err_msg]
            assert block_response is not None, "Missing error message '%s' from block response '(None)': %s" % (expected_errors[0], msg)
            assert any(e in block_response for e in expected_errors), "Missing error message '%s' from block response '%s': %s" % (expected_errors[0], block_response, msg)
        if accept:
    ...
    def test_spenders(self, node, spenders, input_counts):
        ...
        while left:
            ...
            tx = CTransaction()
            # Generate random version but mask off bit 30 (ROTATION_TX_FLAG = 0x40000000)
            # to avoid triggering TensorCash rotation validation
            random_version = random.getrandbits(32) & ~(1 << 30)
            tx.version = random.choice([1, 2, random_version])
            ...
            for fail_input in list(range(len(input_utxos))) + [None]:
                ...
                is_standard_tx = (
                    ...
                    and tx.version <= TX_MAX_STANDARD_VERSION)
                tx.rehash()
    ...
    def gen_test_vectors(self):
        ...
        # Mine it
        prev_hash_hex = self.nodes[0].getbestblockhash()
        block = create_block(hashprev=int(prev_hash_hex, 16), coinbase=coinbase)
        set_block_tick_from_prev(self.nodes[0], block, prev_hash_hex)
        block.rehash()
        block.solve()
