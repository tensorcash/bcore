...
class InvalidTxRequestTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        set_block_tick_from_prev(self.nodes[0], block, f"{tip:064x}")
        block.solve()
        ...
        set_block_tick_from_prev(self.nodes[0], block_A, f"{tip:064x}")
        ...
        set_block_tick_from_prev(self.nodes[0], block_B, f"{tip:064x}")
        ...
        with node.assert_debug_log(["Erased 1 orphan transaction(s) included or conflicted by block"]):
            # Check if peer is still connected, reconnect if needed
            if not node.p2ps[0].is_connected:
                self.log.info("Peer disconnected, reconnecting...")
