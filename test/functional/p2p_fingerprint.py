...
class P2PFingerprintTest(BitcoinTestFramework):
    ...
    def build_chain(self, nblocks, prev_hash, prev_height, prev_median_time, node=None):
        blocks = []
        for _ in range(nblocks):
            ...
            # Set up cumulative_tick for TensorCash if node is available
            if node is not None:
    ...
    def run_test(self):
        ...
        new_blocks = self.build_chain(5, block_hash, height, block_time, self.nodes[0])

        # Force reorg to a longer chain
        node0.send_headers_with_sidecars(new_blocks)
