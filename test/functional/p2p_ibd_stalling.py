...
class P2PIBDStallingTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        for _ in range(NUM_BLOCKS):
            ...
            # Set cumulative_tick for TensorCash before solving
            set_block_tick_from_prev(node, blocks[-1], tip_hex)
            blocks[-1].solve()
            tip = blocks[-1].sha256
            tip_hex = blocks[-1].hash
            block_time += 1
            height += 1
        ...
        # TensorCash blocks are larger due to additional fields (pow, cumulative_tick)

        # Wait for blocks to start being received and then stabilize
        # The staller will withhold one block, so we expect NUM_BLOCKS - 2 blocks to be downloaded
        def blocks_received_and_stable():
            bytes_now = self.total_bytes_recv_for_blocks()
            if bytes_now == 0:
                return False
            # Wait a bit and check if bytes are still increasing
            import time
            time.sleep(0.5)
            bytes_after = self.total_bytes_recv_for_blocks()
            # If bytes haven't changed, we've likely received all available blocks
            return bytes_after == bytes_now and bytes_now > 0

        self.wait_until(blocks_received_and_stable, timeout=300)
