...
class GetBlockFromPeerTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        node1_interface.send_headers_with_sidecars_and_ping(block)
        ...
        # TensorCash blocks are larger due to VDF proofs, so fewer blocks are kept after pruning
        ...
        self.sync_blocks([self.nodes[0], pruned_node])
        # TensorCash: Due to larger blocks, pruning behavior differs from Bitcoin
        pruneheight = 550  # Actual pruned height with TensorCash blocks
        assert_equal(pruned_node.pruneblockchain(700), pruneheight)
        assert_equal(pruned_node.getblock(pruned_block)["hash"], "548348c29981b18ec8d6d552f3e758774b77889f0f912e517570e8e61a08221c")
        ...
        self.generate(self.nodes[0], 250, sync_fun=self.no_op)
        self.sync_blocks([self.nodes[0], pruned_node])
        # TensorCash: Calculate expected pruned height based on actual behavior
        pruneheight = 783  # Actual pruned height after pruning to 1000 with TensorCash blocks
