...
class HelpRpcTest(BitcoinTestFramework):
    ...
    def test_client_conversion_table(self):
        ...
        for argname, convert in converts_by_argname.items():
            if all(convert) != any(convert):
                # Only allow dummy, psbt, and value to fail consistency check
                assert argname in ['dummy', "psbt", "value"], ('WARNING: conversion mismatch for argument named %s (%s)' % (argname, list(zip(all_methods_by_argname[argname], converts_by_argname[argname]))))
    ...
    def test_categories(self):
        ...
        components = ['Blockchain', 'Control', 'Cosign', 'Custom', 'Mining', 'Mock', 'Network', 'Pricing', 'Rawtransactions', 'Util']
