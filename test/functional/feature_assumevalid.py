...
class AssumeValidTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        for _ in range(100):
            ...
            set_block_tick(block, tick=getattr(getattr(block, 'pow', None), 'tick', 0) or 1000, prev_block=self.blocks[-1])
            block.solve()
            self.blocks.append(block)
            self.tip = block.sha256
            self.block_time += 1
        ...
        self.block_time += 1
        set_block_tick(block102, tick=getattr(getattr(block102, 'pow', None), 'tick', 0) or 1000, prev_block=self.blocks[-1])
        block102.solve()
