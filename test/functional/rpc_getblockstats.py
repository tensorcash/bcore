...
class GetblockstatsTest(BitcoinTestFramework):
    ...
    def set_test_params(self):
        ...
        # Enable wallet for this test - we need it to create transactions
        self.extra_args = [[]]  # Empty args = wallet enabled
    ...
    def generate_test_data(self, filename):
        ...
        # The wallet is already created and keys imported by the framework
        # when skip_if_no_wallet() sets self.uses_wallet = True
        self.log.info("Using wallet for test data generation")
        ...
        # Use fee_rate parameter instead of deprecated settxfee (fee_rate in sat/vB)
        self.nodes[0].sendtoaddress(address=address, amount=1, subtractfeefromamount=True, fee_rate=3)
        ...
        self.log.info(f"Generated {len(blocks)} blocks (heights 0-{len(blocks)-1})")
        self.log.info(f"Test data saved to {filename}")
    ...
    def run_test(self):
        ...
        if self.options.gen_test_data or not os.path.exists(test_data):
            action = "Generating" if self.options.gen_test_data else "Rebuilding missing"
            self.log.info(f"{action} rpc_getblockstats test vector at {test_data}")
            self.generate_test_data(test_data)
        else:
            self.load_test_data(test_data)

        required_tip = self.start_height + self.max_stat_pos
        current_tip = self.nodes[0].getblockcount()
        if current_tip < required_tip:
            blocks_to_generate = required_tip - current_tip
            self.log.info(f"Chain height {current_tip} is below required {required_tip}, generating {blocks_to_generate} block(s)")
            self.generate(self.nodes[0], blocks_to_generate)
            current_tip = self.nodes[0].getblockcount()
        assert current_tip >= required_tip
        ...
        # Log and verify the genesis hash for this chain
        self.log.info(f"Genesis block hash: {genesis_stats['blockhash']}")
        assert_equal(genesis_stats["blockhash"], "cf63c021e5a8ea0bc04cf7b12ab12e0a7902ef2dae5bdd8cbb3a0d41c3345da4")
        # Basic sanity checks for genesis block
        ...
        # These are the correct values from your fork's generated test data
