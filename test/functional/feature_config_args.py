...
class ConfArgsTest(BitcoinTestFramework):
    ...
    def test_config_file_parser(self):
        ...
        self.nodes[0].assert_start_raises_init_error(
            extra_args=[f'-conf={bad_conf_file_path}', '-chain=main'],
        ...
        reindex_warning = ['Warning: reindex=1 is set in the configuration file, which will significantly slow down startup. Consider removing or commenting out this option for better performance, unless there is currently a condition which makes rebuilding the indexes necessary']
        prev_timeout = self.rpc_timeout
        self.rpc_timeout = max(self.rpc_timeout, 900)
        try:
            with self.nodes[0].assert_debug_log(expected_msgs=reindex_warning):
                self.start_node(0)
        finally:
        ...
        self.nodes[0].assert_start_raises_init_error(extra_args=[f"-conf={main_conf_file_path}", "-allowignoredconf", "-chain=main"], expected_msg='Error: acceptnonstdtxn is not currently supported for main chain')
