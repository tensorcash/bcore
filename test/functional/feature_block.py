...
class CBrokenBlock(CBlock):
    ...
    def serialize(self, with_witness=False):
        ...
        # Tensor blocks append PoW blob data and cumulative tick after transactions.
        # Mirror the base class behaviour so that weight calculations stay accurate.
        r += self.pow.serialize()
        r += int(getattr(self, 'cumulative_tick', 0)).to_bytes(8, "little")
        return r
...
class FullBlockTest(BitcoinTestFramework):
    def set_test_params(self):
        ...
        self.extra_args = [[
            ...
            '-testactivationheight=bip34@10000',  # Keep BIP34 inactive for entire test so BIP30 duplicate coinbase remains valid
        ]]

    def run_test(self):
        logging.getLogger("TestFramework.p2p").setLevel(logging.INFO)
        ...
        # Create a new block
        # For BIP30 test, we need to create a coinbase transaction that can be duplicated later
        # Create the duplicate coinbase transaction directly (without creating a block)
        # This transaction will be used later in block 61 for BIP30 duplicate test
        duplicate_tx = None  # Will be set to an actual mined coinbase later

        self._genesis_cumulative_tick = None
        ...
        self.send_blocks([b25], success=False, reconnect=True)
        ...
        self.send_blocks([b27], success=False, reconnect=True)
        ...
        self.send_blocks([b29], success=False, reconnect=True)
        ...
        b45.nAdjBits = REGTEST_N_BITS  # Initialize Tensor field
        b45.vtx.append(non_coinbase)
        b45.hashMerkleRoot = b45.calc_merkle_root()
        from test_framework.blocktools import set_block_tick_from_prev
        set_block_tick_from_prev(self.nodes[0], b45, f"{self.tip.sha256:064x}")
        b45.solve()
        ...
        b46.nAdjBits = REGTEST_N_BITS  # Initialize Tensor field
        b46.vtx = []
        b46.hashMerkleRoot = 0
        from test_framework.blocktools import set_block_tick_from_prev
        set_block_tick_from_prev(self.nodes[0], b46, f"{b44.sha256:064x}")
        b46.solve()
        ...
        target = uint256_from_compact(b47.nAdjBits if b47.nAdjBits else b47.nBits)
        # Iterate the nonce until the Tensor PoW short hash exceeds the target, ensuring
        # the block has invalid work.
        b47.rehash()
        while b47.short_sha256 <= target:
        ...
        # Flip nBits to trigger bad-diffbits while keeping PoW tied to prior nAdjBits
        set_block_difficulty(b50, b50.nBits - 1, set_adj=False)
        b50.solve()
        ...
        # Capture a matured coinbase we will later duplicate for the BIP30 test.
        duplicate_tx = None
        duplicate_conf_before = 0
        for candidate_tx in out:
            candidate_tx.rehash()
            utxo = self.nodes[0].gettxout(txid=candidate_tx.hash, n=0)
            if utxo and utxo['confirmations'] > COINBASE_MATURITY:
                duplicate_tx = CTransaction(candidate_tx)
                duplicate_tx.rehash()
                duplicate_conf_before = utxo['confirmations']
                break

        assert duplicate_tx is not None, "Failed to locate matured coinbase for BIP30 test"
        ...
        b61.vtx[0] = CTransaction(duplicate_tx)
        b61.vtx[0].rehash()
        ...
        b_dup_2.vtx[0] = CTransaction(duplicate_tx)
        b_dup_2.vtx[0].rehash()
        b_dup_2 = self.update_block('dup_2', [])
        assert_equal(duplicate_tx.serialize(), b_dup_2.vtx[0].serialize())
        assert_equal(self.nodes[0].gettxout(txid=duplicate_tx.hash, n=0)['confirmations'], duplicate_conf_before)
        ...
        # Note: With new changes, bitcoind now disconnects us for sending a malformed block.
        # The reconnect=True parameter above handles the disconnect and reconnection.
        ...
        bip34_info = self.nodes[0].getblockchaininfo().get('softforks', {}).get('bip34', {})
        bip34_active = bip34_info.get('active', False)
        bip34_height = bip34_info.get('height')
        if bip34_active:
            ...
            details = "unknown"
            if bip34_height is not None:
                details = str(bip34_height)
            self.log.info(f"Skip invalid block header version check because BIP34 is inactive (activation height: {details})")
        ...
        if bip34_active:
            ...
            self.log.info("Skip bad-cb-height coinbase enforcement because BIP34 is inactive")
    ...
    def _read_genesis_cumulative_tick(self):
        cached = getattr(self, '_genesis_cumulative_tick', None)
        if cached is not None:
            return cached

        genesis_hash_hex = self.nodes[0].getblockhash(0)
        raw_block_hex = self.nodes[0].getblock(genesis_hash_hex, 0)

        genesis_block = CBlock()
        genesis_block.deserialize(BytesIO(bytes.fromhex(raw_block_hex)))

        self._genesis_cumulative_tick = getattr(genesis_block, 'cumulative_tick', 0)
        self.log.debug(f"[GENESIS] cached cumulative_tick={self._genesis_cumulative_tick}")
        return self._genesis_cumulative_tick
    ...
    def next_block(self, number, spend=None, additional_coinbase_value=0, *, script=None, version=4):
        ...
        # For the first block after genesis, get the actual genesis cumulative_tick from node
        if self.tip is None:
            # Query the cached genesis cumulative tick from the raw block
            genesis_cumulative = self._read_genesis_cumulative_tick()
            current_tick = getattr(block.pow, 'tick', 0)
            block.cumulative_tick = genesis_cumulative + current_tick
        else:
            # Subsequent blocks: accumulate tick from previous
            prev_cumulative = getattr(self.tip, 'cumulative_tick', 0)
            current_tick = getattr(block.pow, 'tick', 0)
            block.cumulative_tick = prev_cumulative + current_tick

        # Block is created. Find a valid nonce.
        # solve() will generate VDF proof using hashPrevBlock (base_block_hash)
        block.solve()
        target = uint256_from_compact(block.nAdjBits if block.nAdjBits else block.nBits)
        short_hash = getattr(block, 'short_sha256', None)
        if short_hash is None:
            block.rehash()
            short_hash = block.short_sha256
        assert short_hash <= target, (
            f"PoW solve failed for block {number}: short_hash {short_hash:064x} > target {target:064x}"
        )
    ...
    def update_block(self, block_number, new_transactions):
        ...
        # Ensure cumulative tick is consistent when re-solving
        if block.hashPrevBlock == self.genesis_hash:
            # This block follows genesis - get actual genesis cumulative_tick
            prev_cumulative = self._read_genesis_cumulative_tick()
        else:
            # Find the previous block in our blocks dict
            prev_block = None
            for prev_block_candidate in self.blocks.values():
                if prev_block_candidate.sha256 == block.hashPrevBlock:
                    prev_block = prev_block_candidate
                    break
            prev_cumulative = getattr(prev_block, 'cumulative_tick', 0) if prev_block else 0
        ...
        if short_hash is None:
            ...
            short_hash = block.short_sha256
        assert short_hash <= target, (
            f"PoW solve failed for block {block_number}: short_hash {short_hash:064x} > target {target:064x}"
        )
    ...
    def send_blocks(self, blocks, success=True, reject_reason=None, force_send=False, reconnect=False, timeout=960):
        ...
        # for block in blocks:
        #     self.log.info(f"[SENDING BLOCK] hash={block.sha256:064x}")
        #     self.log.info(f"  cumulative_tick={block.cumulative_tick}")
        #     self.log.info(f"  pow.tick={block.pow.tick}")
        #     self.log.info(f"  hashPrevBlock={block.hashPrevBlock:064x}")

        if reconnect:
            # For blocks that will cause disconnection, use expect_disconnect=True and verify state after reconnect
            self.helper_peer.send_blocks_and_test(blocks, self.nodes[0], success=False, reject_reason=reject_reason, force_send=force_send, timeout=timeout, expect_disconnect=True)
            self.reconnect_p2p(timeout=timeout)
            # After reconnection, verify the tip state (should remain unchanged for rejected blocks)
            if not success:
                assert blocks[-1].hash != self.nodes[0].getbestblockhash()
        else:
            self.helper_peer.send_blocks_and_test(blocks, self.nodes[0], success=success, reject_reason=reject_reason, force_send=force_send, timeout=timeout, expect_disconnect=False)
