...
class LoggingTest(BitcoinTestFramework):
    def set_test_params(self):
        ...
        # Clear TC_NODE_ARGS to avoid debug=1 being set in config
        import os
        if 'TC_NODE_ARGS' in os.environ:
            # Save the original value to restore later if needed
            self.original_tc_node_args = os.environ['TC_NODE_ARGS']
            # Remove debug=1 from the args but keep other settings
            args = os.environ['TC_NODE_ARGS'].split()
            filtered_args = [arg for arg in args if not arg.startswith('debug=')]
            os.environ['TC_NODE_ARGS'] = ' '.join(filtered_args)
