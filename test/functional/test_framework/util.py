#!/usr/bin/env python3
# Enable postponed evaluation of annotations to support builtin generics on Python < 3.9
...
def assert_equal(thing1, thing2, *args):
    ...
    if thing1 != thing2:
        error_msg = "not(%s == %s)" % (thing1, thing2)
        if args:
...
def write_config(config_path, *, n, chain, extra_config="", disable_autoconnect=True):
    ...
    elif chain == 'tensor-reg':
        # Boolean selector args drop the hyphen (-tensorreg etc.); the config
        # section keeps the canonical chain name.
        chain_name_conf_arg = 'tensorreg'
        chain_name_conf_section = 'tensor-reg'
    elif chain == 'tensor-test':
        chain_name_conf_arg = 'tensortest'
        chain_name_conf_section = 'tensor-test'
    else:
    ...
    with open(config_path, 'w', encoding='utf8') as f:
        ...
        # TensorCash: allow injecting extra node args via environment for test tuning
        try:
            import os as _os
            _extra = _os.getenv("TC_NODE_ARGS", "").strip()
            if _extra:
                for line in _extra.replace(",", " ").split():
                    if line:
                        f.write(str(line) + "\n")
        except Exception:
            pass
