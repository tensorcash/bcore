...
from test_framework.vdf_helper import create_headers_ext_sidecar, compute_pow_commitment
...
def _hash_to_bytes(value):
    intval = _uint256_to_int(value) & ((1 << 256) - 1)
    return intval.to_bytes(32, byteorder='little', signed=False)


def _build_sidecar_for_block(block):
    pow_blob = getattr(block, 'pow', None)
    if pow_blob is None:
        return None

    tick = getattr(pow_blob, 'tick', 0) or 0
    vdf_proof = getattr(pow_blob, 'vdf', b'') or b''

    try:
        block_hash_bytes = _hash_to_bytes(block.sha256)
        prev_hash_bytes = _hash_to_bytes(block.hashPrevBlock)
    except AttributeError:
        return None

    try:
        sidecar = create_headers_ext_sidecar(block_hash_bytes, prev_hash_bytes, pow_blob)
        if compute_pow_commitment is not None:
            commitment = compute_pow_commitment(pow_blob, use_merkle=True)
            expected = int.from_bytes(commitment, 'little')
            block_hashpow = getattr(block, 'hashPoW', None)
            if block_hashpow is None or block_hashpow != expected:
                return None
        return sidecar
    except Exception:
        return None
...
class P2PConnection(asyncio.Protocol):
    ...
    def send_headers_with_sidecars(self, blocks, *, is_decoy=False, include_sidecars=None):
        """Send headers (and associated VDF sidecars when available)."""
        ...
        for block in blocks:
            header = block if isinstance(block, CBlockHeader) else CBlockHeader(block)
            headers.append(header)

            if include_sidecars:
                sidecar_source = None
                if isinstance(block, CBlock) or hasattr(block, 'pow'):
                    sidecar_source = block
                elif isinstance(block, CBlockHeader) and hasattr(self, 'block_store'):
                    sidecar_source = self.block_store.get(header.sha256)

                if sidecar_source is not None:
                    sidecar = _build_sidecar_for_block(sidecar_source)
                    if sidecar is not None:
                        sidecar_entries.append((int(header.sha256), sidecar))

        if logger.isEnabledFor(logging.DEBUG):
            for hdr in headers:
                try:
                    hdr.calc_sha256()
                    raw = msg_headers([hdr], include_txcount=False).serialize()
                    serialized = raw[1:1 + BLOCK_HEADER_SIZE]
                    header_hex = getattr(hdr, 'hash', None) or f"{hdr.sha256:064x}"
                    short_hex = getattr(hdr, 'short_hash', None) or f"{hdr.short_sha256:064x}"
                    logger.debug(
                        "[HEADER DEBUG] hash=%s short=%s nAdjBits=%08x serialized=%s",
                        header_hex,
                        short_hex,
                        hdr.nAdjBits,
                        serialized.hex(),
                    ...
                    logger.debug("[HEADER DEBUG] failed to serialize header: %r", exc)

        headers_msg = msg_headers(headers, include_txcount=True)
        self.send_without_ping(headers_msg, is_decoy=is_decoy)
        if logger.isEnabledFor(logging.DEBUG):
            aggregate = headers_msg.serialize()
            logger.debug("[HEADERS MSG DEBUG] count=%d payload=%s", aggregate[0], aggregate[:min(256, len(aggregate))].hex())
        NODE_VDFSPV = 1 << 24
        if include_sidecars is None:
            include_sidecars = (self.nServices & NODE_VDFSPV) == NODE_VDFSPV
        send_sidecars = include_sidecars and bool(sidecar_entries)
        if send_sidecars:
            sidecars = []
            for key, sidecar in sidecar_entries:
                sidecars.append(sidecar)
                self._known_sidecars[key] = sidecar
                if len(self._known_sidecars) > 4096:
                    self._known_sidecars.pop(next(iter(self._known_sidecars)))
            from test_framework.vdf_messages import msg_headers_ext
            ext_msg = msg_headers_ext()
            ext_msg.sidecars = sidecars
            self.send_without_ping(ext_msg, is_decoy=is_decoy)

    def send_headers_with_sidecars_and_ping(self, blocks, *, is_decoy=False, include_sidecars=True):
        self.send_headers_with_sidecars(blocks, is_decoy=is_decoy, include_sidecars=include_sidecars)
        self.sync_with_ping()

    def _handle_getheadext(self, message):
        if not self._known_sidecars:
            return

        requested = []
        for entry in getattr(message, 'locator', []):
            try:
                key = int(entry)
            except TypeError:
                continue
            sidecar = self._known_sidecars.get(key)
            if sidecar is not None:
        ...
        if requested:
            from test_framework.vdf_messages import msg_headers_ext
            ext_msg = msg_headers_ext()
            ext_msg.sidecars = requested
            self.send_without_ping(ext_msg)

    def on_getheadext(self, message):
        self._handle_getheadext(message)
...
class P2PDataStore(P2PInterface):
    ...
    def on_getheaders(self, message):
        ...
        locator_hashes = {int(h) for h in locator.vHave}
        stop_hash = int(hash_stop)

        blocks = [self.block_store[self.last_block_hash]]
        while True:
            current = blocks[-1]
            current_hash = current.sha256
            if current_hash in locator_hashes or current_hash == stop_hash:
                break

            prev_block_hash = current.hashPrevBlock
            if prev_block_hash in self.block_store:
                blocks.append(self.block_store[prev_block_hash])
            else:
                logger.debug('block hash {} not found in block store'.format(hex(prev_block_hash)))
                break

        blocks = blocks[:MAX_HEADERS_RESULTS]
        blocks.reverse()
        if not blocks:
            return

        self.send_headers_with_sidecars(blocks)
    ...
    def send_blocks_and_test(self, blocks, node, *, success=True, force_send=False, reject_reason=None, expect_disconnect=False, timeout=60, is_decoy=False):
        ...
        with node.assert_debug_log(expected_msgs=reject_reason):
            ...
            if force_send:
                ...
                self.send_headers_with_sidecars(blocks, is_decoy=is_decoy)

                def _block_requested_or_disconnected() -> bool:
                    # Allow the wait to finish early if the peer disconnects after
                    # rejecting our headers. In that situation we won't ever see a
                    # getdata for the block, and the old behaviour would hang until
                    # the timeout (e.g. when the parent header was already banned).
                    return (
                        blocks[-1].sha256 in self.getdata_requests
                        or not self.is_connected
                    )

                if expect_disconnect:
                    self.wait_until(
                        _block_requested_or_disconnected,
                        timeout=timeout,
            ...
            if expect_disconnect:
                ...
                # Skip state verification when disconnected - caller should reconnect first
            else:
                self.sync_with_ping(timeout=timeout)
                # Only verify state when still connected
                if success:
