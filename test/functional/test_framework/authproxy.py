...
HTTP_TIMEOUT = 120  # Increased for ZK proof generation which can take 30-60 seconds
...
class AuthServiceProxy():
    ...
    def _get_response(self):
        ...
        try:
            ...
            # Special handling for "stop" command which may close connection before response
            if self._service_name == "stop":
                # For stop command, we can treat this as success since the node is shutting down
                # Return a valid JSON-RPC response dictionary indicating success
                response = {'jsonrpc': '2.0', 'result': None, 'id': AuthServiceProxy._AuthServiceProxy__id_count}
