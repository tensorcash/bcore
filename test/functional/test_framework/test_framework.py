...
class BitcoinTestFramework(metaclass=BitcoinTestMetaClass):
    ...
    def parse_args(self, test_file):
        ...
        parser.add_argument("--skip-cache", dest="skip_cache", default=False, action="store_true",
                            help="Disable use of the pre-mined cache and start from a clean chain")
    ...
    def shutdown(self):
        ...
        # For asset tests, always clean up to prevent state contamination
        force_cleanup = getattr(self, 'force_cleanup_on_failure', False)
        should_clean_up = (
            (not self.options.nocleanup and
             self.success != TestStatus.FAILED and
             not self.options.perf) or
            (force_cleanup and self.success == TestStatus.FAILED)
        )
        if should_clean_up:
    ...
    def setup_nodes(self):
        ...
        if not self.setup_clean_chain and not self.options.skip_cache:
            for n in self.nodes:
    ...
    def add_nodes(self, num_nodes: int, extra_args=None, *, rpchost=None, versions=None):
        ...
        for i in range(num_nodes):
            ...
            # Tests run all nodes on loopback, so only a single ASN shows up.
            # Disable ASN corroboration by default to keep direct-fetch behavior active.
            if not any(arg.startswith("-spv-asn-corroboration") for arg in args):
                args.append("-spv-asn-corroboration=0")
    ...
    def restart_node(self, i, extra_args=None, clear_addrman=False, *, expected_stderr=''):
        ...
        # Refresh cumulative_tick cache after restart to prevent validation issues
        try:
            from test_framework.vdf_helper import refresh_cumulative_tick_cache_from_node
            refresh_cumulative_tick_cache_from_node(self.nodes[i], depth=20)
        except Exception:
            pass  # Best effort - don't fail if not needed
    ...
    def join_network(self):
        ...
        self.wait_until(lambda: len(self.nodes[1].getpeerinfo()) > 0 and len(self.nodes[2].getpeerinfo()) > 0, timeout=10)
        self.sync_all()
    ...
    def _initialize_chain(self):
        ...
        expected_cache_blocks = 199
        cache_block_file = cache_node_dir / self.chain / "blocks" / "blk00000.dat"
        cache_height_marker = cache_node_dir / "cache_height"
        cache_valid = cache_block_file.is_file() and cache_height_marker.is_file()
        if cache_valid:
            try:
                cache_valid = int(cache_height_marker.read_text().strip()) == expected_cache_blocks
            except ValueError:
                cache_valid = False
        ...
        if not cache_valid:
            ...
            print("Creating cache directory {}".format(cache_node_dir))
            ...
            generated_blocks = 0
            last_requested = 0
            initial_height = cache_node.getblockcount()
            try:
                for i in range(8):
                    nblocks = 25 if i != 7 else 24
                    last_requested = nblocks
                ...
                try:
                    chain_info = cache_node.getblockchaininfo()
                    current_height = chain_info.get("blocks")
                    written_blocks = max(0, current_height - initial_height) if current_height is not None else None
                except Exception as info_err:
                    self.log.error(
                        "Cache generation aborted after %d successful blocks (last batch requested %d). Failed to query node state: %s",
                        generated_blocks, last_requested, info_err,
                    )
                else:
                    blk_size = cache_block_file.stat().st_size if cache_block_file.exists() else 0
                    self.log.error(
                        "Cache generation aborted after %d successful blocks (last batch requested %d). Node height=%s, newly written blocks=%s, blk00000.dat size=%d bytes",
                        generated_blocks,
                        last_requested,
                        current_height,
                        written_blocks,
            ...
            assert_equal(cache_node.getblockchaininfo()["blocks"], expected_cache_blocks)
            with open(cache_height_marker, 'w', encoding='utf8') as marker:
                marker.write(str(expected_cache_blocks))
            ...
            # Preserve core on-disk state; remove everything else from cache dir.
            # TensorCash stores additional state (e.g. 'modeldb') under the chain dir.
            preserve_dirs = ['chainstate', 'blocks', 'indexes', 'modeldb']
            for entry in os.listdir(cache_path()):
                full = cache_path(entry)
                if entry in preserve_dirs:
                    continue
