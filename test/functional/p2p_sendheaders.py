...
"""
from test_framework.blocktools import create_block, create_coinbase, set_block_tick_from_prev
...
class SendHeadersTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        # Ensure sampled blocks do not trigger quick verification failures when the
        # ValidationAPIMock backend is enabled (functional tests run without chiavdf).
        for node in self.nodes:
            try:
                node.validationmockdefault("quick", "quick_ok_smell_ok")
            except Exception:
                # Backend may be disabled (e.g. when running against a non-mock build);
                # ignore RPC failures here.
                pass
    ...
    def test_null_locators(self, test_node, inv_node):
        ...
        set_block_tick_from_prev(self.nodes[0], block, tip["hash"]) 
        block.solve()
    ...
    def test_nonnull_locators(self, test_node, inv_node):
        ...
        for i in range(4):
            ...
            if i == 0:
                ...
                set_block_tick_from_prev(self.nodes[0], new_block, f"{tip:064x}")
                new_block.solve()
        ...
        for i in range(10):
            ...
            for j in range(2):
                ...
                blocks = []
                for k in range(i + 1):
                    blk = create_block(tip, create_coinbase(height), block_time)
                    if k == 0:
                        # First block connects to the actual chain
                        set_block_tick_from_prev(self.nodes[0], blk, f"{tip:064x}")
                    elif len(blocks) > 0:
                        # Subsequent blocks use the previous block in memory
                        from test_framework.blocktools import set_block_tick
                        parent_tick = getattr(getattr(blocks[-1], 'pow', None), 'tick', 0) or 1000
                        set_block_tick(blk, tick=parent_tick, prev_block=blocks[-1])
                    blocks.append(blk)
                    blocks[-1].solve()
                    self.log.debug(
                        "Synthetic block: height=%d hash=%064x nBits=%08x nAdjBits=%08x",
                        height,
                        blocks[-1].sha256,
                        blocks[-1].nBits,
                        getattr(blocks[-1], 'nAdjBits', 0),
                    )
                    tip = blocks[-1].sha256
                    block_time += 1
                    height += 1
