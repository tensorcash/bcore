...
CTxDestination DecodeDestination(const std::string& str, const CChainParams& params, std::string& error_str, std::vector<int>* error_locations)
{
    ...
    const std::string& bech32_hrp = params.Bech32HRP();
    const size_t hrp_len = bech32_hrp.size();
    const bool has_hrp_prefix = str.size() > hrp_len && ToLower(str.substr(0, hrp_len)) == bech32_hrp;
    const bool has_separator = has_hrp_prefix && str.size() > hrp_len && str[hrp_len] == bech32::SEPARATOR;
    ...
    if (dec.encoding == bech32::Encoding::BECH32 || dec.encoding == bech32::Encoding::BECH32M) {
        ...
        // Bech32 decoding
        if (dec.hrp != bech32_hrp) {
            error_str = strprintf("Invalid or unsupported prefix for Segwit (Bech32) address (expected %s, got %s).", bech32_hrp, dec.hrp);
            return CNoDestination();
        }
        ...
        if (ConvertBits<5, 8, false>([&](unsigned char c) { data.push_back(c); }, dec.data.begin() + 1, dec.data.end())) {
            ...
            if (version == 2 && data.size() == WITNESS_V1_TAPROOT_SIZE) {
                static_assert(WITNESS_V1_TAPROOT_SIZE == WitnessV2Taproot::size());
