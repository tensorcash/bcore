...
// ML-DSA (Post-Quantum) fields for witness v2
static constexpr uint8_t PSBT_IN_MLDSA_PUBKEY = 0x1d;
static constexpr uint8_t PSBT_IN_MLDSA_SIGNATURE = 0x1e;
static constexpr uint8_t PSBT_IN_MLDSA_PARAM_SET = 0x1f;
static constexpr uint8_t PSBT_IN_V2_TAP_PARITY = 0x20;
...
struct PSBTInput
{
    ...
    // ML-DSA (Post-Quantum) fields for witness v2
    std::vector<unsigned char> m_mldsa_pubkey;      // Encoded ML-DSA public key (alg_id || param_set || varint || pk)
    std::vector<unsigned char> m_mldsa_signature;   // ML-DSA signature (without sighash byte)
    std::optional<uint8_t> m_mldsa_param_set;       // Parameter set: 44, 65, or 87
    std::optional<uint8_t> m_v2_tap_parity;         // Output key parity for witness v2 (0 or 1)
    ...
    inline void Serialize(Stream& s) const {
        ...
        if (final_script_sig.empty() && final_script_witness.IsNull()) {
            ...
            if (!m_mldsa_pubkey.empty()) {
                SerializeToVector(s, CompactSizeWriter(PSBT_IN_MLDSA_PUBKEY));
                s << m_mldsa_pubkey;
            }

            // Write ML-DSA signature
            if (!m_mldsa_signature.empty()) {
                SerializeToVector(s, CompactSizeWriter(PSBT_IN_MLDSA_SIGNATURE));
                s << m_mldsa_signature;
            }

            // Write ML-DSA parameter set
            if (m_mldsa_param_set.has_value()) {
                SerializeToVector(s, CompactSizeWriter(PSBT_IN_MLDSA_PARAM_SET));
                SerializeToVector(s, *m_mldsa_param_set);
            }

            // Write witness v2 taproot parity
            if (m_v2_tap_parity.has_value()) {
                SerializeToVector(s, CompactSizeWriter(PSBT_IN_V2_TAP_PARITY));
                SerializeToVector(s, *m_v2_tap_parity);
            }
        }
    ...
    inline void Unserialize(Stream& s) {
        ...
        while(!s.empty()) {
            ...
            switch(type) {
                ...
                case PSBT_IN_MLDSA_PUBKEY:
                {
                    if (!key_lookup.emplace(key).second) {
                        throw std::ios_base::failure("Duplicate Key, input ML-DSA public key already provided");
                    } else if (key.size() != 1) {
                        throw std::ios_base::failure("Input ML-DSA public key key is more than one byte type");
                    }
                    s >> m_mldsa_pubkey;
                    if (m_mldsa_pubkey.size() < 4) {
                        throw std::ios_base::failure("Input ML-DSA public key is too short");
                    }
                    break;
                }
                case PSBT_IN_MLDSA_SIGNATURE:
                {
                    if (!key_lookup.emplace(key).second) {
                        throw std::ios_base::failure("Duplicate Key, input ML-DSA signature already provided");
                    } else if (key.size() != 1) {
                        throw std::ios_base::failure("Input ML-DSA signature key is more than one byte type");
                    }
                    s >> m_mldsa_signature;
                    if (m_mldsa_signature.size() < 2420) {
                        throw std::ios_base::failure("Input ML-DSA signature is too short (minimum 2420 bytes for ML-DSA-44)");
                    } else if (m_mldsa_signature.size() > 4627) {
                        throw std::ios_base::failure("Input ML-DSA signature is too long (maximum 4627 bytes for ML-DSA-87)");
                    }
                    break;
                }
                case PSBT_IN_MLDSA_PARAM_SET:
                {
                    if (!key_lookup.emplace(key).second) {
                        throw std::ios_base::failure("Duplicate Key, input ML-DSA parameter set already provided");
                    } else if (key.size() != 1) {
                        throw std::ios_base::failure("Input ML-DSA parameter set key is more than one byte type");
                    }
                    uint8_t param_set;
                    UnserializeFromVector(s, param_set);
                    if (param_set != 44 && param_set != 65 && param_set != 87) {
                        throw std::ios_base::failure("Invalid ML-DSA parameter set (must be 44, 65, or 87)");
                    }
                    m_mldsa_param_set = param_set;
                    break;
                }
                case PSBT_IN_V2_TAP_PARITY:
                {
                    if (!key_lookup.emplace(key).second) {
                        throw std::ios_base::failure("Duplicate Key, input witness v2 taproot parity already provided");
                    } else if (key.size() != 1) {
                        throw std::ios_base::failure("Input witness v2 taproot parity key is more than one byte type");
                    }
                    uint8_t parity;
                    UnserializeFromVector(s, parity);
                    if (parity > 1) {
                        throw std::ios_base::failure("Invalid witness v2 taproot parity (must be 0 or 1)");
                    }
                    m_v2_tap_parity = parity;
                    break;
                }
...
struct PartiallySignedTransaction
{
    ...
    inline void Serialize(Stream& s) const {
        ...
        // Write serialized tx to a stream. Use the extended serializer so
        // per-output extensions (vExt) are preserved inside the PSBT.
        ...
        while(!s.empty()) {
            ...
            switch(type) {
                case PSBT_GLOBAL_UNSIGNED_TX:
                {
                    ...
                    // Read the raw transaction payload first so we can retry
                    // decoding without witness support if necessary.
                    const size_t expected_size = ReadCompactSize(s);
                    const size_t remaining_before = s.size();
                    SerializeData tx_payload(expected_size);
                    if (expected_size != 0) s.read(MakeWritableByteSpan(tx_payload));
                    const size_t remaining_after = s.size();
                    if (remaining_after + expected_size != remaining_before) {
                        throw std::ios_base::failure("Size of value was not the stated size");
                    }
                    ...
                    bool decoded = false;
                    std::string witness_error_msg;
                    {
                        DataStream witness_stream(tx_payload);
                        try {
                            witness_stream >> TX_WITH_WITNESS(mtx);
                            if (!witness_stream.empty()) {
                                throw std::ios_base::failure("Extra data after unsigned transaction");
                            }
                            decoded = true;
                        } catch (const std::ios_base::failure& e) {
                            witness_error_msg = e.what();
                        }
                    }

                    if (!decoded) {
                        const bool should_retry_legacy = witness_error_msg.find("Unknown transaction optional data") != std::string::npos;
                        if (should_retry_legacy) {
                            DataStream legacy_stream(tx_payload);
                            try {
                                legacy_stream >> TX_NO_WITNESS(mtx);
                                if (!legacy_stream.empty()) {
                                    throw std::ios_base::failure("Extra data after unsigned transaction");
                                }
                                decoded = true;
                            } catch (const std::ios_base::failure&) {
                                throw std::ios_base::failure(witness_error_msg);
                            }
                        } else {
                            if (!witness_error_msg.empty()) {
                                throw std::ios_base::failure(witness_error_msg);
                            }
                            throw std::ios_base::failure("Failed to decode PSBT unsigned transaction");
                        }
                    }
