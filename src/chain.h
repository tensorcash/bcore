...
enum BlockStatus : uint32_t {
    ...
    BLOCK_FULL_RED_LOCAL     =   512, //!< Local external Full validation failed; block contributes zero local chainwork.
    BLOCK_MODEL_CHALLENGE_ZERO_WORK = 1024, //!< Successful model challenge made this block contribute zero chainwork.
};
...
public:
    ...
    //! (memory only) Total accepted positive work in the chain up to and including this block.
    //! Locally full-red and model-challenge zero-work blocks contribute no positive work.
    //! This and nChainPenalty are candidate-set ordering keys; callers must clear/rebuild
    //! setBlockIndexCandidates before mutating either field on indexed blocks.
    arith_uint256 nChainWork{};

    //! (memory only) Cumulative local fork-choice penalty from full-red blocks.
    ...
    explicit CBlockIndex(const CBlockHeader& block)
        : nVersion{block.nVersion},
          ...
          nBits{block.nBits},
          nNonce{block.nNonce},
          nAdjBits(block.nAdjBits),
          hashPoW(block.hashPoW),
          flags(block.flags)
    {
    }
    ...
    CBlockHeader GetBlockHeader() const
    {
        ...
        block.nBits = nBits;
        block.nNonce = nNonce;
        block.nAdjBits = nAdjBits;
        block.hashPoW = hashPoW;
        block.flags = flags;
        return block;
    }
    ...
    uint256 GetBlockShortHash() const
    {
        return GetBlockHeader().GetShortHash();
    }
...
public:
    ...
    SERIALIZE_METHODS(CDiskBlockIndex, obj)
    {
        ...
        READWRITE(obj.nTime);
        READWRITE(obj.nAdjBits);
        READWRITE(obj.nNonce);
        READWRITE(obj.nBits);
        READWRITE(obj.hashPoW);
        READWRITE(obj.flags);
    }
    ...
    uint256 ConstructBlockShortHash() const
    {
        CBlockHeader block;
        block.nVersion = nVersion;
        block.hashPrevBlock = hashPrev;
    ...
    uint256 GetBlockShortHash() = delete;
