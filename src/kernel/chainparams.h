...
public:
    ...
    /** Return the HTTP seed API URLs (queried in order as fallback) */
    const std::vector<std::string>& HttpSeedApiUrls() const { return m_http_seed_api_urls; }
    ...
    static std::unique_ptr<const CChainParams> TensorMain();
    static std::unique_ptr<const CChainParams> TensorTest();
    static std::unique_ptr<const CChainParams> TensorReg(const RegTestOptions& options);
...
protected:
    ...
    std::vector<std::string> m_http_seed_api_urls;
