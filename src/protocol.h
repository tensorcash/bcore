...
/**
 * TensorCash: extended headers sidecar carrying VDF tick/proof for presync (optional).
 * Note: Wire message names are limited to 12 chars. Use "getheadext" (10 chars).
 */
inline constexpr const char* GETHEADERS_EXT{"getheadext"};
inline constexpr const char* HEADERS_EXT{"headers_ext"};
/**
...
/**
 * Serializer for BIP130 HEADERS payloads (vector of extended block headers
 * followed by a CompactSize-encoded transaction count of zero after each).
 */
struct HeadersMessage
{
    ...
    explicit HeadersMessage(std::vector<CBlockHeader> headers_in) : headers(std::move(headers_in)) {}
    ...
    void Unserialize(Stream& s)
    {
        ...
        for (uint64_t i = 0; i < count; ++i) {
            ...
            s >> headers.back();
            (void)::ReadCompactSize(s); // ignore per-entry txcount (must remain zero per spec)
        }
    }
};
...
inline const std::array ALL_NET_MESSAGE_TYPES{std::to_array<std::string>({
    ...
    NetMsgType::TX,
    NetMsgType::HEADERS,
    NetMsgType::GETHEADERS_EXT,
    NetMsgType::HEADERS_EXT,
    NetMsgType::BLOCK,
    NetMsgType::GETADDR,
    NetMsgType::MEMPOOL,
    NetMsgType::PING,
...
enum ServiceFlags : uint64_t {
    ...
    // TensorCash experimental service flag for VDF SPV sidecars
    NODE_VDFSPV = (1 << 24),
