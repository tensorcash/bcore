...
bool BlockFilterIndex::CustomInit(const std::optional<interfaces::BlockRef>& block)
{
    if (!m_db->Read(DB_FILTER_POS, m_next_filter_pos)) {
        ...
        // A missing DB_FILTER_POS is only valid when the database has never been populated.
        // If other entries are present, treat this as a corruption signal instead of silently
        // resetting the write cursor.
        if (block) {
            LogError("%s: Missing %s write cursor while a best block is recorded; index may be corrupted\n",
                     __func__, GetName());
            return false;
        }
        if (!m_db->IsEmpty()) {
            LogError("%s: Missing %s write cursor but database is not empty; index may be corrupted\n",
                     __func__, GetName());
            return false;
        }
