...
std::list<SectionInfo> ArgsManager::GetUnrecognizedSections() const
{
    ...
    static const std::set<std::string> available_sections{
        ...
        // TensorCash chains
        ChainTypeToString(ChainType::TENSOR_MAIN),
        ChainTypeToString(ChainType::TENSOR_TEST),
        ChainTypeToString(ChainType::TENSOR_REG),
    };
    ...
    for (const auto& arg_map : m_available_args) {
        switch(arg_map.first) {
            ...
            case OptionsCategory::CLI_COMMANDS:
                ...
                break;
            case OptionsCategory::EXTERNAL_API:
                usage += HelpMessageGroup("Eaternal API connection options:");
                break;
            default:
                break;
        }
...
{
    // Determine the data directory name based on the compile-time default chain.
    // Each separately-shipped app (DEFAULT_CHAIN_TYPE=tensor vs tensor-test vs
    // tensor-reg) gets a DISTINCT data directory so the mainnet and testnet
    // wallets can run simultaneously on one machine without sharing a
    // bitcoin.conf / settings.json / chainstate lock. Mirrors the per-chain
    // app-bundle names in packaging/macos/build-macos.sh
    // (TensorCash / TensorCash-Testnet). Upstream chains use Bitcoin/.bitcoin.
    constexpr const char* default_chain = DEFAULT_CHAIN_TYPE;
    const std::string_view dc{default_chain};
    [[maybe_unused]] const char* app_dir_name;
    [[maybe_unused]] const char* unix_dir_name;
    if (dc == "tensor-test") {
        app_dir_name = "TensorCash-Testnet";
        unix_dir_name = ".tensorcash-testnet";
    } else if (dc == "tensor-reg") {
        app_dir_name = "TensorCash-Regtest";
        unix_dir_name = ".tensorcash-regtest";
    } else if (dc.starts_with("tensor")) {
        // tensor (mainnet) and any future tensor* default.
        app_dir_name = "TensorCash";
        unix_dir_name = ".tensorcash";
    } else {
        app_dir_name = "Bitcoin";
        unix_dir_name = ".bitcoin";
    }

#ifdef WIN32
    // Windows — check for legacy location first
    fs::path legacy_path = GetSpecialFolderPath(CSIDL_APPDATA) / app_dir_name;
    if (fs::exists(legacy_path)) return legacy_path;
    return GetSpecialFolderPath(CSIDL_LOCAL_APPDATA) / app_dir_name;
#else
    fs::path pathRet;
...
#ifdef __APPLE__
    // macOS: ~/Library/Application Support/TensorCash (or Bitcoin)
    return pathRet / "Library/Application Support" / app_dir_name;
#else
    // Unix-like: ~/.tensorcash (or ~/.bitcoin)
    return pathRet / unix_dir_name;
#endif
#endif
}
...
std::variant<ChainType, std::string> ArgsManager::GetChainArg() const
{
    ...
    const bool fTensor = get_net("-tensor");
    const bool fTensorTest = get_net("-tensortest");
    const bool fTensorReg = get_net("-tensorreg");
    const auto chain_arg = GetArg("-chain");

    if ((int)chain_arg.has_value() + (int)fRegTest + (int)fSigNet + (int)fTestNet + (int)fTestNet4 + 
        (int)fTensor + (int)fTensorTest + (int)fTensorReg > 1) {
        throw std::runtime_error("Invalid combination of chain selection arguments. Can use at most one.");
    }
    if (chain_arg) {
    ...
    if (fTensor) return ChainType::TENSOR_MAIN;
    if (fTensorTest) return ChainType::TENSOR_TEST;
    if (fTensorReg) return ChainType::TENSOR_REG;

    // Use compile-time default chain type (set via -DDEFAULT_CHAIN_TYPE=...)
    // This allows testnet and mainnet builds to be shipped as separate apps
    // that "just work" on double-click without any CLI flags.
    constexpr const char* default_chain = DEFAULT_CHAIN_TYPE;
    if (default_chain[0] != '\0') {
        if (auto parsed = ChainTypeFromString(default_chain)) return *parsed;
    }
