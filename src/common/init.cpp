...
fs::path GetBundledDefaultConfigPath()
{
    ...
    const std::vector<fs::path> candidates{
        exe_dir / "defaults" / BITCOIN_CONF_FILENAME,
        exe_dir.parent_path() / "defaults" / BITCOIN_CONF_FILENAME,
        exe_dir.parent_path() / "Resources" / "defaults" / BITCOIN_CONF_FILENAME,
    };
...
std::optional<ConfigError> InitConfig(ArgsManager& args, SettingsAbortFn settings_abort_fn)
{
    try {
        ...
        const bool use_bundled_default_config =
            !args.IsArgSet("-conf") &&
            !fs::exists(orig_config_path) &&
            !GetBundledDefaultConfigPath().empty();

        fs::path bundled_config_path;
        if (use_bundled_default_config) {
            bundled_config_path = GetBundledDefaultConfigPath();
            args.SetConfigFilePath(bundled_config_path);
            LogPrintf("Using bundled default configuration from %s\n", fs::PathToString(bundled_config_path));
        }
        ...
        if (use_bundled_default_config && !fs::exists(base_config_path)) {
            try {
                fs::copy_file(bundled_config_path, base_config_path, fs::copy_options::none);
                LogPrintf("Seeded default configuration at %s\n", fs::PathToString(base_config_path));
            } catch (const std::exception& e) {
                LogWarning("Could not seed default configuration at %s: %s\n",
                           fs::PathToString(base_config_path), e.what());
            }
        }
