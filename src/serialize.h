...
template <typename Stream> void Serialize(Stream& s, float a)     { uint32_t f; std::memcpy(&f, &a, sizeof(float)); ser_writedata32(s, f); }
template <typename Stream> void Unserialize(Stream& s, float& a)  { uint32_t f = ser_readdata32(s); std::memcpy(&a, &f, sizeof(float)); }
template <typename Stream> void Serialize(Stream& s, double a)    { uint64_t d; std::memcpy(&d, &a, sizeof(double)); ser_writedata64(s, d); }
template <typename Stream> void Unserialize(Stream& s, double& a) { uint64_t d = ser_readdata64(s); std::memcpy(&a, &d, sizeof(double)); }
// clang-format on
...
/**
 * deque (used for ring buffers with efficient push_front/pop_back)
 */
template <typename Stream, typename T, typename A>
