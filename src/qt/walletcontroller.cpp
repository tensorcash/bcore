...
void WalletController::closeAllWallets(QWidget* parent)
{
    ...
    // Snapshot under the lock, then call removeWallet() OUTSIDE the lock.
    // removeWallet() reaches into the node and (asynchronously, via the unload
    // signal) ends up calling removeAndDeleteWallet(), which itself takes
    // m_mutex. Holding m_mutex across that call risks deadlocking against any
    // RPC/wallet thread that's already in getOrCreateWallet().
    std::vector<WalletModel*> snapshot;
    {
        QMutexLocker locker(&m_mutex);
        snapshot = m_wallets;
    }
    for (WalletModel* wallet_model : snapshot) {
        removeWallet(wallet_model);
    }
}
...
WalletModel* WalletController::getOrCreateWallet(std::unique_ptr<interfaces::Wallet> wallet)
{
    ...
    // Phase 1: existing-wallet lookup. Lock held; no blocking calls inside.
    {
        QMutexLocker locker(&m_mutex);
        for (WalletModel* wm : m_wallets) {
            if (wm->wallet().getWalletName() == name) {
                return wm;
            }
        }
    }

    // Phase 2: construct + thread-move + setParent. NO LOCK HELD here.
    //
    // setParent(parent) must run on the parent's thread (the GUI thread). When
    // we are called from an RPC / wallet-loader thread, that requires a
    // BlockingQueuedConnection back to the GUI thread. If we held m_mutex
    // across this invoke and the GUI thread were itself blocked on a method
    // that acquires m_mutex (listWalletDir, removeAndDeleteWallet, …), this is
    // a textbook A-B-A deadlock. Releasing the lock first eliminates the
    // hazard. See Apple TN2071 / Qt docs on blocking inter-thread invokes.
    ...
    // Phase 3: register, with race re-check. Lock held; no blocking calls.
    WalletModel* race_winner = nullptr;
    {
        QMutexLocker locker(&m_mutex);
        for (WalletModel* wm : m_wallets) {
            if (wm->wallet().getWalletName() == name) {
    ...
    if (race_winner) {
        // Another thread registered the same wallet between Phase 1 and Phase
        // 3. Discard ours; do NOT emit walletAdded (the winner already did or
        // will). wallet_model is parented to `this` on the GUI thread, so
        // schedule deleteLater on that thread.
        QMetaObject::invokeMethod(wallet_model, [wallet_model] {
            wallet_model->deleteLater();
        }, Qt::QueuedConnection);
        return race_winner;
    }

    // Phase 4: post-registration setup. NO LOCK HELD. Slot connects, balance
    // polling, and walletAdded emission all happen here so that any handler
    // they trigger (which may call back into WalletController methods that
    // take m_mutex) doesn't deadlock.
