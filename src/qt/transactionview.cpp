    ...
    : QWidget(parent), m_platform_style{platformStyle}
{
    ...
    // Asset filter will be populated when model is set
    assetWidget->addItem(tr("All Assets"), QString(""));
    ...
    if (!transactionView->horizontalHeader()->restoreState(settings.value("TransactionViewHeaderState").toByteArray())) {
        ...
        transactionView->setColumnWidth(TransactionTableModel::AssetAmount, ASSET_AMOUNT_COLUMN_WIDTH);
...
void TransactionView::setModel(WalletModel *_model)
{
    ...
    if(_model)
    {
        ...
        // Populate asset filter dropdown with unique assets from transaction history
        if (assetWidget && _model->getTransactionTableModel()) {
            // Clear and repopulate asset widget
            assetWidget->clear();
            assetWidget->addItem(tr("All Assets"), QString(""));
            assetWidget->addItem(tr("TSC"), QString("TSC"));

            // Add unique assets from transaction history
            QSet<QString> uniqueAssets;
            TransactionTableModel* ttm = _model->getTransactionTableModel();
            int rows = ttm->rowCount(QModelIndex());

            for (int i = 0; i < rows; ++i) {
                QModelIndex idx = ttm->index(i, 0);
                QString ticker = idx.data(TransactionTableModel::AssetTickerRole).toString();
                if (!ticker.isEmpty() && ticker != "TSC") {
                    uniqueAssets.insert(ticker);
                }
            }

            // Sort and add to dropdown
            QStringList sortedAssets = uniqueAssets.values();
            sortedAssets.sort();
            for (const QString& asset : sortedAssets) {
                assetWidget->addItem(asset, asset);
            }
        }
    }
}
...
void TransactionView::chooseAsset(int idx)
{
    if(!transactionProxyModel)
        return;
    transactionProxyModel->setAssetFilter(
        assetWidget->itemData(idx).toString());
}
...
void TransactionView::contextualMenu(const QPoint &point)
{
    ...
    // If user right-clicked on a row, select it first
    if (index.isValid()) {
        transactionView->selectionModel()->setCurrentIndex(index, QItemSelectionModel::SelectCurrent | QItemSelectionModel::Rows);
    }
