...
static std::string DummyAddress(const CChainParams &params)
{
    ...
    case ChainType::TENSOR_MAIN:
        addr = "tc1p35yvjel7srp783ztf8v6jdra7dhfzk5jaun8xz2qp6ws7z80n4tqlvkng2";
        break;
    case ChainType::TENSOR_TEST:
        addr = "tct1p35yvjel7srp783ztf8v6jdra7dhfzk5jaun8xz2qp6ws7z80n4tqxmqtgw";
        break;
    case ChainType::TENSOR_REG:
        addr = "tcrt1p35yvjel7srp783ztf8v6jdra7dhfzk5jaun8xz2qp6ws7z80n4tq8hvsav";
        break;
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a TensorCash address (e.g. %1)").arg(
...
{
    // return if URI is not valid or is no bitcoin: or tensorcash: URI
    if(!uri.isValid() || (uri.scheme() != QString("bitcoin") && uri.scheme() != QString("tensorcash")))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        {
            // Parse asset ID parameter
            QString assetIdStr = i->second;
        ...
        else if (i->first == "asset_decimals")
        {
            bool ok = false;
            const int dec = i->second.toInt(&ok);
            if (!ok || dec < 0 || dec > 18) {
                return false;
            }
            rv.asset_decimals = static_cast<uint8_t>(dec);
            fShouldReturnFalse = false;
        }
        else if (i->first == "asset_ticker")
        {
            rv.asset_ticker = i->second;
            fShouldReturnFalse = false;
        }
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if (rv.asset_id.has_value()) {
                    // For assets, defer conversion until we know the decimal precision.
                    rv.asset_amount_string = i->second;
                } else {
                    // For BTC, use standard parsing
    ...
    if (rv.asset_id.has_value()) {
        // Convert the stored amount string (if any) now that we know decimals.
        if (!rv.asset_amount_string.isEmpty()) {
            uint64_t units{0};
            if (!convertAssetAmountStringToUnits(rv.asset_amount_string, rv.asset_decimals, units)) {
                return false;
            }
            rv.asset_units = units;
            rv.asset_amount_string.clear();
        }

        // Asset transfers never use the BTC amount field.
        rv.amount = 0;
    }
    if(out)
    {
...
QString formatBitcoinURI(const SendCoinsRecipient &info)
{
    ...
    // Use tensorcash scheme for all payments
    QString scheme = "tensorcash";
    QString ret = QString("%1:%2").arg(scheme).arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;

    // Add asset ID and decimals FIRST for asset payments (before amount)
    // This ensures the parser knows it's an asset when it encounters the amount
    if (info.asset_id.has_value()) {
        QString assetIdStr = QString::fromStdString(info.asset_id->ToString());
        ret += QString("?asset=%1").arg(assetIdStr);
        paramCount++;

        ret += QString("&asset_decimals=%1").arg(info.asset_decimals);
        paramCount++;

        // Add asset ticker if available
        if (!info.asset_ticker.isEmpty()) {
            QString ticker(QUrl::toPercentEncoding(info.asset_ticker));
            ret += QString("&asset_ticker=%1").arg(ticker);
            paramCount++;
        }

        // Now add the amount (after asset parameters are set)
        QString amountStr = info.asset_amount_string.isEmpty()
                                 ? formatAssetAmount(info.asset_units, info.asset_decimals)
                                 : info.asset_amount_string;
        if (info.asset_units > 0 && !amountStr.isEmpty()) {
            ret += QString("&amount=%1").arg(amountStr);
            paramCount++;
        }
...
bool convertAssetAmountStringToUnits(const QString& amountStr, uint8_t decimals, uint64_t& units)
{
    ...
    static const QRegularExpression amountRE(QStringLiteral("^\\d*(?:\\.\\d*)?$"));
    if (!amountRE.match(trimmed).hasMatch()) {
        return false;
    }

    QStringList parts = trimmed.split('.');
    QString integerPart = parts.value(0);
    QString fractionalPart = parts.size() > 1 ? parts.value(1) : QString();

    if (fractionalPart.size() > decimals) {
        return false;
    }

    while (integerPart.startsWith('0') && integerPart.size() > 1) {
        integerPart.remove(0, 1);
    }

    QString fractionalPadded = fractionalPart;
    while (fractionalPadded.size() < decimals) {
        fractionalPadded.append('0');
    }

    QString combined = integerPart.isEmpty() ? QStringLiteral("0") : integerPart;
    combined += fractionalPadded;

    int firstNonZero = 0;
    while (firstNonZero < combined.size() - 1 && combined[firstNonZero] == '0') {
        ++firstNonZero;
    }
    combined = combined.mid(firstNonZero);
...
QString formatAssetAmount(uint64_t units, uint8_t decimals)
{
    if (decimals == 0) {
    ...
    QString remainderStr = QString::number(remainder).rightJustified(decimals, '0');
    while (remainderStr.endsWith('0')) {
        remainderStr.chop(1);
    }

    return result + QStringLiteral(".") + remainderStr;
}
...
bool openBitcoinConf()
{
    ...
    /* Open tensorcash.conf with the associated application */
...
void PopupMenu(QMenu* menu, const QPoint& point, QAction* at_action)
{
    ...
    // On macOS 26+ with Qt 6.10, QMenu::popup() can crash in
    // QWindow::geometry() with a null d-pointer dereference if the backing
    // QWindow has not been created yet.  Both the parent widget AND the
    // menu itself must have a native window handle before popup(): the
    // cocoa plugin reads the menu's own QWindow inside setVisible(true),
    // not just the parent's.  Force native window creation on both.
    if (QWidget* parent = menu->parentWidget()) {
        if (!parent->windowHandle()) {
            parent->winId();   // forces native window creation on parent
        }
    }
    if (!menu->windowHandle()) {
        menu->winId();         // forces native window creation on the menu
    }
...
QString RpcExceptionMessage(const UniValue& e)
{
    if (e.isObject() && e.exists("message") && e["message"].isStr()) {
        return QString::fromStdString(e["message"].get_str());
    }
    if (e.isStr()) {
        return QString::fromStdString(e.get_str());
    }
    return QStringLiteral("RPC error");
}

void PrintSlotRpcException(
    const UniValue& exception,
    const QObject* sender,
    const QObject* receiver)
{
    std::string description = sender->metaObject()->className();
    ...
    if (exception.isObject()) {
        if (exception.exists("code") && exception["code"].isNum()) {
            code = exception["code"].getInt<int>();
        }
        if (exception.exists("message") && exception["message"].isStr()) {
            message = exception["message"].get_str();
        }
    } else if (exception.isStr()) {
        message = exception.get_str();
    }

    LogPrintf("\n\n************************\nRPC EXCEPTION code=%d message=\"%s\"\nbitcoin in %s\n\n",
...
bool MouseWheelEventFilter::eventFilter(QObject* obj, QEvent* event)
{
    if (event->type() == QEvent::Wheel) {
        ...
        // Only process wheel events if the widget has focus
        // This prevents accidental changes while scrolling the page
        if (widget && !widget->hasFocus()) {
            event->ignore();
            return true;  // Event is filtered (not processed)
        }
    }
...
void InstallWheelEventFilter(QWidget* widget)
{
    if (!widget) return;

    static MouseWheelEventFilter* filter = new MouseWheelEventFilter(qApp);
