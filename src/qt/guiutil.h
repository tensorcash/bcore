...
namespace GUIUtil
{
    ...
    // Convert asset amount string to units based on decimals
    /** Convert a decimal asset amount string into raw units based on the
     *  provided number of decimals. Returns false if the string is invalid
     *  or would overflow a uint64_t. */
    bool convertAssetAmountStringToUnits(const QString& amountStr, uint8_t decimals, uint64_t& units);

    /** Format raw asset units into a human readable decimal string using the
     *  supplied number of decimals. */
    ...
    */
    ...
    /** Extract a human-readable message from a UniValue thrown by a JSONRPC handler. */
    QString RpcExceptionMessage(const UniValue& e);

    /** Mirrors PrintSlotException, but logs the RPC code+message rather than "UNKNOWN EXCEPTION". */
        ...
        return QObject::connect(
            ...
            [sender, receiver, method](auto&&... args) {
                ...
                try {
                    ...
                    // RPC handlers throw UniValue (not std::exception). A panel
                    // RPC failure should not kill the wallet — route to non-fatal.
                    PrintSlotRpcException(e, sender, receiver);
                    ok = QMetaObject::invokeMethod(
                        qApp, "handleNonFatalException",
                        blockingGUIThreadConnection(),
                        Q_ARG(QString, RpcExceptionMessage(e)));
    ...
    /**
     * Event filter that prevents accidental modification of input widgets
     * (QComboBox, QSpinBox, etc.) via mouse wheel when scrolling.
     * Only allows wheel events when the widget has explicit focus.
     */
    ...
    /**
     * Install wheel event filter on a widget to prevent accidental changes while scrolling.
     * Useful for QComboBox, QSpinBox, QDoubleSpinBox, etc.
     */
