...
void SendCoinsDialog::useAvailableBalance(SendCoinsEntry* entry)
{
    ...
    auto selected_asset = entry->getSelectedAsset();

    if (selected_asset.has_value()) {
        // For asset transactions, get the asset balance
        uint64_t available_units = 0;
        uint8_t decimals = 8; // default

        // Get asset balances from wallet
        std::vector<interfaces::AssetBalance> balances = model->getAssetBalances();
        for (const auto& balance : balances) {
            if (balance.asset_id == selected_asset.value()) {
                available_units = balance.balance;
                if (balance.has_decimals) {
        ...
        // Subtract amounts from other entries with the same asset
        for (int i = 0; i < ui->entries->count(); ++i) {
            SendCoinsEntry* e = qobject_cast<SendCoinsEntry*>(ui->entries->itemAt(i)->widget());
            if (e && !e->isHidden() && e != entry) {
                if (e->getSelectedAsset() == selected_asset) {
                    // This entry is for the same asset, subtract its units
                    uint64_t other_units = e->getValue().asset_units;
                    // Check for underflow
                    if (other_units > available_units) {
                        available_units = 0;  // Set to 0 to prevent underflow
                        break;  // No point continuing
                    } else {
                        available_units -= other_units;
                    }
                }
            }
        }

        // Convert units to display amount (satoshis for BitcoinAmountField)
        if (available_units > 0) {
            CAmount display_amount;

            if (decimals <= 8) {
                // Asset has fewer or equal decimals than Bitcoin
                // Multiply to get satoshis
                uint64_t multiplier = 1;
                for (uint8_t i = 0; i < (8 - decimals); ++i) {
                    multiplier *= 10;
                }
                display_amount = static_cast<CAmount>(available_units * multiplier);
            } else {
                // Asset has more than 8 decimals - we can only display up to 8
                // This will truncate precision beyond 8 decimal places
                uint64_t divisor = 1;
                for (uint8_t i = 0; i < (decimals - 8); ++i) {
                    divisor *= 10;
                }
                display_amount = static_cast<CAmount>(available_units / divisor);
            }

            entry->checkSubtractFeeFromAmount();
            entry->setAmount(display_amount);
        } else {
            entry->setAmount(0);
        }
    } else {
        // For BTC transactions, use the original logic
        ...
        for (int i = 0; i < ui->entries->count(); ++i) {
            SendCoinsEntry* e = qobject_cast<SendCoinsEntry*>(ui->entries->itemAt(i)->widget());
            if (e && !e->isHidden() && e != entry) {
                // Only subtract if it's also a BTC transaction
                if (!e->getSelectedAsset().has_value()) {
