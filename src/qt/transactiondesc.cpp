...
QString TransactionDesc::toHTML(interfaces::Node& node, interfaces::Wallet& wallet, TransactionRecord* rec, BitcoinUnit unit)
{
    ...
    // Asset information
    if (rec->is_asset_transfer) {
        strHTML += "<b>" + tr("Asset") + ":</b> " + GUIUtil::HtmlEscape(rec->asset_ticker) + "<br>";

        QString amountText = GUIUtil::formatAssetAmount(rec->asset_units, rec->asset_decimals);
        const qint64 netAmount = rec->credit + rec->debit;
        const QString sign = (netAmount > 0) ? QStringLiteral("+") : QStringLiteral("-");
        strHTML += "<b>" + tr("Asset Amount") + ":</b> " + sign + amountText + " " + GUIUtil::HtmlEscape(rec->asset_ticker) + "<br>";

        if (rec->asset_id.has_value()) {
            strHTML += "<b>" + tr("Asset ID") + ":</b> " + QString::fromStdString(rec->asset_id.value().ToString()) + "<br>";
        }
        if (!rec->asset_is_registered) {
            strHTML += "<b>" + tr("Asset Status") + ":</b> " + tr("Registry entry not found") + "<br>";
        }
    }
    ...
    // Message from normal tensorcash:URI (tensorcash:123...?message=example)
    ...
    //
    // Post-Quantum ML-DSA Signature Information
    //
    bool has_mldsa_sig = false;
    for (const auto& txin : wtx.tx->vin) {
        if (!txin.scriptWitness.IsNull() && txin.scriptWitness.stack.size() >= 3) {
            ...
            if (sig_size >= 2400 && sig_size <= 5000) {
                if (!has_mldsa_sig) {
                    strHTML += "<br><b>" + tr("Post-Quantum Signature") + ":</b><br>";
                    has_mldsa_sig = true;
                }

                QString security_level;
                if (sig_size >= 2400 && sig_size <= 2500) {
                    security_level = tr("ML-DSA-44 (Basic Security)");
                } else if (sig_size >= 3200 && sig_size <= 3400) {
                    security_level = tr("ML-DSA-65 (Standard Security)");
                } else if (sig_size >= 4500 && sig_size <= 4700) {
                    security_level = tr("ML-DSA-87 (High Security)");
                } else {
                    security_level = tr("ML-DSA (Unknown Level)");
                }

                strHTML += security_level + "<br>";
                strHTML += "<b>" + tr("Witness Size") + ":</b> ~" +
                           QString::number(sig_size / 1024.0, 'f', 1) + " KB<br>";
                break; // Only show first ML-DSA input
            }
        }
    }
