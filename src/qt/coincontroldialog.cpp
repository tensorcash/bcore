...
void CoinControlDialog::updateLabels(CCoinControl& m_coin_control, WalletModel *model, QDialog* dialog)
{
    ...
    unsigned int witnessFudgeInputs = 0;
    unsigned int mldsaInputs        = 0;

    auto vCoinControl{m_coin_control.ListSelected()};
    const wallet::CWallet* cwallet = model->wallet().wallet();
    ...
    for (const auto& out : model->wallet().getCoins(vCoinControl)) {
        ...
        bool is_witness = out.txout.scriptPubKey.IsWitnessProgram(witnessversion, witnessprogram);
        bool is_mldsa_output = is_witness && witnessversion == 2;
        if (is_witness) {
            anyWitness = true;
            if (is_mldsa_output) {
                ++mldsaInputs;
                int est_vsize = -1;
                if (cwallet != nullptr) {
                    est_vsize = wallet::CalculateMaximumSignedInputSize(out.txout, cwallet, &m_coin_control);
                }
                if (est_vsize <= 0) {
                    // Fallback to ML-DSA-65 sizing to avoid underestimation.
                    const int64_t pk_size = 1952;
                    const int64_t sig_size = 3293;
                    const int64_t mldsa_signature_size = sig_size + 1;
                    ...
                    est_vsize = GetVirtualTransactionSize(input_weight, 0, 0);
                }
                nBytesInputs += est_vsize;
            } else {
    ...
    if (nQuantity > 0)
    {
        ...
        if (anyWitness)
        {
            ...
            nBytes += witnessFudgeInputs; // only add witness stack varint for inputs still using the legacy estimator.
        }

        if (mldsaInputs > 0) {
            // Provide a small safety cushion so ML-DSA spends never underestimate fees.
            nBytes += mldsaInputs * 4;
        }
...
void CoinControlDialog::updateView()
{
    ...
    for (const auto& coins : model->wallet().listCoins()) {
        ...
        for (const auto& outpair : coins.second) {
            ...
            QString sAddress = "";
            const bool output_is_mldsa = out.txout.scriptPubKey.IsWitnessProgram(witnessversion, witnessprogram) && witnessversion == 2;
            ...
            if (output_is_mldsa) {
                const QString currentLabel = itemOutput->text(COLUMN_LABEL);
                QString badgeLabel;
                if (currentLabel.isEmpty()) {
                    badgeLabel = tr("PQ");
                } else {
                    badgeLabel = tr("%1  PQ").arg(currentLabel);
                }
                itemOutput->setText(COLUMN_LABEL, badgeLabel);

                const QString existingTooltip = itemOutput->toolTip(COLUMN_LABEL);
                QString pqTooltip = tr("Post-Quantum (ML-DSA) UTXO – spending requires much larger witnesses and higher fees.");
                if (!existingTooltip.isEmpty()) {
                    pqTooltip = existingTooltip + QStringLiteral("\n") + pqTooltip;
                }
                itemOutput->setToolTip(COLUMN_LABEL, pqTooltip);

                QColor badgeBg(76, 175, 80);
                badgeBg.setAlpha(220);
                itemOutput->setBackground(COLUMN_LABEL, QBrush(badgeBg));
                itemOutput->setForeground(COLUMN_LABEL, QColor(Qt::white));
            }
        }
