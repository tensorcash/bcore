...
int main(int argc, char* argv[])
{
...

    {
        ...
        AssetBalanceTests test7;
        num_test_failures += QTest::qExec(&test7);

        SendCoinsEntryTests test8;
        num_test_failures += QTest::qExec(&test8);

        WalletModelTests test9;
        num_test_failures += QTest::qExec(&test9);

        TransactionTableModelTests test10;
        num_test_failures += QTest::qExec(&test10);

        TransactionFilterProxyTests test11;
        num_test_failures += QTest::qExec(&test11);

        ReceiveDialogTests test12(app.node());
        num_test_failures += QTest::qExec(&test12);

        IntegrationTests test13(app.node());
        num_test_failures += QTest::qExec(&test13);
#endif
