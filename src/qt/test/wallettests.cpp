...
void TestGUI(interfaces::Node& node, const std::shared_ptr<CWallet>& wallet)
{
    ...
    for (QWidget* widget : QApplication::topLevelWidgets()) {
        if (widget->inherits("ReceiveRequestDialog")) {
            ...
            int scheme_count = uri.count("bitcoin:") + uri.count("tensorcash:");
            QCOMPARE(scheme_count, 2);
            ...
            QString expectedAmount = BitcoinUnits::formatWithUnit(BitcoinUnits::Unit::BTC, /*amount=*/1, false, BitcoinUnits::SeparatorStyle::STANDARD);
            QCOMPARE(receiveRequestDialog->QObject::findChild<QLabel*>("amount_content")->text(), expectedAmount);
...
void TestGUI(interfaces::Node& node)
{
    ...
    if (WalletContext* ctx = wallet_loader->context()) {
        ctx->node_context = &test.m_node;
    }
    test.m_node.wallet_loader = wallet_loader.get();
    ScopedNodeContextSetter context_guard(node, &test.m_node);
