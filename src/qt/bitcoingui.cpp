...
void BitcoinGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a TensorCash address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and tensorcash: URIs)"));
    ...
    treasuryAction = new QAction(platformStyle->SingleColorIcon(":/icons/tx_output"), tr("A&ssets"), this);
    treasuryAction->setStatusTip(tr("Asset management (holder && issuer operations)"));
    treasuryAction->setToolTip(treasuryAction->statusTip());
    treasuryAction->setCheckable(true);
    treasuryAction->setShortcut(QKeySequence(QStringLiteral("Alt+5")));
    tabGroup->addAction(treasuryAction);

    exchangeP2PAction = new QAction(platformStyle->SingleColorIcon(":/icons/send"), tr("&Trading"), this);
    exchangeP2PAction->setStatusTip(tr("Create and manage decentralized financial contracts (Repo/Forward/Spot)"));
    exchangeP2PAction->setToolTip(exchangeP2PAction->statusTip());
    exchangeP2PAction->setCheckable(true);
    exchangeP2PAction->setShortcut(QKeySequence(QStringLiteral("Alt+6")));
    tabGroup->addAction(exchangeP2PAction);

    miningApisAction = new QAction(platformStyle->SingleColorIcon(":/icons/tx_mined"), tr("Mining &APIs"), this);
    miningApisAction->setStatusTip(tr("Monitor mining and verification API status"));
    miningApisAction->setToolTip(miningApisAction->statusTip());
    miningApisAction->setCheckable(true);
    miningApisAction->setShortcut(QKeySequence(QStringLiteral("Alt+7")));
    tabGroup->addAction(miningApisAction);

    modelsAction = new QAction(platformStyle->SingleColorIcon(":/icons/connect_4"), tr("AI &Models"), this);
    modelsAction->setStatusTip(tr("Register and manage AI models for proof-of-work"));
    modelsAction->setToolTip(modelsAction->statusTip());
    modelsAction->setCheckable(true);
    modelsAction->setShortcut(QKeySequence(QStringLiteral("Alt+8")));
    tabGroup->addAction(modelsAction);
    ...
    connect(exchangeP2PAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(exchangeP2PAction, &QAction::triggered, this, &BitcoinGUI::gotoExchangeP2PPage);
    connect(historyAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(historyAction, &QAction::triggered, this, &BitcoinGUI::gotoHistoryPage);
    connect(treasuryAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(treasuryAction, &QAction::triggered, this, &BitcoinGUI::gotoTreasuryPage);
    connect(modelsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(modelsAction, &QAction::triggered, this, &BitcoinGUI::gotoModelsPage);
    connect(miningApisAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(miningApisAction, &QAction::triggered, this, &BitcoinGUI::gotoMiningApisPage);
#endif // ENABLE_WALLET
    ...
    m_export_descriptors_action = new QAction(tr("Export Descriptors for Hosted Wallet…"), this);
    m_export_descriptors_action->setStatusTip(tr("Export private descriptors as a migration bundle for the hosted wallet"));
    m_import_descriptors_action = new QAction(tr("Import Descriptors from Hosted Wallet…"), this);
    m_import_descriptors_action->setStatusTip(tr("Import a descriptor migration bundle into a new wallet"));
    changePassphraseAction = new QAction(tr("&Change Passphrase…"), this);
    changePassphraseAction->setStatusTip(tr("Change the passphrase used for wallet encryption"));
    signMessageAction = new QAction(tr("Sign &message…"), this);
    signMessageAction->setStatusTip(tr("Sign messages with your TensorCash addresses to prove you own them"));
    verifyMessageAction = new QAction(tr("&Verify message…"), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified TensorCash addresses"));
    m_load_psbt_action = new QAction(tr("&Load PSBT from file…"), this);
    m_load_psbt_action->setStatusTip(tr("Load Partially Signed TensorCash Transaction"));
    m_load_psbt_clipboard_action = new QAction(tr("Load PSBT from &clipboard…"), this);
    m_load_psbt_clipboard_action->setStatusTip(tr("Load Partially Signed TensorCash Transaction from clipboard"));
    ...
    openAction->setStatusTip(tr("Open a tensorcash: URI"));
    ...
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible TensorCash command-line options").arg(CLIENT_NAME));
    ...
    if(walletFrame)
    {
        ...
        connect(m_import_descriptors_action, &QAction::triggered, [this] {
            auto* dlg = new ImportDescriptorsDialog(m_wallet_controller, clientModel, this);
...
void BitcoinGUI::createToolBars()
{
    if(walletFrame)
    {
        ...
        toolbar->addAction(treasuryAction);
        toolbar->addAction(exchangeP2PAction);
        toolbar->addAction(miningApisAction);
        toolbar->addAction(modelsAction);
...
void BitcoinGUI::addWallet(WalletModel* walletModel)
{
    ...
    connect(wallet_view, &WalletView::requestGotoModelsPage, this, &BitcoinGUI::gotoModelsPage);
...
void BitcoinGUI::setWalletActionsEnabled(bool enabled)
{
    ...
    exchangeP2PAction->setEnabled(enabled);
    historyAction->setEnabled(enabled);
    treasuryAction->setEnabled(enabled);
    modelsAction->setEnabled(enabled);
...
void BitcoinGUI::gotoExchangeP2PPage()
{
    exchangeP2PAction->setChecked(true);
    if (walletFrame) walletFrame->gotoExchangeP2PPage();
}

void BitcoinGUI::gotoTreasuryPage()
{
    treasuryAction->setChecked(true);
    if (walletFrame) walletFrame->gotoTreasuryPage();
}

void BitcoinGUI::gotoModelsPage()
{
    modelsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoModelsPage();
}

void BitcoinGUI::gotoMiningApisPage()
{
    miningApisAction->setChecked(true);
    if (walletFrame) walletFrame->gotoMiningApisPage();
}
...
void BitcoinGUI::updateNetworkState()
{
    ...
    if (m_node.getNetworkActive()) {
        ...
        tooltip = tr("%n active connection(s) to TensorCash network.", "", count);
    } else {
        //: A substring of the tooltip.
