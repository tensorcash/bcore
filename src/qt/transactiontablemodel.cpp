...
static int column_alignments[] = {
        ...
        Qt::AlignLeft|Qt::AlignVCenter, /*asset=*/
        Qt::AlignRight|Qt::AlignVCenter, /*asset_amount=*/
        ...
        platformStyle(_platformStyle)
{
    ...
    columns << QString() << QString() << tr("Date") << tr("Type") << tr("Label") << tr("Asset") << tr("Asset Amount") << BitcoinUnits::getAmountColumnTitle(walletModel->getOptionsModel()->getDisplayUnit());
...
QString TransactionTableModel::formatTxType(const TransactionRecord *wtx) const
{
    ...
    case TransactionRecord::RecvWithAddress:
        type = tr("Received with");
        break;
    case TransactionRecord::RecvFromOther:
        type = tr("Received from");
        break;
    ...
    // Add post-quantum indicator if transaction uses ML-DSA
    if (wtx->mldsa_input || wtx->mldsa_output) {
        type += " [PQ]";
    }
...
QString TransactionTableModel::formatTxAsset(const TransactionRecord *wtx) const
{
    if (!wtx->is_asset_transfer) {
        return QString("TSC");
    }

    QString ticker = wtx->asset_ticker;
    bool isRegistered = wtx->asset_is_registered;

    // Consult live balances when we are missing display metadata or registration status.
    if (wtx->asset_id.has_value()) {
        if (ticker.isEmpty() || !wtx->asset_has_ticker || !isRegistered) {
            const auto asset_balances = walletModel->getAssetBalances();
            for (const auto& asset : asset_balances) {
                if (asset.asset_id == wtx->asset_id.value()) {
                    if ((ticker.isEmpty() || !wtx->asset_has_ticker) && !asset.ticker.empty()) {
                        ticker = QString::fromStdString(asset.ticker);
                    }
                    if (!isRegistered) {
                        isRegistered = asset.is_registered;
                    }
                    break;
                }
            }
        ...
        if (ticker.isEmpty()) {
            ticker = QString::fromStdString(wtx->asset_id->ToString()).left(8) + "...";
        }
    }

    if (ticker.isEmpty()) {
        ticker = QStringLiteral("?");
    }

    QString displayTicker = ticker;
    if (!isRegistered) {
        displayTicker = QStringLiteral("⚠ %1").arg(ticker);
    }

    return displayTicker;
}

QString TransactionTableModel::formatTxAssetAmount(const TransactionRecord *wtx) const
{
    if (!wtx->is_asset_transfer) {
        return QString(); // Empty for TSC transactions
    }

    // Determine sign based on net amount (debit values are stored as negative)
    const qint64 netAmount = wtx->credit + wtx->debit;
    const QString sign = (netAmount > 0) ? QStringLiteral("+") : QStringLiteral("-");

    uint8_t decimals = wtx->asset_decimals;

    // Consult live balances for updated decimals if available
    if (wtx->asset_id.has_value()) {
        const auto asset_balances = walletModel->getAssetBalances();
        for (const auto& asset : asset_balances) {
            if (asset.asset_id == wtx->asset_id.value()) {
                if (asset.has_decimals) {
    ...
    const QString amountStr = GUIUtil::formatAssetAmount(wtx->asset_units, decimals);

    return QStringLiteral("%1%2").arg(sign, amountStr);
}
...
QString TransactionTableModel::formatTooltip(const TransactionRecord *rec) const
{
    ...
    if (rec->is_asset_transfer && rec->asset_id.has_value()) {
        QString assetLabel = rec->asset_ticker.isEmpty()
            ? QString::fromStdString(rec->asset_id->ToString()).left(8) + "..."
            : rec->asset_ticker;
        tooltip += QStringLiteral("\n%1: %2")
            .arg(tr("Asset"), assetLabel);

        if (rec->asset_units > 0) {
            tooltip += QStringLiteral(" (%1 %2)")
                .arg(GUIUtil::formatAssetAmount(rec->asset_units, rec->asset_decimals))
                .arg(assetLabel);
        }
        if (!rec->asset_is_registered) {
            tooltip += QStringLiteral("\n%1")
                .arg(tr("Warning: registry entry not found for this asset."));
        }
    }
    if (rec->mldsa_input || rec->mldsa_output) {
        tooltip += QString("\n") + tr("Transaction uses Post-Quantum (ML-DSA) signatures");
    }
    return tooltip;
}
...
QVariant TransactionTableModel::data(const QModelIndex &index, int role) const
{
    ...
    case RawDecorationRole:
        ...
        case Asset: return {};  // No decoration for asset column
        case AssetAmount: return {};  // No decoration for asset amount column
        case Amount: return {};
    ...
    case Qt::DisplayRole:
        ...
        case Asset:
            return formatTxAsset(rec);
        case AssetAmount:
            return formatTxAssetAmount(rec);
        case Amount:
    ...
    case Qt::EditRole:
        ...
        case Asset:
            return rec->is_asset_transfer ?
                QString::fromStdString(rec->asset_id.value().ToString()) :
                QString("TSC");
        case AssetAmount:
            return rec->is_asset_transfer ? static_cast<qint64>(rec->asset_units) : 0;
        case Amount:
    ...
    case Qt::ForegroundRole:
        ...
        if (index.column() == Asset && rec->is_asset_transfer && !rec->asset_is_registered) {
    ...
    case AssetTickerRole:
        return rec->is_asset_transfer ? rec->asset_ticker : QString("TSC");
    case AssetAmountRole:
        return rec->is_asset_transfer ? QVariant::fromValue(rec->asset_units) : QVariant();
    }
    return QVariant();
}
...
QVariant TransactionTableModel::headerData(int section, Qt::Orientation orientation, int role) const
{
    if(orientation == Qt::Horizontal)
    {
        ...
        } else if (role == Qt::ToolTipRole)
        {
            ...
            case Asset:
                return tr("Asset type being transferred (TSC for regular transactions).");
            case AssetAmount:
                return tr("Asset amount transferred (blank for TSC transactions).");
            case Amount:
