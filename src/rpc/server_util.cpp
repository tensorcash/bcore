...
#ifdef ENABLE_WALLET
    if (!node_context) {
        if (auto wallet_context = util::AnyPtr<wallet::WalletContext>(context)) {
            node_context = wallet_context && wallet_context->node_context ? wallet_context->node_context : nullptr;
        }
    }
#endif
    if (!node_context) {
