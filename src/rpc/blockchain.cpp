...
UniValue blockheaderToJSON(const CBlockIndex& tip, const CBlockIndex& blockindex, const uint256 pow_limit)
{
    ...
    result.pushKV("shortHash", blockindex.GetBlockShortHash().GetHex());
    ...
    result.pushKV("adjBits", strprintf("%08x", blockindex.nAdjBits));
    result.pushKV("target", GetTarget(tip, pow_limit).GetHex());
    result.pushKV("difficulty", GetDifficulty(blockindex));
    result.pushKV("chainwork", blockindex.nChainWork.GetHex());
    result.pushKV("hashPoW", blockindex.hashPoW.GetHex());
...
UniValue blockToJSON(BlockManager& blockman, const CBlock& block, const CBlockIndex& tip, const CBlockIndex& blockindex, TxVerbosity verbosity, const uint256 pow_limit)
{
    ...
    result.pushKV("tst", block.pow.model_identifier);
                ...
                },
                {
                    RPCResult{"for verbose = true",
                        RPCResult::Type::OBJ, "", "",
                        {
                            ...
                            {RPCResult::Type::STR_HEX, "shortHash", "the shortHash (used for PoW check)"},
                            ...
                            {RPCResult::Type::STR_HEX, "adjBits", "nBits: compact representation of the block difficulty target with model modifier"},
                            {RPCResult::Type::STR_HEX, "target", "The difficulty target"},
                            {RPCResult::Type::NUM, "difficulty", "The difficulty"},
                            {RPCResult::Type::STR_HEX, "chainwork", "Expected number of hashes required to produce the current chain"},
                            {RPCResult::Type::STR_HEX, "hashPoW", "The PoW blob hash"},
    ...
    return RPCHelpMan{"gettxout",
        ...
        },
        {
            ...
            RPCResult{"Otherwise", RPCResult::Type::OBJ, "", "", {
                ...
                {RPCResult::Type::STR_HEX, "asset_id", /*optional=*/true, "Asset ID if this UTXO contains an asset"},
                {RPCResult::Type::NUM, "asset_units", /*optional=*/true, "Asset units if this UTXO contains an asset"},
            }},
        },
        RPCExamples{
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    // Parse and include asset data if present (from vExt TLV)
    if (auto asset_tag = assets::ParseAssetTag(coin->out.vExt)) {
        ret.pushKV("asset_id", asset_tag->id.GetHex());
        ret.pushKV("asset_units", asset_tag->amount);
    }
                    ...
                    {{RPCResult::Type::OBJ, "", "",
                        {
                            ...
                            {RPCResult::Type::STR_HEX, "shortHash", "short-derived hash of the tip header (Tensor extension)"},
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    for (const CBlockIndex* block : setTips) {
        ...
        obj.pushKV("shortHash", block->GetBlockShortHash().GetHex());
...
static RPCHelpMan getreorgadvisories()
{
    return RPCHelpMan{"getreorgadvisories",
                "\nReturns recent reorg advisory data.\n"
                "Reorg advisories are generated when the node detects a chain reorganization\n"
                "deeper than 3 blocks, providing metrics to help assess the event.\n",
                {
                    {"count", RPCArg::Type::NUM, RPCArg::Default{10}, "Number of recent advisories to return (max 100)"},
                },
                RPCResult{
                    RPCResult::Type::OBJ, "", "",
                    {
                        {RPCResult::Type::NUM, "total_stored", "Total number of advisories in memory"},
                        {RPCResult::Type::ARR, "advisories", "Recent reorg advisories (newest first)",
                        {
                            {RPCResult::Type::OBJ, "", "",
                            {
                                {RPCResult::Type::BOOL, "valid", "Whether advisory data is complete"},
                                {RPCResult::Type::NUM, "lca_height", "Height of the last common ancestor"},
                                {RPCResult::Type::NUM, "depth_current", "Reorg depth on the current chain"},
                                {RPCResult::Type::NUM, "depth_fork", "Reorg depth on the fork chain"},
                                {RPCResult::Type::NUM, "tx_overlap_pct", "Transaction overlap percentage (Jaccard index)"},
                                {RPCResult::Type::NUM, "first_block_delay_secs", "Delay (seconds) before first fork block was seen"},
                                {RPCResult::Type::NUM, "hashrate_current_pct", "Estimated hashrate on current segment as % of baseline"},
                                {RPCResult::Type::NUM, "hashrate_fork_pct", "Estimated hashrate on fork segment as % of baseline"},
                                {RPCResult::Type::NUM, "since_last_block_secs", "Seconds since last block was seen at advisory time"},
                                {RPCResult::Type::OBJ, "calibration", "Tick-to-time calibration data",
                                {
                                    {RPCResult::Type::BOOL, "valid", "Whether calibration succeeded"},
                                    {RPCResult::Type::NUM, "sec_per_tick", "Seconds per VDF tick"},
                                    {RPCResult::Type::NUM, "baseline_hashrate", "Baseline network hashrate"},
                                    {RPCResult::Type::NUM, "window_blocks", "Number of blocks used for calibration"},
                                    {RPCResult::Type::STR, "invalid_reason", /*optional=*/true, "Reason if calibration failed"},
                                }},
                                {RPCResult::Type::OBJ, "segment_current", "Statistics for current chain segment",
                                {
                                    {RPCResult::Type::NUM, "block_count", "Number of blocks"},
                                    {RPCResult::Type::NUM, "ticks_diff", "VDF tick difference"},
                                    {RPCResult::Type::NUM, "clock_time_secs", "Wall-clock time in seconds"},
                                    {RPCResult::Type::NUM, "miner_time_secs", "Miner-reported time in seconds"},
                                    {RPCResult::Type::NUM, "tx_count", "Number of non-coinbase transactions"},
                                    {RPCResult::Type::NUM, "blocks_read", "Blocks successfully read for txids"},
                                    {RPCResult::Type::NUM, "blocks_pruned", "Blocks that couldn't be read"},
                                    {RPCResult::Type::BOOL, "txids_capped", "Whether txid collection was limited"},
                                    {RPCResult::Type::BOOL, "data_complete", "Whether all required data was available"},
                                }},
                                {RPCResult::Type::OBJ, "segment_fork", "Statistics for fork chain segment",
                                {
                                    {RPCResult::Type::NUM, "block_count", "Number of blocks"},
                                    {RPCResult::Type::NUM, "ticks_diff", "VDF tick difference"},
                                    {RPCResult::Type::NUM, "clock_time_secs", "Wall-clock time in seconds"},
                ...
                RPCExamples{
                    HelpExampleCli("getreorgadvisories", "")
                    + HelpExampleCli("getreorgadvisories", "5")
                    + HelpExampleRpc("getreorgadvisories", "10")
                },
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    size_t count = 10;
    if (!request.params[0].isNull()) {
        count = request.params[0].getInt<size_t>();
        if (count > MAX_STORED_ADVISORIES) {
            count = MAX_STORED_ADVISORIES;
        }
    }

    ReorgAdvisoryStore& store = GetReorgAdvisoryStore();
    std::vector<ReorgAdvisory> advisories = store.GetRecent(count);

    UniValue result(UniValue::VOBJ);
    result.pushKV("total_stored", static_cast<int64_t>(store.Size()));

    UniValue arr(UniValue::VARR);
    for (const ReorgAdvisory& adv : advisories) {
        UniValue obj(UniValue::VOBJ);
        obj.pushKV("valid", adv.is_valid);
        obj.pushKV("lca_height", adv.lca_height);
        obj.pushKV("depth_current", adv.depth_current);
        obj.pushKV("depth_fork", adv.depth_fork);
        obj.pushKV("tx_overlap_pct", adv.tx_overlap_pct);
        obj.pushKV("first_block_delay_secs", adv.first_block_delay_secs);
        obj.pushKV("hashrate_current_pct", adv.hashrate_current_pct);
        obj.pushKV("hashrate_fork_pct", adv.hashrate_fork_pct);
        obj.pushKV("since_last_block_secs", adv.since_last_block_secs);

        // Calibration object
        UniValue cal(UniValue::VOBJ);
        cal.pushKV("valid", adv.calibration.is_valid);
        cal.pushKV("sec_per_tick", adv.calibration.sec_per_tick);
        cal.pushKV("baseline_hashrate", adv.calibration.baseline_hashrate);
        cal.pushKV("window_blocks", adv.calibration.window_blocks);
        if (!adv.calibration.is_valid && !adv.calibration.invalid_reason.empty()) {
            cal.pushKV("invalid_reason", adv.calibration.invalid_reason);
        }
        obj.pushKV("calibration", cal);

        // Segment current object
        UniValue seg_cur(UniValue::VOBJ);
        seg_cur.pushKV("block_count", adv.seg_current.block_count);
        seg_cur.pushKV("ticks_diff", static_cast<int64_t>(adv.seg_current.ticks_diff));
        seg_cur.pushKV("clock_time_secs", adv.seg_current.clock_time_secs);
        seg_cur.pushKV("miner_time_secs", adv.seg_current.miner_time_secs);
        seg_cur.pushKV("tx_count", static_cast<int64_t>(adv.seg_current.txids.size()));
        seg_cur.pushKV("blocks_read", adv.seg_current.blocks_read_for_txids);
        seg_cur.pushKV("blocks_pruned", adv.seg_current.blocks_pruned);
        seg_cur.pushKV("txids_capped", adv.seg_current.txids_capped);
        seg_cur.pushKV("data_complete", adv.seg_current.data_complete);
        obj.pushKV("segment_current", seg_cur);

        // Segment fork object
        UniValue seg_fork(UniValue::VOBJ);
        seg_fork.pushKV("block_count", adv.seg_fork.block_count);
        seg_fork.pushKV("ticks_diff", static_cast<int64_t>(adv.seg_fork.ticks_diff));
        seg_fork.pushKV("clock_time_secs", adv.seg_fork.clock_time_secs);
        seg_fork.pushKV("miner_time_secs", adv.seg_fork.miner_time_secs);
        seg_fork.pushKV("tx_count", static_cast<int64_t>(adv.seg_fork.txids.size()));
        seg_fork.pushKV("blocks_read", adv.seg_fork.blocks_read_for_txids);
        seg_fork.pushKV("blocks_pruned", adv.seg_fork.blocks_pruned);
        seg_fork.pushKV("txids_capped", adv.seg_fork.txids_capped);
        seg_fork.pushKV("data_complete", adv.seg_fork.data_complete);
        obj.pushKV("segment_fork", seg_fork);

        arr.push_back(obj);
    }

    result.pushKV("advisories", arr);
    return result;
},
    };
}

static RPCHelpMan clearreorgadvisories()
{
    return RPCHelpMan{"clearreorgadvisories",
                "\nClears all stored reorg advisories from memory.\n",
                {},
                RPCResult{
                    RPCResult::Type::OBJ, "", "",
                    {
                        {RPCResult::Type::NUM, "cleared", "Number of advisories that were cleared"},
                    }
                },
                RPCExamples{
                    HelpExampleCli("clearreorgadvisories", "")
                    + HelpExampleRpc("clearreorgadvisories", "")
                },
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ReorgAdvisoryStore& store = GetReorgAdvisoryStore();
    ...
    UniValue result(UniValue::VOBJ);
    result.pushKV("cleared", static_cast<int64_t>(count));
    return result;
},
    };
}

static RPCHelpMan getlatestreorgadvisory()
{
    return RPCHelpMan{"getlatestreorgadvisory",
                "\nReturns the most recent reorg advisory, or null if none exists.\n",
                {},
                {
                    RPCResult{"If no advisories exist", RPCResult::Type::NONE, "", ""},
                    RPCResult{"Otherwise", RPCResult::Type::OBJ, "", "",
                    {
                        {RPCResult::Type::BOOL, "valid", "Whether advisory data is complete"},
                        {RPCResult::Type::NUM, "lca_height", "Height of the last common ancestor"},
                        ...
                        {RPCResult::Type::OBJ, "segment_current", "Statistics for current chain segment",
                        {
                            ...
                            {RPCResult::Type::NUM, "miner_time_secs", "Miner-reported time in seconds"},
                            {RPCResult::Type::NUM, "tx_count", "Number of non-coinbase transactions"},
                            {RPCResult::Type::NUM, "blocks_read", "Number of blocks read for txid analysis"},
                            {RPCResult::Type::NUM, "blocks_pruned", "Number of blocks that were pruned"},
                            {RPCResult::Type::BOOL, "txids_capped", "Whether txid collection was capped"},
                            {RPCResult::Type::BOOL, "data_complete", "Whether all data was collected"},
                        }},
                        {RPCResult::Type::OBJ, "segment_fork", "Statistics for fork chain segment",
                        {
                            {RPCResult::Type::NUM, "block_count", "Number of blocks"},
                ...
                RPCExamples{
                    HelpExampleCli("getlatestreorgadvisory", "")
                    + HelpExampleRpc("getlatestreorgadvisory", "")
                },
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ReorgAdvisoryStore& store = GetReorgAdvisoryStore();
    std::optional<ReorgAdvisory> latest = store.GetLatest();

    if (!latest) {
        return UniValue(UniValue::VNULL);
    }

    const ReorgAdvisory& adv = *latest;
    UniValue obj(UniValue::VOBJ);
    obj.pushKV("valid", adv.is_valid);
    obj.pushKV("lca_height", adv.lca_height);
    obj.pushKV("depth_current", adv.depth_current);
...
static RPCHelpMan getpendingreorg()
{
    return RPCHelpMan{"getpendingreorg",
                ...
                RPCExamples{
                    HelpExampleCli("getpendingreorg", "")
                    + HelpExampleRpc("getpendingreorg", "")
                },
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ChainstateManager& chainman = EnsureAnyChainman(request.context);
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    if (action == "accept") {
        decision = ReorgDecision::ACCEPT;
    } else if (action == "reject") {
        decision = ReorgDecision::REJECT;
    } else {
...
void RegisterBlockchainRPCCommands(CRPCTable& t)
{
    static const CRPCCommand commands[]{
        ...
        {"blockchain", &getreorgadvisories},
        {"blockchain", &clearreorgadvisories},
