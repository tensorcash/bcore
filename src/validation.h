...
private:
    ...
    //! Non-owning difficulty-covenant fixing resolver (OP_NBITS_AT / OP_DIFFCFD_SETTLE). Null in
    //! contexts without chain access; must outlive the check (the resolver is a stack object in
    //! ConnectBlock / PreChecks that lives until the parallel check queue drains).
    const FixingContext* m_fixing_context;

public:
    CScriptCheck(const CTxOut& outIn, const CTransaction& txToIn, SignatureCache& signature_cache, unsigned int nInIn, unsigned int nFlagsIn, bool cacheIn, PrecomputedTransactionData* txdataIn, const FixingContext* fixingIn = nullptr) :
        m_tx_out(outIn), ptxTo(&txToIn), nIn(nInIn), nFlags(nFlagsIn), cacheStore(cacheIn), txdata(txdataIn), m_signature_cache(&signature_cache), m_fixing_context(fixingIn) { }
 ...
 *
 * NOTE: the model-registry check is NOT performed here. Model registration is
 * chain state (it depends on which blocks have been connected), so it is
 * enforced at connection time in ConnectBlock(), where ModelDB is synced to
 * the block's parent. Keeping CheckBlock() context-free prevents false
 * `unreg-model` rejections during -reindex import and out-of-order IBD. */
bool CheckBlock(const CBlock& block, BlockValidationState& state, const Consensus::Params& consensusParams, bool fCheckApi = true, bool fCheckPOW = true, bool fCheckMerkleRoot = true);
...
public:
     ...
     */
    std::set<CBlockIndex*, node::CBlockIndexPolicyComparator> setBlockIndexCandidates;
    ...
    DisconnectResult DisconnectBlock(const CBlock& block, const CBlockIndex* pindex, CCoinsViewCache& view, bool update_modeldb = true)
        EXCLUSIVE_LOCKS_REQUIRED(::cs_main);
    bool ConnectBlock(const CBlock& block, BlockValidationState& state, CBlockIndex* pindex,
                      CCoinsViewCache& view, bool fJustCheck = false, bool update_side_indexes = true) EXCLUSIVE_LOCKS_REQUIRED(cs_main);
...
public:
    ...
    std::unordered_set <uint256, BlockHasher> m_recent_announced_set GUARDED_BY(::cs_main);
    ...
    // SPV sampling path: accept a block for storage/indexing without external API dependency.
    bool ProcessNewSampledBlock(const std::shared_ptr<const CBlock>& block) LOCKS_EXCLUDED(cs_main);
    ...
    bool EarlyPropagation(const std::shared_ptr<const CBlock>& pblock) EXCLUSIVE_LOCKS_REQUIRED(cs_main);
     ...
     */
    bool AcceptBlock(const std::shared_ptr<const CBlock>& pblock, BlockValidationState& state, CBlockIndex** ppindex, bool fRequested, const FlatFilePos* dbp, bool* fNewBlock, bool min_pow_checked, bool need_full_validation) EXCLUSIVE_LOCKS_REQUIRED(cs_main);
    ...
    //! Persist local Full RED status on a known block index.
    //! This bit participates in local policy fork-choice. The setter clears
    //! block-index candidates before mutation; callers that change it must rebuild
    //! them through RecalculateBlockIndexWorkForFullValidation().
    CBlockIndex* SetBlockFullValidationRedStatus(const uint256& block_hash, bool full_red, bool* changed = nullptr) EXCLUSIVE_LOCKS_REQUIRED(::cs_main);
    bool SetBlockFullValidationRedStatus(CBlockIndex* pindex, bool full_red) EXCLUSIVE_LOCKS_REQUIRED(::cs_main);

    //! Mark every known block mined by a successfully banned model at or after
    //! min_height as zero-work. This is consensus-derived challenge state, not
    //! local Full RED state.
    bool ApplyModelChallengeZeroWork(const uint256& model_hash, int min_height) EXCLUSIVE_LOCKS_REQUIRED(::cs_main);
    bool ClearModelChallengeZeroWork(const uint256& model_hash, int min_height) EXCLUSIVE_LOCKS_REQUIRED(::cs_main);
    bool SetBlockModelChallengeZeroWorkStatus(CBlockIndex* pindex, bool zero_work) EXCLUSIVE_LOCKS_REQUIRED(::cs_main);

    //! Recompute block-index chainwork so zero-work blocks contribute no proof.
    void RecalculateBlockIndexWorkForFullValidation(const CBlockIndex* pindex_start = nullptr) EXCLUSIVE_LOCKS_REQUIRED(::cs_main);
