...
namespace assets { struct IcuStorageEntry; }
...
struct CoinsViewCacheCursor
{
    ...
    //! Optional asset-registry delta to commit alongside the coin changes in the
    //! SAME final BatchWrite batch (never in a partial coin batch). Null when the
    //! flushing view has no staged asset mutations. Carried on the cursor so the
    //! BatchWrite virtual signature is unchanged.
    CAssetRegistryDelta* AssetDelta() const noexcept { return m_asset_delta; }
    void SetAssetDelta(CAssetRegistryDelta* delta) noexcept { m_asset_delta = delta; }
...
private:
    ...
    CCoinsMap& m_map;
    bool m_will_erase;
    CAssetRegistryDelta* m_asset_delta{nullptr};
};
...
public:
    ...
    //! Asset-registry reads, exposed through the view stack so validation reads
    //! asset state via the same logical view as coins instead of dynamic_cast'ing
    //! down to CCoinsViewDB. The default implementations report "not found";
    //! CCoinsViewDB (the authoritative store) overrides them and CCoinsViewBacked
    //! forwards to its base. Consensus::CheckTxInputs reads through these.
    //! CCoinsViewCache carries a CAssetRegistryDelta overlay (staged writes/erases
    //! consulted before the base) that CCoinsViewDB::BatchWrite commits. The
    //! remaining work is to have ConnectBlock/DisconnectBlock stage into the view
    //! instead of writing the DB directly.
    virtual bool ReadAssetPolicy(const uint256& aid, AssetRegistryEntry& out) const;
    virtual bool ReadTickerBinding(const std::string& ticker, uint256& out_asset) const;
    virtual bool ReadZkVerifyingKey(const uint256& vk_hash, std::vector<unsigned char>& out) const;
    virtual bool ReadIcuPayload(const uint256& asset_id, const uint256& icu_ctxt_commit, assets::IcuStorageEntry& out) const;
    //! Scalar-feed reads (CFD_GENERALISATION.md §3): a per-epoch publication record
    //! and the per-feed head (last published epoch). O(1) point lookups.
    virtual bool ReadAssetScalar(const uint256& asset_id, uint32_t feed_id, uint64_t epoch, ScalarRecord& out) const;
    virtual bool ReadAssetScalarHead(const uint256& asset_id, uint32_t feed_id, uint64_t& out_last_epoch) const;
...
public:
    ...
    // Forward asset-registry reads to the backing view.
    bool ReadAssetPolicy(const uint256& aid, AssetRegistryEntry& out) const override { return base->ReadAssetPolicy(aid, out); }
    bool ReadTickerBinding(const std::string& ticker, uint256& out_asset) const override { return base->ReadTickerBinding(ticker, out_asset); }
    bool ReadZkVerifyingKey(const uint256& vk_hash, std::vector<unsigned char>& out) const override { return base->ReadZkVerifyingKey(vk_hash, out); }
    bool ReadIcuPayload(const uint256& asset_id, const uint256& icu_ctxt_commit, assets::IcuStorageEntry& out) const override { return base->ReadIcuPayload(asset_id, icu_ctxt_commit, out); }
    bool ReadAssetScalar(const uint256& asset_id, uint32_t feed_id, uint64_t epoch, ScalarRecord& out) const override { return base->ReadAssetScalar(asset_id, feed_id, epoch, out); }
    bool ReadAssetScalarHead(const uint256& asset_id, uint32_t feed_id, uint64_t& out_last_epoch) const override { return base->ReadAssetScalarHead(asset_id, feed_id, out_last_epoch); }

    const CCoinsView* GetBase() const noexcept { return base; }
    CCoinsView* GetBase() noexcept { return base; }
};
...
public:
    ...
    //! Out-of-line so the unique_ptr<CAssetRegistryDelta> member (incomplete in
    //! this header) is destroyed in coins.cpp.
    ~CCoinsViewCache();
    ...
    // Staged policy/ticker/VK/ICU writes and erases live in an in-memory
    // CAssetRegistryDelta and are consulted by these Read* overrides before
    // falling through to the base view, mirroring how cacheCoins overlays coins.
    // The delta accumulates across blocks until a flush, where Flush()/Sync() hand
    // it to CCoinsViewDB::BatchWrite, which commits it in the final batch. Lazily
    // allocated, so coin-only caches pay nothing.
    ...
    void StageAssetPolicy(const uint256& aid, const AssetRegistryEntry& entry);
    void StageEraseAssetPolicy(const uint256& aid);
    void StageTickerBinding(const std::string& ticker, const uint256& asset);
    void StageEraseTickerBinding(const std::string& ticker);
    void StageZkVerifyingKey(const uint256& vk_hash, const std::vector<unsigned char>& payload);
    void StageEraseZkVerifyingKey(const uint256& vk_hash);
    void StageIcuPayload(const uint256& asset_id, const uint256& icu_ctxt_commit, const assets::IcuStorageEntry& entry);
    void StageEraseIcuPayload(const uint256& asset_id, const uint256& icu_ctxt_commit);
    //! Stage a scalar publication record and/or its per-feed head. Both go into the
    //! same delta and commit atomically (CFD_GENERALISATION.md §3.2).
    void StageAssetScalar(const uint256& asset_id, uint32_t feed_id, uint64_t epoch, const ScalarRecord& record);
    void StageEraseAssetScalar(const uint256& asset_id, uint32_t feed_id, uint64_t epoch);
    void StageAssetScalarHead(const uint256& asset_id, uint32_t feed_id, uint64_t last_epoch);
    void StageEraseAssetScalarHead(const uint256& asset_id, uint32_t feed_id);

    //! Report asset ids with a staged policy write/erase in this cache's in-memory
    //! overlay (not yet flushed to the base DB). `added` receives ids whose staged
    //! op is a write, `erased` those whose op is an erase. Lets a caller that
    //! enumerates the on-disk registry reconcile it with the live view (e.g. the
    //! listregisteredassets RPC), so an asset registered since the last flush is
    //! not invisible. Read-only; does not allocate the overlay.
    void GetStagedAssetPolicyIds(std::vector<uint256>& added, std::vector<uint256>& erased) const;

    //! Report staged scalar-feed heads for an asset from the in-memory overlay (not yet
    //! flushed): `added` receives (feed_id, last_epoch) for staged writes, `erased` the
    //! feed_ids with a staged erase. Mirrors GetStagedAssetPolicyIds so scalarlistfeeds
    //! can reconcile the on-disk enumeration with a just-connected unflushed feed.
    void GetStagedScalarHeads(const uint256& asset_id,
                              std::vector<std::pair<uint32_t, uint64_t>>& added,
                              std::vector<uint32_t>& erased) const;

private:
    //! Lazily-allocated asset-registry overlay (see the Read*/Stage* methods above).
    std::unique_ptr<CAssetRegistryDelta> m_asset_delta;

    //! Lazily create and return the asset overlay.
    CAssetRegistryDelta& EnsureDelta();
