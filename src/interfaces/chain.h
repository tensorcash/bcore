...
public:
    ...
    //! Asset registry helpers for wallets operating against a remote node.
    virtual std::optional<AssetRegistryEntry> getAssetRegistryEntry(const uint256& asset_id) = 0;
    virtual std::optional<uint256> getAssetIdByTicker(const std::string& ticker) = 0;
