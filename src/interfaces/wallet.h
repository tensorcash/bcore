...
public:
    ...
    //! Fund transaction from template (adds fee inputs to pre-selected inputs).
    virtual util::Result<FundTransactionResult> fundTransaction(const CMutableTransaction& tx,
    ...
    //! Get asset balances.
    virtual std::vector<AssetBalance> getAssetBalances() = 0;

    //! Get the base64-encoded DEK for an asset the wallet holds (nullopt if none).
    //! Sensitive: reveals the ICU governance document to anyone who learns it.
    virtual std::optional<std::string> getAssetDek(const uint256& asset_id) = 0;

    //! Get asset UTXOs for a specific asset.
    virtual std::vector<AssetUtxo> getAssetUtxos(const uint256& asset_id, int min_depth = 1) = 0;
...
struct WalletBalances
{
    ...
    CAmount bonded_balance = 0;
    bool have_watch_only = false;
    CAmount watch_only_balance = 0;
    CAmount unconfirmed_watch_only_balance = 0;
    CAmount immature_watch_only_balance = 0;
    CAmount watch_only_bonded_balance = 0;

    bool balanceChanged(const WalletBalances& prev) const
    {
        return balance != prev.balance || unconfirmed_balance != prev.unconfirmed_balance ||
               immature_balance != prev.immature_balance || bonded_balance != prev.bonded_balance ||
               watch_only_balance != prev.watch_only_balance ||
               unconfirmed_watch_only_balance != prev.unconfirmed_watch_only_balance ||
               immature_watch_only_balance != prev.immature_watch_only_balance ||
               watch_only_bonded_balance != prev.watch_only_bonded_balance;
    }
};
...
struct WalletTx
{
    ...
    std::vector<AssetDisplayInfo> asset_details;
    CAmount credit;
    CAmount debit;
    CAmount change;
    int64_t time;
...
//! Asset UTXO information
struct AssetUtxo
{
    COutPoint outpoint;
    uint256 asset_id;
    uint64_t units = 0;
    CAmount value = 0;  // BTC dust value
    bool spendable = false;
    int depth = 0;  // Confirmations
};

//! Transaction funding result
struct FundTransactionResult
{
