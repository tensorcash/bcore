...
/**
 * Asset registration carried by an UNCONFIRMED mempool transaction. The mempool tracks
 * these so admission can reject duplicate registrations and ticker collisions against
 * other in-mempool txs (matching the same-block registration visibility ConnectBlock has
 * via its staging overlay). This is REGISTRATION-level only: transfer validation continues
 * to read the confirmed prior-block registry (the one-confirmation rule), so these records
 * are never consulted by CheckTxInputs / the virtual CCoinsView asset accessors.
 */
struct MempoolAssetReg {
    COutPoint icu_outpoint;   //!< the IssuerReg output that carries this registration
    std::string ticker;       //!< bound ticker ("" if none)
    uint8_t decimals{0xFF};   //!< declared decimals (0xFF = unset)
    Txid txid;                //!< the mempool tx carrying this registration (chain tip per asset)
};
...
private:
    ...
    /**
     * Asset-registration overlay (registration-conflict detection only; see MempoolAssetReg).
     * Single source of truth: asset_id -> the registration carried by the in-mempool tx for that
     * asset. Mempool admission rejects BOTH an independent duplicate AND a would-be bond rotation
     * of a mempool-only asset (we cannot validate rotation bond rules — rotation_min / unlock /
     * accrued fees live in the confirmed registry), so at most one mempool tx ever registers a
     * given asset; the map is therefore single-valued, not a rotation chain. Ticker bindings are
     * DERIVED from this map (GetMempoolTickerBinding scans it) rather than held in a second map,
     * so the two can never drift (the bug a separate ticker->asset map had). Maintained in
     * addNewTransaction() (AddAssetRegs) / removeUnchecked() (RemoveAssetRegs, erasing only the
     * entry this tx owns, matched by txid).
     */
    std::map<uint256, MempoolAssetReg> m_asset_regs GUARDED_BY(cs);

    void AddAssetRegs(const CTransaction& tx) EXCLUSIVE_LOCKS_REQUIRED(cs);
    void RemoveAssetRegs(const CTransaction& tx) EXCLUSIVE_LOCKS_REQUIRED(cs);
...
public:
     ...
     * check does nothing.
     */
    void check(const CCoinsViewCache& active_coins_tip, int64_t spendheight) EXCLUSIVE_LOCKS_REQUIRED(::cs_main);
    ...
    /** The registration carried by the unconfirmed mempool tx for `asset_id`, if any. Used only by
     *  mempool admission's registration-conflict checks (the .txid lets the caller exempt a tx it
     *  is replacing via RBF). */
    std::optional<MempoolAssetReg> GetMempoolAssetReg(const uint256& asset_id) const EXCLUSIVE_LOCKS_REQUIRED(cs);

    /** {bound asset_id, owner txid} for an unconfirmed mempool tx binding `ticker`, if any. The
     *  owner txid lets admission exempt a tx being replaced via RBF. */
    std::optional<std::pair<uint256, Txid>> GetMempoolTickerBinding(const std::string& ticker) const EXCLUSIVE_LOCKS_REQUIRED(cs);

    /** Evict mempool asset-registration txs that the new chain tip has made unmineable: their asset
     *  is now confirmed-registered and they do not rotate the confirmed ICU, or their ticker is now
     *  confirmed-bound to a different asset. Called on block connect (after removeForBlock) with the
     *  post-block confirmed view, keeping the registration-conflict overlay consistent with the
     *  confirmed registry. `confirmed` must be the active CoinsTip(); caller holds cs_main. */
    void removeStaleAssetRegistrations(const CCoinsView& confirmed) EXCLUSIVE_LOCKS_REQUIRED(cs);
