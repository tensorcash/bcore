 ...
 *
 *  Temporarily lowered from 40000 to 30000 tick/s during the 2026-07 fast-block
 *  recovery. Unlike the slack (which only shifts the intercept), the expectation
 *  sets the floor's SLOPE, so lowering it both widens headroom and slows the
 *  rate the floor rises per block, letting default-configured nodes ride out the
 *  compressed-spacing episode. Honest miners run well above this (blended
 *  ~48-70k/s in the affected window), so it stays a real sequential-time gate.
 *  Restore to 40000 once block spacing normalises above the breakeven cadence. */
static constexpr uint64_t DEFAULT_SPV_MIN_CUMULATIVE_TICK_PER_BLOCK{30000 * 9 * 60};
