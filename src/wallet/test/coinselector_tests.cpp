...
// Test that automatic coin selection prefers non-PQ outputs over PQ outputs
// when both can independently fund the transaction.
BOOST_AUTO_TEST_CASE(pq_avoidance_test)
{
    FastRandomContext rand{};

    // Create coin selection params with effective_feerate=1, long_term_feerate=10
    // This is the regime where waste metric would otherwise prefer the expensive PQ input
    // for consolidation (spending big inputs is cheap now relative to the future).
    CoinSelectionParams cs_params{
        rand,
        /*change_output_size=*/ 31 + 9,  // typical p2wpkh change
        /*change_spend_size=*/ 68,
        /*min_change_target=*/ CENT,
        /*effective_feerate=*/ CFeeRate(1000),  // 1 sat/vB
        /*long_term_feerate=*/ CFeeRate(10000), // 10 sat/vB
        /*discard_feerate=*/ CFeeRate(3000),
        /*tx_noinputs_size=*/ 11 + 31 + 9,  // version + output + locktime
        /*avoid_partial=*/ false,
    };

    // Build a non-PQ coin (10 COIN, ~68 vbyte input) in BECH32M bucket
    CMutableTransaction tx_nonpq;
    tx_nonpq.vout.resize(1);
    tx_nonpq.vout[0].nValue = 10 * COIN;
    tx_nonpq.vout[0].scriptPubKey = CScript() << OP_1 << std::vector<unsigned char>(32, 0xAA); // taproot v1
    tx_nonpq.nLockTime = nextLockTime++;
    COutput out_nonpq(COutPoint(tx_nonpq.GetHash(), 0), tx_nonpq.vout[0], /*depth=*/10,
                      /*input_bytes=*/58, /*spendable=*/true, /*solvable=*/true, /*safe=*/true,
                      /*time=*/0, /*from_me=*/true, cs_params.m_effective_feerate);

    // Build a PQ coin (10 COIN, ~1370 vbyte input) in WITNESS_V2_TAPROOT bucket
    CMutableTransaction tx_pq;
    tx_pq.vout.resize(1);
    tx_pq.vout[0].nValue = 10 * COIN;
    tx_pq.vout[0].scriptPubKey = CScript() << OP_2 << std::vector<unsigned char>(32, 0xBB); // witness v2
    tx_pq.nLockTime = nextLockTime++;
    COutput out_pq(COutPoint(tx_pq.GetHash(), 0), tx_pq.vout[0], /*depth=*/10,
                   /*input_bytes=*/1370, /*spendable=*/true, /*solvable=*/true, /*safe=*/true,
                   /*time=*/0, /*from_me=*/true, cs_params.m_effective_feerate);

    // Build OutputGroupTypeMap with both types
    OutputGroupTypeMap groups;
    {
        OutputGroup grp_nonpq(cs_params);
        grp_nonpq.Insert(std::make_shared<COutput>(out_nonpq), /*ancestors=*/0, /*descendants=*/0);
        groups.Push(grp_nonpq, OutputType::BECH32M, /*insert_positive=*/true, /*insert_mixed=*/true);

        OutputGroup grp_pq(cs_params);
        grp_pq.Insert(std::make_shared<COutput>(out_pq), /*ancestors=*/0, /*descendants=*/0);
        groups.Push(grp_pq, OutputType::WITNESS_V2_TAPROOT, /*insert_positive=*/true, /*insert_mixed=*/true);
    }

    // Target: 1 COIN. Both coins can fund this independently.
    const CAmount target = 1 * COIN;
    auto result = AttemptSelection(*m_node.chain, target, groups, cs_params, /*allow_mixed_output_types=*/false);
    BOOST_CHECK(result);

    // The selected coin should be the non-PQ one (BECH32M), not the PQ one.
    const auto& selected = result->GetInputSet();
    BOOST_CHECK_EQUAL(selected.size(), 1U);
    const auto& chosen = *selected.begin();
    BOOST_CHECK(chosen->outpoint == out_nonpq.outpoint);
}

// Regression test: ML-DSA spend-size estimation must match the crypto layer's
// actual signature sizes. This catches the stale-constant bug where spend.cpp
// used 3293 for ML-DSA-65 while the crypto layer uses 3309.
BOOST_AUTO_TEST_CASE(mldsa_spend_size_estimation_test)
{
    // Build a witness v2 scriptPubKey: OP_2 <32 bytes>
    std::vector<unsigned char> dummy_key(32, 0xCC);
    CScript v2_spk;
    v2_spk << OP_2 << dummy_key;

    CTxOut v2_txout(10 * COIN, v2_spk);

    // Call CalculateMaximumSignedInputSize with no provider (triggers ML-DSA-65 fallback)
    int estimated_vsize = CalculateMaximumSignedInputSize(v2_txout, COutPoint(), /*provider=*/nullptr, /*can_grind_r=*/true, /*coin_control=*/nullptr);
    BOOST_CHECK_GT(estimated_vsize, 0);

    // Manually compute expected vsize using the crypto layer's authoritative sizes.
    // ML-DSA-65: sig=3309, pk=1952 (from mldsa::GetParamSetSizes)
    const int64_t sig_size = 3309;
    const int64_t pk_size = 1952;
    const int64_t MLDSA_SIGNATURE_SIZE = sig_size + 1;  // +1 sighash byte
    const int64_t encoded_pk_size = 1 + 1 + 3 + pk_size;
    const int64_t MLDSA_TAPSCRIPT_SIZE = 1 + 2 + encoded_pk_size + 1 + 1;
    constexpr int64_t MLDSA_CONTROL_BLOCK_SIZE = 33;

    int64_t input_weight = (36 + 4 + 1) * WITNESS_SCALE_FACTOR;
    input_weight += GetSizeOfCompactSize(3);
    input_weight += GetSizeOfCompactSize(MLDSA_SIGNATURE_SIZE) + MLDSA_SIGNATURE_SIZE;
    input_weight += GetSizeOfCompactSize(MLDSA_TAPSCRIPT_SIZE) + MLDSA_TAPSCRIPT_SIZE;
    input_weight += GetSizeOfCompactSize(MLDSA_CONTROL_BLOCK_SIZE) + MLDSA_CONTROL_BLOCK_SIZE;
    int64_t expected_vsize = GetVirtualTransactionSize(input_weight, 0, 0);

    BOOST_CHECK_EQUAL(estimated_vsize, expected_vsize);

    // Also verify that the estimate uses the crypto layer's sizes, not the old stale ones.
    // With the old 3293, the estimate would be 4 vbytes smaller.
    BOOST_CHECK_GT(estimated_vsize, expected_vsize - 1);  // no shortfall allowed
}
