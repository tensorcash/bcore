...
/**
 * Compute the witness weight for spending a witness v2 (ML-DSA) input.
 *
 * Sources pk/sig sizes from mldsa::GetParamSetSizes() so the estimator
 * stays in sync with the crypto layer.
 *
 * @param[in] level_byte  The ML-DSA parameter set byte (44/65/87), or 0 to use the ML-DSA-65 fallback.
 * @return Witness-only weight contribution for this input (does not include prevout/sequence/scriptSig).
 */
static int64_t CalcMLDSAWitnessWeight(uint8_t level_byte)
{
    ...
    switch (level_byte) {
        ...
        default: level = mldsa::ParamSet::MLDSA_65; break; // fallback
    }
    const auto sizes = mldsa::GetParamSetSizes(level);
    const int64_t sig_size = static_cast<int64_t>(sizes.sig_bytes);
    const int64_t pk_size  = static_cast<int64_t>(sizes.pk_bytes);

    const int64_t MLDSA_SIGNATURE_SIZE = sig_size + 1;  // raw sig + sighash byte
    // Tapscript = OP_PUSHDATA2 (1) + length (2) + encoded_pk + OP_CHECKMLSIGVERIFY (1) + OP_TRUE (1)
    // Encoded PK = ALG_ID (1) + level (1) + varint (3) + pk
    const int64_t encoded_pk_size = 1 + 1 + 3 + pk_size;
    const int64_t MLDSA_TAPSCRIPT_SIZE = 1 + 2 + encoded_pk_size + 1 + 1;
    constexpr int64_t MLDSA_CONTROL_BLOCK_SIZE = 33;  // version + internal_key, no merkle proof

    int64_t witness_weight = GetSizeOfCompactSize(3);  // 3 witness elements
    witness_weight += GetSizeOfCompactSize(MLDSA_SIGNATURE_SIZE) + MLDSA_SIGNATURE_SIZE;
    witness_weight += GetSizeOfCompactSize(MLDSA_TAPSCRIPT_SIZE) + MLDSA_TAPSCRIPT_SIZE;
    witness_weight += GetSizeOfCompactSize(MLDSA_CONTROL_BLOCK_SIZE) + MLDSA_CONTROL_BLOCK_SIZE;
    return witness_weight;
}

/**
 * Compute the full input weight for a witness v2 (ML-DSA) input.
 *
 * Input weight = non-witness (prevout + sequence + empty scriptSig) * 4 + witness data.
 */
static int64_t CalcMLDSAInputWeight(uint8_t level_byte)
{
    int64_t input_weight = (36 + 4 + 1) * WITNESS_SCALE_FACTOR;  // prevout + sequence + scriptSig length
    input_weight += CalcMLDSAWitnessWeight(level_byte);
    return input_weight;
}

std::vector<OutputExtensionSnapshot> CollectOutputExtensionSnapshots(const CMutableTransaction& tx)
{
    std::vector<OutputExtensionSnapshot> snapshots;
    snapshots.reserve(tx.vout.size());
    for (const auto& out : tx.vout) {
        if (out.vExt.empty()) continue;
        snapshots.push_back(OutputExtensionSnapshot{out.scriptPubKey, out.nValue, out.vExt});
    }
    return snapshots;
}

bool ReapplyOutputExtensionSnapshots(const std::vector<OutputExtensionSnapshot>& snapshots, CMutableTransaction& tx)
{
    if (snapshots.empty()) return false;
    bool modified{false};
    std::vector<bool> used(tx.vout.size(), false);
    for (const auto& snap : snapshots) {
        ...
        for (size_t i = 0; i < tx.vout.size(); ++i) {
            ...
            if (!out.vExt.empty()) continue;
            if (out.nValue != snap.value) continue;
            if (out.scriptPubKey != snap.script_pub_key) continue;
            out.vExt = snap.vext;
            used[i] = true;
            modified = true;
            matched = true;
        ...
        if (!matched) {
            // No matching output found; continue so multiple TLVs don't interfere.
        }
    }
    return modified;
}

AssetIdScanResult ScanAssetIds(const std::vector<CTxOut>& outputs)
{
    AssetIdScanResult result;
    for (const auto& out : outputs) {
        const auto tag = assets::ParseAssetTag(out.vExt);
        if (!tag) continue;
        if (!result.asset_id) {
            result.asset_id = tag->id;
            continue;
        }
        if (*result.asset_id != tag->id) {
...
/**
 * Check if a CTxOut is an ICU (Issuer Credential UTXO).
 * ICUs are special control UTXOs that must be explicitly selected to prevent
 * accidental consumption as fee inputs. See STABLECOINS_FINALPLAN.md lines 96-97:
 * "any tx that spends the current ICU must recreate exactly one new IssuerReg"
 * Prevents consensus failure: "asset-bond-rotation"
 */
static bool IsIcuOutput(const CTxOut& output) {
    if (output.vExt.empty()) return false;
    // Use proper parsing for robustness - handles multiple TLVs and validates structure
    return static_cast<bool>(assets::ParseIssuerReg(output.vExt));
}
...
{
    // Check if this is a witness v2 (ML-DSA) output: OP_2 <32 bytes>
    if (txout.scriptPubKey.size() == 34 && txout.scriptPubKey[0] == OP_2 && txout.scriptPubKey[1] == 0x20) {
        // Try to determine the ML-DSA security level from tapscript spend data
        uint8_t level_byte = 0;  // 0 means fallback to ML-DSA-65
        if (provider) {
            ...
            if (provider->GetMLDSATaprootSpendData(output_key, spenddata, parity)) {
                if (!spenddata.scripts.empty()) {
                    const auto& [leaf_script_pair, control_blocks] = *spenddata.scripts.begin();
                    const auto& [tapscript_bytes, leaf_ver] = leaf_script_pair;
                    if (tapscript_bytes.size() > 4) {
                        level_byte = tapscript_bytes[4];
                    }
                }
            }
        }
        return static_cast<int>(GetVirtualTransactionSize(CalcMLDSAInputWeight(level_byte), 0, 0));
    }
                                                  ...
                                                  const bool can_grind_r)
{
    ...
    // Check if this is a witness v2 (ML-DSA) input: OP_2 <32 bytes>
    if (txo.scriptPubKey.size() == 34 && txo.scriptPubKey[0] == OP_2 && txo.scriptPubKey[1] == 0x20) {
        // Try to determine the ML-DSA security level from tapscript spend data
        uint8_t level_byte = 0;  // 0 means fallback to ML-DSA-65
        if (wallet) {
            std::vector<unsigned char> output_key_bytes(txo.scriptPubKey.begin() + 2, txo.scriptPubKey.end());
            XOnlyPubKey output_key{output_key_bytes};
            auto provider = wallet->GetSolvingProvider(txo.scriptPubKey);
            if (provider) {
                TaprootSpendData spenddata;
                uint8_t parity;
                if (provider->GetMLDSATaprootSpendData(output_key, spenddata, parity)) {
        ...
        return CalcMLDSAInputWeight(level_byte);
    }
...
OutputType GetOutputType(TxoutType type, bool is_from_p2sh)
{
    switch (type) {
        case TxoutType::WITNESS_V2_TAPROOT:
            return OutputType::WITNESS_V2_TAPROOT;
                           ...
                           const CoinFilterParams& params)
{
    ...
    const bool have_coin_control = coinControl != nullptr;
    const bool avoid_asset_utxos = !have_coin_control || coinControl->m_avoid_asset_utxos;
    const bool allow_icu_selection = have_coin_control && coinControl->m_allow_icu_selection;
    const std::optional<uint256> required_asset_id = have_coin_control ? coinControl->m_required_asset_id : std::optional<uint256>{};
    // Wallet's last-processed block height: the conservative view for evaluating
    // model-deposit maturity in coin selection (never ahead of what the wallet has
    // actually validated). Snapshot once outside the loop. Note this is not literally
    // equal to the balance path's reference (receive.cpp uses wallet.chain().getHeight(),
    // i.e. the chain tip); the two differ only mid-rescan.
    const int model_view_height = wallet.GetLastBlockHeight();
    ...
    for (const auto& entry : wallet.mapWallet)
    {
        ...
        for (unsigned int i = 0; i < wtx.tx->vout.size(); i++) {
            ...
            const bool is_selected = have_coin_control && coinControl->IsSelected(outpoint);

            std::optional<AssetMetadata> asset_meta = wallet.GetAssetMetadata(outpoint);
            bool is_icu_output = asset_meta && asset_meta->is_issuer_credential;
            bool is_asset_output = asset_meta && !asset_meta->is_issuer_credential;
            std::optional<uint256> asset_id = asset_meta ? std::make_optional(asset_meta->asset_id) : std::optional<uint256>{};

            if (!asset_meta && !output.vExt.empty()) {
                if (auto reg = assets::ParseIssuerReg(output.vExt)) {
                    is_icu_output = true;
                    asset_id = reg->asset_id;
                } else if (auto tag = assets::ParseAssetTag(output.vExt)) {
                    is_asset_output = true;
                    asset_id = tag->id;
                }
            }

            if (!is_icu_output && IsIcuOutput(output)) {
                is_icu_output = true;
            }
            ...
            if (is_icu_output) {
                if (!is_selected) {
                    if (required_asset_id) {
                        // Only allow automatic ICU selection when the asset id matches the
                        // transaction's required asset. Prevents unrelated ICUs from being
                        // pulled in for fees, which would violate bond rotation rules.
                        if (!asset_id || asset_id != *required_asset_id) {
                            continue;
                        }
                    }
                    if (!allow_icu_selection) {
                        continue;
                    }
                }
            }

            if (is_asset_output && !is_selected) {
                if (required_asset_id) {
                    if (!asset_id || asset_id != *required_asset_id) {
                        continue;
                    }
                } else if (avoid_asset_utxos) {
                    continue;
                }
            }
            ...
            // Skip model-bonded outputs via the same predicate balance accounting
            // uses (receive.cpp). Parser-first and fail-closed, so a stale or
            // mid-rebuild g_modeldb does not silently make these selectable.
            if (IsModelReservedOutput(outpoint, *wtx.tx, model_view_height)) {
                continue;
            }
...
std::optional<TaprootNativeFundingResult>
SelectTaprootNativeFunding(const CWallet& wallet,
                           const CCoinControl& coin_control,
                           const TaprootNativeFundingParams& params)
{
    AssertLockHeld(wallet.cs_wallet);

    // Gating: this selector is opt-in. A caller passing default CCoinControl
    // gets a nullopt rather than silent Taproot bias, so the preference is
    // always an explicit decision at the call site.
    if (!coin_control.m_prefer_taproot_native) {
        return std::nullopt;
    }

    if (params.target <= 0 || params.max_inputs == 0) {
        return std::nullopt;
    }

    // Refuse zero/negative feerate. The default-constructed CFeeRate is zero,
    // so a lazy caller forgetting to set params.feerate would silently collapse
    // effective_value to raw nValue and turn the dust gating off — the exact
    // failure mode this selector exists to prevent. Force callers to be
    // explicit about the funded tx's feerate.
    if (params.feerate.GetFeePerK() <= 0) {
        return std::nullopt;
    }

    // Inventory: pull through AvailableCoins so IsModelReservedOutput,
    // IsLockedCoin, IsSpent, ICU/asset gating and depth/safety filters are
    // applied uniformly. Local copy of the coin control with asset/ICU
    // avoidance forced on — this selector funds the NATIVE leg and must not
    // pull asset-bearing UTXOs even if the caller forgot to set those flags.
    CCoinControl local_cc = coin_control;
    local_cc.m_avoid_asset_utxos = true;
    local_cc.m_allow_icu_selection = false;
    local_cc.m_required_asset_id.reset();
    local_cc.m_include_unsafe_inputs = false;

    CoinFilterParams filter;
    filter.skip_locked = true;
    filter.only_spendable = true;

    const CoinsResult coins = AvailableCoins(wallet, &local_cc, params.feerate, filter);

    // Take only Taproot-native (BECH32M) and re-check vExt / asset metadata
    // defensively — AvailableCoins already enforces these but the contract
    // path is sensitive enough that we don't want a missing predicate to
    // become a silent funding bug.
    struct Cand
    {
        COutPoint outpoint;
        CAmount value;
    ...
    auto it_tr = coins.coins.find(OutputType::BECH32M);
    if (it_tr == coins.coins.end() || it_tr->second.empty()) {
        return std::nullopt;
    }
    cands.reserve(it_tr->second.size());
    for (const COutput& out : it_tr->second) {
        if (!out.txout.vExt.empty()) continue;
        if (wallet.GetAssetMetadata(out.outpoint)) continue;
        if (out.input_bytes <= 0) continue;  // unsolvable / unknown spend size

        // Effective value as AvailableCoins computed it for `params.feerate`,
        // which already accounts for ancestor bump fees on safe unconfirmed
        // coins. Recomputing locally from nValue - feerate * input_bytes here
        // would overvalue those coins.
        if (!out.HasEffectiveValue()) continue;
        const CAmount eff = out.GetEffectiveValue();
        if (eff < params.min_effective_value) continue;

        cands.push_back({out.outpoint, out.txout.nValue, eff, out.input_bytes});
    }
    if (cands.empty()) {
        return std::nullopt;
    }

    // Pass 1: smallest single candidate whose effective value already covers
    // the target. This is the anti-consolidation pass — if a 1000-sat target
    // can be served by a 1500-sat UTXO, we don't reach for a 5-TSC one.
    std::sort(cands.begin(), cands.end(),
              [](const Cand& a, const Cand& b) {
                  return a.effective_value < b.effective_value;
              });
    for (const Cand& c : cands) {
        if (c.effective_value >= params.target) {
            TaprootNativeFundingResult r;
            r.outpoints = {c.outpoint};
            r.total_value = c.value;
            r.total_effective_value = c.effective_value;
            r.total_input_vsize = c.input_vsize;
            return r;
        }
    }

    // Pass 2: target exceeds every single input — combine. Use minimum
    // cardinality (largest effective values first) up to the input cap.
    // Going ascending here would chain dust into a self-defeating tx; the
    // smallest *count* of inputs that covers is what we want.
    std::sort(cands.begin(), cands.end(),
              [](const Cand& a, const Cand& b) {
                  return a.effective_value > b.effective_value;
              });
    TaprootNativeFundingResult r;
    for (const Cand& c : cands) {
        r.outpoints.push_back(c.outpoint);
        r.total_value += c.value;
        r.total_effective_value += c.effective_value;
        r.total_input_vsize += c.input_vsize;
        if (r.total_effective_value >= params.target) {
            return r;
        }
        if (r.outpoints.size() >= params.max_inputs) {
            break;
        }
    }

    // Couldn't meet target under the constraints. Caller falls back to
    // normal AutomaticCoinSelection (which can mix input types).
    return std::nullopt;
}
                               ...
                               const CoinSelectionParams& coin_selection_params, bool allow_mixed_output_types)
{
    ...
    // Separate non-PQ results from PQ-only results so we can prefer non-PQ.
    std::vector<SelectionResult> non_pq_results;
    std::vector<SelectionResult> pq_results;
    for (auto& [type, group] : groups.groups_by_type) {
        ...
        // Append the favorable result to the appropriate bucket.
        if (result) {
            if (type == OutputType::WITNESS_V2_TAPROOT) {
                pq_results.push_back(*result);
            } else {
                non_pq_results.push_back(*result);
            }
        }
    }

    // Prefer non-PQ results when available.
    if (!non_pq_results.empty()) return *std::min_element(non_pq_results.begin(), non_pq_results.end());
    // Fall back to PQ-only results if no non-PQ solution exists.
    if (!pq_results.empty()) return *std::min_element(pq_results.begin(), pq_results.end());

    // If we can't fund the transaction from any individual OutputType, run coin selection one last time
    // over all available coins, which would allow mixing.
    // If TypesCount() <= 1, there is nothing to mix.
    if (allow_mixed_output_types && groups.TypesCount() > 1) {
        // First try mixing only non-PQ groups.
        Groups non_pq_all;
        for (const auto& [type, grp] : groups.groups_by_type) {
            if (type == OutputType::WITNESS_V2_TAPROOT) continue;
            non_pq_all.positive_group.insert(non_pq_all.positive_group.end(), grp.positive_group.begin(), grp.positive_group.end());
            non_pq_all.mixed_group.insert(non_pq_all.mixed_group.end(), grp.mixed_group.begin(), grp.mixed_group.end());
        }
        if (!non_pq_all.positive_group.empty() || !non_pq_all.mixed_group.empty()) {
            auto non_pq_res{ChooseSelectionResult(chain, nTargetValue, non_pq_all, coin_selection_params)};
            if (non_pq_res) return non_pq_res;
            // If the non-PQ attempt returned a detailed error (e.g. max-weight),
            // propagate it rather than masking it with a later generic failure.
            if (HasErrorMsg(non_pq_res)) return non_pq_res;
        }
        // Non-PQ mixed selection failed without a specific error; fall back to all groups including PQ.
        ...
        bool sign) EXCLUSIVE_LOCKS_REQUIRED(wallet.cs_wallet)
{
    ...
    if (const auto* no_dest = std::get_if<CNoDestination>(&coin_control.destChange)) {
        if (!no_dest->GetScript().empty()) {
            scriptChange = no_dest->GetScript();
    ...
    if (!select_coins_res) {
        ...
        if (err.empty()) {
            if (coin_control.m_required_asset_id) {
                return util::Error{strprintf(_("Insufficient funds for asset %s"), coin_control.m_required_asset_id->GetHex())};
            }
        }
