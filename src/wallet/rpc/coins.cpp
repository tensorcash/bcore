        ...
        RPCResult{
            RPCResult::Type::OBJ, "", "",
            {
                {RPCResult::Type::OBJ, "mine", "balances from outputs that the wallet can sign",
                {
                    ...
                    {RPCResult::Type::STR_AMOUNT, "bonded", "balance bonded in issuer credential UTXOs"},
                    {RPCResult::Type::STR_AMOUNT, "used", /*optional=*/true, "(only present if avoid_reuse is set) balance from coins sent to addresses that were previously spent from (potentially privacy violating)"},
                }},
                {RPCResult::Type::OBJ, "watchonly", /*optional=*/true, "balances from watch-only outputs (only present if the wallet tracks watch-only scripts)",
                {
                    {RPCResult::Type::STR_AMOUNT, "trusted", "trusted watch-only balance"},
                    {RPCResult::Type::STR_AMOUNT, "untrusted_pending", "untrusted pending watch-only balance"},
                    {RPCResult::Type::STR_AMOUNT, "immature", "immature watch-only coinbase balance"},
                    {RPCResult::Type::STR_AMOUNT, "bonded", "watch-only balance bonded in issuer credential UTXOs"},
                }},
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    UniValue balances{UniValue::VOBJ};
    {
        ...
        balances_mine.pushKV("bonded", ValueFromAmount(bal.m_mine_bonded));
    ...
    const bool has_watchonly = bal.m_watchonly_trusted != 0 ||
        bal.m_watchonly_untrusted_pending != 0 ||
        bal.m_watchonly_immature != 0 ||
        bal.m_watchonly_bonded != 0;
    ...
    if (has_watchonly) {
        ...
        balances_watchonly.pushKV("bonded", ValueFromAmount(bal.m_watchonly_bonded));
