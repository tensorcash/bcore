...
std::optional<uint256> RequireSingleAssetId(const std::vector<CTxOut>& outputs, const char* rpc_name)
{
    AssetIdScanResult scan = ScanAssetIds(outputs);
    if (scan.conflict) {
        throw JSONRPCError(RPC_INVALID_PARAMETER, strprintf("%s does not yet support multiple asset ids in one call", rpc_name));
    }
    return scan.asset_id;
}

bool TxOutputsContainIssuerReg(const std::vector<CTxOut>& outputs)
{
    for (const auto& out : outputs) {
        if (out.vExt.empty()) {
            continue;
        }
        if (assets::ParseIssuerReg(out.vExt)) {
            return true;
        }
    }
    return false;
...
static std::optional<ZkProprietaryKeyInfo> DecodeZkProprietaryKey(const PSBTProprietary& entry)
{
    SpanReader skey{std::span<const unsigned char>(entry.key)};
    ZkProprietaryKeyInfo info;
    try {
...
static void ApplyZkWitness(PartiallySignedTransaction& psbt)
{
    for (size_t i = 0; i < psbt.inputs.size(); ++i) {
        ...
        if (!psbt.inputs[i].final_script_witness.IsNull()) {
            witness = psbt.inputs[i].final_script_witness;
        } else if (psbt.tx && i < psbt.tx->vin.size()) {
            witness = psbt.tx->vin[i].scriptWitness;
        }

        if (witness.stack.size() >= 2) {
            const auto& maybe_proof = witness.stack[witness.stack.size() - 2];
            const auto& maybe_pub = witness.stack[witness.stack.size() - 1];
            if (maybe_proof == *proof && maybe_pub == *pub) {
                psbt.inputs[i].final_script_witness = witness;
                if (psbt.tx && i < psbt.tx->vin.size()) {
                    psbt.tx->vin[i].scriptWitness = witness;
                }
                continue;
            }
        }
...
static std::optional<std::string> DecodeFsSuffix(const PSBTProprietary& entry)
{
    SpanReader skey{std::span<const unsigned char>(entry.key)};
    try {
        const uint64_t type = ReadCompactSize(skey);
        if (type != PSBT_GLOBAL_PROPRIETARY) return std::nullopt;
        if (skey.size() < entry.identifier.size()) return std::nullopt;
        for (size_t i = 0; i < entry.identifier.size(); ++i) {
            uint8_t ch{0};
            skey >> ch;
            if (ch != entry.identifier[i]) return std::nullopt;
        }
        const uint64_t subtype = ReadCompactSize(skey);
        if (subtype != entry.subtype) return std::nullopt;
        std::string suffix;
        suffix.reserve(skey.size());
        while (!skey.empty()) {
            uint8_t ch{0};
            skey >> ch;
...
static std::optional<uint256> ExtractForwardContractMeta(const PartiallySignedTransaction& psbt)
{
    const auto identifier = wallet::fairsign::Identifier();
    for (const auto& entry : psbt.m_proprietary) {
        if (entry.identifier != identifier || entry.subtype != 0) continue;
        auto suffix = DecodeFsSuffix(entry);
        if (!suffix || *suffix != "contract_meta") continue;
        if (entry.value.size() != sizeof(uint256)) {
            return std::nullopt;
        }
        return uint256(std::span<const unsigned char>(entry.value));
    }
    return std::nullopt;
}
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    const std::vector<OutputExtensionSnapshot> snapshots = CollectOutputExtensionSnapshots(tx);
    ...
    CCoinControl coin_control;
    const std::optional<uint256> required_asset_id = RequireSingleAssetId(tx.vout, "fundrawtransaction");
    const bool has_issuer_reg_output = TxOutputsContainIssuerReg(tx.vout);
    if (required_asset_id) {
        coin_control.m_required_asset_id = required_asset_id;
        if (has_issuer_reg_output) {
            // Asset transactions that rotate the ICU may auto-select it; others must opt in explicitly.
    ...
    // Reattach preserved vExt TLVs by matching (scriptPubKey, value) pairs.
    CTransactionRef final_tx = txr.tx;
    if (!snapshots.empty()) {
        CMutableTransaction mtx(*txr.tx);
        if (ReapplyOutputExtensionSnapshots(snapshots, mtx)) {
            final_tx = MakeTransactionRef(std::move(mtx));
        }
    }
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    // Fallback: populate missing wallet coins (e.g. witness v2 ML-DSA outputs) from the wallet database
    for (auto& [outpoint, coin] : coins) {
        if (!coin.IsSpent()) {
            continue;
        }
        auto it = pwallet->mapWallet.find(outpoint.hash);
        ...
        coin.out = wtx.tx->vout[outpoint.n];
        coin.nHeight = MEMPOOL_HEIGHT;
        coin.fCoinBase = wtx.IsCoinBase();
    }

    int nHashType = ParseSighashString(request.params[2]);

    // Detect witness v2 (ML-DSA) inputs which require PSBT-based signing so the ML-DSA
    // key handling path is used.
    std::set<int> witness_v2_inputs;
    for (unsigned int i = 0; i < mtx.vin.size(); ++i) {
        const auto it = coins.find(mtx.vin[i].prevout);
        if (it == coins.end() || it->second.IsSpent()) continue;
        int witness_version;
        std::vector<unsigned char> witness_program;
        if (it->second.out.scriptPubKey.IsWitnessProgram(witness_version, witness_program) &&
            witness_version == 2 && witness_program.size() == 32) {
            witness_v2_inputs.insert(i);
        }
    }

    if (!witness_v2_inputs.empty()) {
        PartiallySignedTransaction psbt(mtx);
        bool missing_utxo = false;
        for (unsigned int i = 0; i < mtx.vin.size(); ++i) {
            const auto it = coins.find(mtx.vin[i].prevout);
            if (it != coins.end() && !it->second.IsSpent()) {
                psbt.inputs[i].witness_utxo = it->second.out;
            } else if (witness_v2_inputs.count(i)) {
                missing_utxo = true;
            }
        }
        ...
        if (missing_utxo) {
            ...
            for (int idx : witness_v2_inputs) {
                input_errors.emplace(idx, Untranslated("Missing ML-DSA witness UTXO"));
            }
            UniValue result(UniValue::VOBJ);
            SignTransactionResultToJSON(mtx, /*complete=*/false, coins, input_errors, result);
            return result;
        }

        bool psbt_complete{false};
        const auto psbt_err = pwallet->FillPSBT(psbt, psbt_complete, nHashType, /*sign=*/true, /*bip32derivs=*/false, /*n_signed=*/nullptr, /*finalize=*/true);
        if (psbt_err) {
            throw JSONRPCError(RPC_WALLET_ERROR, common::PSBTErrorString(*psbt_err).translated);
        }

        CMutableTransaction extracted;
        const bool extracted_ok = FinalizeAndExtractPSBT(psbt, extracted);
        if (extracted_ok) {
            mtx = std::move(extracted);
        } else {
            mtx = CMutableTransaction(*psbt.tx);
        }
        psbt_complete = psbt_complete && extracted_ok;

        std::map<int, bilingual_str> psbt_errors;
        if (!psbt_complete) {
            for (int idx : witness_v2_inputs) {
                psbt_errors.emplace(idx, Untranslated("Unable to sign ML-DSA input"));
            }
        }

        UniValue result(UniValue::VOBJ);
        SignTransactionResultToJSON(mtx, psbt_complete, coins, psbt_errors, result);
        return result;
    }
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    // Allow multi-asset transactions for forward contracts and atomic swaps
    // Check for asset consistency but allow multiple assets if explicitly needed
    std::optional<uint256> outputs_asset_id;
    [[maybe_unused]] bool has_multiple_assets = false;
    if (psbtx.tx) {
        AssetIdScanResult output_scan = ScanAssetIds(psbtx.tx->vout);
        if (output_scan.conflict) {
            has_multiple_assets = true;
        }
        outputs_asset_id = output_scan.asset_id;
    }

    std::vector<CTxOut> input_utxos;
    if (psbtx.tx) {
        input_utxos.reserve(psbtx.tx->vin.size());
        for (size_t i = 0; i < psbtx.inputs.size() && i < psbtx.tx->vin.size(); ++i) {
            const PSBTInput& input = psbtx.inputs[i];
            if (!input.witness_utxo.IsNull()) {
                input_utxos.push_back(input.witness_utxo);
                continue;
            }
            if (input.non_witness_utxo) {
                const auto index = psbtx.tx->vin[i].prevout.n;
                if (index < input.non_witness_utxo->vout.size()) {
                    input_utxos.push_back(input.non_witness_utxo->vout[index]);
                }
            }
        }
    }
    AssetIdScanResult input_scan = ScanAssetIds(input_utxos);
    if (input_scan.conflict) {
        has_multiple_assets = true;
    }

    // For multi-asset transactions, we'll sign each input independently
    // This is safe because signing is already done per-input in FillPSBT
    ...
    if (finalize) {
        ApplyZkWitness(psbtx);
    }

    if (auto forward_meta = ExtractForwardContractMeta(psbtx)) {
        const auto forward_records = wallet.ListForwardContracts();
        for (const auto& record : forward_records) {
            if (ComputeForwardContractMeta(record) != *forward_meta) continue;
            if (record.long_margin_vault && record.short_margin_vault) break;
            pwallet->VerifyAndPersistForwardVaults(record.contract_id, psbtx, /*allow_open_discovery=*/true);
            break;
        }
    }
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    const std::vector<OutputExtensionSnapshot> snapshots = CollectOutputExtensionSnapshots(rawTx);
    const std::optional<uint256> required_asset_id = RequireSingleAssetId(rawTx.vout, "walletcreatefundedpsbt");
    const bool has_issuer_reg_output = TxOutputsContainIssuerReg(rawTx.vout);
    CCoinControl coin_control;
    // Automatically select coins, unless at least one is manually selected. Can
    // be overridden by options.add_inputs.
    coin_control.m_allow_other_inputs = rawTx.vin.size() == 0;
    ...
    if (!snapshots.empty()) {
        CMutableTransaction mtx(*txr.tx);
        if (ReapplyOutputExtensionSnapshots(snapshots, mtx)) {
            txr.tx = MakeTransactionRef(std::move(mtx));
        }
    }
