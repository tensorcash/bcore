...
public:
    ...
    /** Optional hook for registering covenant scripts associated with an existing descriptor output. */
    virtual bool RegisterCovenantScript(const CScript& base_script, const CScript& derived_script) { return false; }
...
// Information about the script owner for deterministic key resolution
struct SpkOwner {
    uint256 desc_id;                // Descriptor ID
    int32_t index;                  // Child derivation index
    XOnlyPubKey internal_xonly;     // Descriptor's internal pubkey at index
    bool is_watch_only;             // True if wallet is watch-only
    bool has_priv_at_index;         // True if child private key is present
};
...
private:
    ...
    //! Vault registry for covenant scripts
    VaultRegistry m_vault_registry GUARDED_BY(cs_desc_man);

    bool AddDescriptorKeyWithDB(WalletBatch& batch, const CKey& key, const CPubKey &pubkey) EXCLUSIVE_LOCKS_REQUIRED(cs_desc_man);

    KeyMap GetKeys() const EXCLUSIVE_LOCKS_REQUIRED(cs_desc_man);
    bool GetKeyByXOnlyLocked(const XOnlyPubKey& xonly, CKey& key_out) const EXCLUSIVE_LOCKS_REQUIRED(cs_desc_man);
    ...
    // Fetch the SigningProvider for the given script and optionally include private keys. When a tap_scripts_filter is supplied,
    // restrict the injected taproot spend data to the specified leaves.
    std::unique_ptr<FlatSigningProvider> GetSigningProvider(const CScript& script, bool include_private = false, const std::set<std::pair<std::vector<unsigned char>, int>>* tap_scripts_filter = nullptr) const;
...
public:
    ...
    /** Fetch a signing provider for the given scriptPubKey. When include_private=true and the
     *  wallet is unlocked, the provider will contain private keys needed to sign/spend. */
    std::unique_ptr<FlatSigningProvider> GetSolvingProviderForScript(const CScript& script, bool include_private) const;
    ...
    /** Attach an additional covenant script to the same index as an existing descriptor output. */
    bool RegisterCovenantScript(const CScript& base_script, const CScript& derived_script) override LOCKS_EXCLUDED(cs_desc_man);

    /** Register a covenant vault with complete taproot metadata */
    bool RegisterCovenantVault(const CScript& base_script, const VaultMetadata& metadata) LOCKS_EXCLUDED(cs_desc_man);

    /** Load vault from database (used during wallet load, skips m_map_script_pub_keys mapping) */
    bool LoadVaultFromDB(const VaultMetadata& metadata) LOCKS_EXCLUDED(cs_desc_man);

    /** Retrieve vault metadata for a covenant script */
    std::optional<VaultMetadata> GetVaultMetadata(const CScript& covenant_spk) const LOCKS_EXCLUDED(cs_desc_man);

    /** Check if a script is a registered covenant vault */
    bool IsCovenantVault(const CScript& script) const LOCKS_EXCLUDED(cs_desc_man);

    /** Get private key by x-only pubkey (tries both parity versions) */
    bool GetKeyByXOnly(const XOnlyPubKey& xonly, CKey& key_out) const LOCKS_EXCLUDED(cs_desc_man);

    /** Cache a private key in the vault registry for signing */
    bool CacheVaultKey(const XOnlyPubKey& xonly, const CKey& key) const LOCKS_EXCLUDED(cs_desc_man);
    ...
    /** Resolve the exact descriptor + index that owns a given scriptPubKey.
     *  Returns std::nullopt if this descriptor SPKM does not own the script. */
    std::optional<SpkOwner> GetDescriptorOwnerForScript(const CScript& spk) const;

    /** Ensure the private key exists at the given index for this descriptor.
     *  If the wallet has secrets and can derive, this will TopUp to include that index.
     *  Returns true if the private key exists after this call. */
    bool EnsurePrivKeyAtIndex(int32_t index);

    /** Derive the private key at a specific index for a given internal x-only pubkey.
     *  This explicitly derives child keys at the specified index and returns the key
     *  whose x-only pubkey matches the provided internal_xonly.
     *  Returns std::nullopt if derivation fails or key is not found. */
    std::optional<CKey> DerivePrivateKeyAtIndex(int32_t index, const XOnlyPubKey& internal_xonly);
};
