...
/**
 * True if `outpoint` (created by `creating_tx`) is a model-bonded output that
 * must not be auto-selected for ordinary funding: a still-locked model deposit
 * or challenge deposit, or a burn-indexed outpoint. Parser-first and
 * fail-closed: an output that parses as a deposit/challenge output is treated
 * as reserved unless g_modeldb positively confirms it is spendable, so a
 * missing or mid-rebuild g_modeldb errs toward NOT spending. Shared by balance
 * accounting (GetBalance) and coin selection (AvailableCoins) so both use the
 * same reservation policy. Mirrors the deposit/challenge/burn checks in
 * validation.cpp.
 */
...
struct Balance {
    ...
    CAmount m_mine_bonded{0};            //!< Bonded in ICU + contract vaults (repo collateral, forward IM)
    CAmount m_watchonly_trusted{0};
    CAmount m_watchonly_untrusted_pending{0};
    CAmount m_watchonly_immature{0};
    CAmount m_watchonly_bonded{0};       //!< Watch-only bonded in ICU + contract vaults
};
