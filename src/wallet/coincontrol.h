...
public:
    ...
    //! Skip asset-bearing UTXOs during automatic selection (default true)
    bool m_avoid_asset_utxos = true;
    //! Permit automatic selection of ICU (Issuer Credential UTXO) outputs
    bool m_allow_icu_selection = false;
    //! Hint for contract/ceremony funding paths: try Taproot-native UTXOs
    //! first for native funding legs (smaller witness, modern type, avoids
    //! mixed-input ceremony degradation). Default false — the regular
    //! Bitcoin Core BnB/Knapsack selection is unaffected. Contract builders
    //! that opt in should call SelectTaprootNativeFunding() and fall back
    //! to normal selection if that returns nullopt; this is a *preference*,
    //! not a hard constraint.
    bool m_prefer_taproot_native = false;
    //! Limit automatic asset selection to the specified asset identifier
    std::optional<uint256> m_required_asset_id;
