...
bool OutputHasAssetTag(const CTxOut& txout)
{
    return !txout.vExt.empty() && assets::ParseAssetTag(txout.vExt).has_value();
}

bool HasAssetOutputs(const std::vector<CTxOut>& txouts)
{
    for (const auto& out : txouts) {
        if (OutputHasAssetTag(out)) return true;
    }
    return false;
}

feebumper::Result CreateAssetAwareBumpTransaction(CWallet& wallet,
                                                  const CWalletTx& wtx,
                                                  ...
                                                  std::vector<bilingual_str>& errors) EXCLUSIVE_LOCKS_REQUIRED(wallet.cs_wallet)
{
    if (!coin_control.m_feerate) {
        errors.emplace_back(Untranslated("Internal error: missing asset bump feerate"));
        return feebumper::Result::MISC_ERROR;
    }

    CFeeRate target_feerate = *coin_control.m_feerate;

    CMutableTransaction bumped_tx{*wtx.tx};
    for (auto& txin : bumped_tx.vin) {
        txin.scriptSig.clear();
        txin.scriptWitness.SetNull();
    }

    const TxSize tx_sizes = CalculateMaximumSignedTxSize(CTransaction(bumped_tx), &wallet, &coin_control);
    if (tx_sizes.vsize <= 0) {
        errors.emplace_back(Untranslated("Unable to estimate transaction size for asset RBF"));
        return feebumper::Result::WALLET_ERROR;
    }
    const int64_t max_tx_size = tx_sizes.vsize;

    std::vector<COutPoint> reused_inputs;
    reused_inputs.reserve(bumped_tx.vin.size());
    for (const auto& txin : bumped_tx.vin) {
        reused_inputs.push_back(txin.prevout);
    }

    std::optional<CAmount> combined_bump_fee = wallet.chain().calculateCombinedBumpFee(reused_inputs, target_feerate);
    if (!combined_bump_fee.has_value()) {
        errors.emplace_back(Untranslated("Failed to calculate combined bump fee for asset transaction"));
        return feebumper::Result::WALLET_ERROR;
    }

    CAmount new_total_fee = target_feerate.GetFee(max_tx_size) + combined_bump_fee.value();

    // Enforce standard relay constraints identical to CheckFeeRate.
    CFeeRate incremental_relay_fee = wallet.chain().relayIncrementalFee();
    const CAmount min_total_fee = old_fee + incremental_relay_fee.GetFee(max_tx_size);
    if (new_total_fee < min_total_fee) {
        new_total_fee = min_total_fee;
    }

    const CAmount required_fee = GetRequiredFee(wallet, max_tx_size);
    if (new_total_fee < required_fee) {
        new_total_fee = required_fee;
    }

    if (new_total_fee <= old_fee) {
        new_total_fee = old_fee + std::max<CAmount>(incremental_relay_fee.GetFee(max_tx_size), 1);
    }

    if (new_total_fee > wallet.m_default_max_tx_fee) {
        errors.emplace_back(Untranslated(strprintf("Specified or calculated fee %s is too high (cannot be higher than -maxtxfee %s)",
            FormatMoney(new_total_fee), FormatMoney(wallet.m_default_max_tx_fee))));
        return feebumper::Result::WALLET_ERROR;
    }

    CAmount delta_fee = new_total_fee - old_fee;
    if (delta_fee <= 0) {
        errors.emplace_back(Untranslated("New fee is not greater than old fee for asset transaction"));
        return feebumper::Result::INVALID_PARAMETER;
    }

    const CFeeRate dust_relay_fee = wallet.chain().relayDustFee();
    CAmount remaining = delta_fee;

    for (auto& out : bumped_tx.vout) {
        if (OutputHasAssetTag(out)) {
            continue; // Never modify asset-carrying outputs.
        }
        ...
        if (out.nValue <= dust_threshold) {
            continue;
        }

        const CAmount max_deduct = out.nValue - dust_threshold;
        if (max_deduct <= 0) {
            continue;
        }

        const CAmount deduct = std::min(max_deduct, remaining);
        out.nValue -= deduct;
        remaining -= deduct;
    ...
    if (remaining > 0) {
        errors.emplace_back(Untranslated("Insufficient wallet-owned BTC change to bump asset transaction fee"));
        return feebumper::Result::WALLET_ERROR;
    }

    CAmount new_output_value{0};
    for (const auto& out : bumped_tx.vout) {
        new_output_value += out.nValue;
    }

    new_fee = input_value - new_output_value;
    if (new_fee <= old_fee) {
        errors.emplace_back(Untranslated("Failed to increase fee for asset transaction"));
        return feebumper::Result::MISC_ERROR;
    }

    if (new_fee < new_total_fee) {
        // Should not happen, but guard to avoid violating policy.
        new_fee = new_total_fee;
    }

    mtx = std::move(bumped_tx);
                                 ...
                                 CAmount& old_fee, CAmount& new_fee, CMutableTransaction& mtx, bool require_mine, const std::vector<CTxOut>& outputs, std::optional<uint32_t> original_change_index)
{
    ...
    const bool has_asset_outputs = HasAssetOutputs(txouts);
    ...
    if (has_asset_outputs && outputs.empty()) {
        CMutableTransaction bumped;
        Result asset_res = CreateAssetAwareBumpTransaction(wallet, wtx, new_coin_control, input_value, old_fee, new_fee, bumped, errors);
        if (asset_res != Result::OK) {
            return asset_res;
        }
        mtx = std::move(bumped);
        return Result::OK;
