...
static std::atomic<uint32_t> g_ecc_refcount{0};
...
std::array<unsigned char, 32> CKey::ComputeECDHSecret(const CPubKey& pubkey) const
{
    assert(keydata);
    ...
    if (!secp256k1_ec_pubkey_parse(secp256k1_context_sign, &parsed_pubkey, pubkey.data(), pubkey.size())) {
        return {};
    }

    std::array<unsigned char, 32> output{};
    if (!secp256k1_ecdh(secp256k1_context_sign, output.data(), &parsed_pubkey, UCharCast(begin()), nullptr, nullptr)) {
        return {};
    }
...
bool CKey::TweakAdd(const uint256& tweak)
{
    assert(keydata);
    if (secp256k1_ec_seckey_tweak_add(secp256k1_context_sign, keydata->data(), tweak.begin()) != 1) {
        ClearKeyData();
        return false;
    }
    return true;
...
bool CKey::Negate()
{
    assert(keydata);
    secp256k1_pubkey pubkey;
    if (!secp256k1_ec_pubkey_create(secp256k1_context_sign, &pubkey, keydata->data())) {
        ClearKeyData();
        return false;
    }
    secp256k1_xonly_pubkey xonly;
    int parity = 0;
    if (!secp256k1_xonly_pubkey_from_pubkey(secp256k1_context_sign, &xonly, &parity, &pubkey)) {
        ClearKeyData();
        return false;
    }
    if (parity == 1) {
        if (!secp256k1_ec_seckey_negate(secp256k1_context_sign, keydata->data())) {
            ClearKeyData();
            return false;
        }
    }
...
static void ECC_Start() {
    const uint32_t previous = g_ecc_refcount.fetch_add(1, std::memory_order_acq_rel);
    if (previous > 0) {
        return;
    }
...
static void ECC_Stop() {
    const uint32_t previous = g_ecc_refcount.fetch_sub(1, std::memory_order_acq_rel);
    assert(previous > 0);
    if (previous > 1) {
        return;
    }
