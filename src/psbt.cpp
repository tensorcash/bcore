...
bool PSBTInput::IsNull() const
{
    return !non_witness_utxo && witness_utxo.IsNull() && partial_sigs.empty() && unknown.empty() && hd_keypaths.empty() && redeem_script.empty() && witness_script.empty() &&
           m_mldsa_pubkey.empty() && m_mldsa_signature.empty() && !m_mldsa_param_set.has_value() && !m_v2_tap_parity.has_value();
}
...
void PSBTInput::Merge(const PSBTInput& input)
{
    ...
    m_proprietary.insert(input.m_proprietary.begin(), input.m_proprietary.end());  // Preserve proprietary fields (e.g. vault intent)
    ...
    // Merge ML-DSA fields
    if (m_mldsa_pubkey.empty() && !input.m_mldsa_pubkey.empty()) m_mldsa_pubkey = input.m_mldsa_pubkey;
    if (m_mldsa_signature.empty() && !input.m_mldsa_signature.empty()) m_mldsa_signature = input.m_mldsa_signature;
    if (!m_mldsa_param_set.has_value() && input.m_mldsa_param_set.has_value()) m_mldsa_param_set = input.m_mldsa_param_set;
    if (!m_v2_tap_parity.has_value() && input.m_v2_tap_parity.has_value()) m_v2_tap_parity = input.m_v2_tap_parity;
}
...
void PSBTOutput::Merge(const PSBTOutput& output)
{
    ...
    m_proprietary.insert(output.m_proprietary.begin(), output.m_proprietary.end());  // Preserve proprietary fields
...
bool SignPSBTInput(const SigningProvider& provider, PartiallySignedTransaction& psbt, int index, const PrecomputedTransactionData* txdata, int sighash,  SignatureData* out_sigdata, bool finalize)
{
    ...
    if (node::IsMLDSAInput(psbt, index)) {
        // First, update the input with ML-DSA data if not already present
        if (input.m_mldsa_pubkey.empty()) {
            node::UpdatePSBTInputMLDSA(provider, psbt, index);
        }

        // Sign the input with ML-DSA
        bool signed_ok = node::SignPSBTInputMLDSA(provider, psbt, index, txdata, sighash);

        // If finalize is requested and we successfully signed, finalize it
        if (finalize && signed_ok && !input.m_mldsa_signature.empty()) {
            return node::FinalizePSBTInputMLDSA(input, sighash);
        }
