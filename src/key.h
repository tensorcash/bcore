...
public:
    ...
    /** Compute a standard ECDH shared secret (SHA256 of the shared point's x-coordinate).
     *
     *  - pubkey: The counterparty's public key (33 bytes compressed format).
     *  Returns 32-byte shared secret, or empty array on failure.
     */
    std::array<unsigned char, 32> ComputeECDHSecret(const CPubKey& pubkey) const;
    /** Add a tweak (mod n) to this private key. Returns false and invalidates the key on failure. */
    bool TweakAdd(const uint256& tweak);
    /** Negate the private key (k -> -k mod n). Returns false if key invalid and clears it. */
    bool Negate();
    /** Compute a KeyPair
     *
