...
CBlockHeaderAndShortTxIDs::CBlockHeaderAndShortTxIDs(const CBlock& block, const uint64_t nonce) :
        nonce(nonce),
        shorttxids(block.vtx.size() - 1), prefilledtxn(1), header(block), pow(block.pow), cumulative_tick(block.cumulative_tick) {
...
ReadStatus PartiallyDownloadedBlock::InitData(const CBlockHeaderAndShortTxIDs& cmpctblock, const std::vector<CTransactionRef>& extra_txn) {
    ...
    m_pow = cmpctblock.pow;
    m_cumulative_tick = cmpctblock.cumulative_tick;
    m_has_tensor = true;
...
ReadStatus PartiallyDownloadedBlock::FillBlock(CBlock& block, const std::vector<CTransactionRef>& vtx_missing)
{
    ...
    const bool had_tensor = m_has_tensor;
    const CProofBlob pow_copy = m_pow;
    const uint64_t cumulative_tick_copy = m_cumulative_tick;
    ...
    if (had_tensor) {
        block.pow = pow_copy;
        block.cumulative_tick = cumulative_tick_copy;
    }
    if (!check_block(block, state, Params().GetConsensus(), /*fCheckApi=*/false, /*fCheckPoW=*/true, /*fCheckMerkleRoot=*/true)) {
