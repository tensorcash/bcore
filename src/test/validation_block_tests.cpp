...
struct MinerTestingSetup : public RegTestingSetup {
        ...
        : RegTestingSetup()
    {
        ...
        m_cumulative_ticks.emplace(genesis.GetHash(), genesis.cumulative_tick);
    }

    std::map<uint256, uint64_t> m_cumulative_ticks;
};
} // namespace validation_block_tests
...
std::shared_ptr<CBlock> MinerTestingSetup::FinalizeBlock(std::shared_ptr<CBlock> pblock)
{
    ...
    m_node.chainman->GenerateCoinbaseCommitment(*pblock, prev_index);
    ...
    if (auto it = m_cumulative_ticks.find(pblock->hashPrevBlock); it != m_cumulative_ticks.end()) {
        prev_cumulative_tick = it->second;
    }
    UpdateTestBlockVdf(*pblock, prev_cumulative_tick, next_height, consensus);

    while (!CheckProofOfWork(pblock->GetShortHash(), pblock->nAdjBits ? pblock->nAdjBits : pblock->nBits, Params().GetConsensus())) {
        ++(pblock->nNonce);
    }
    ...
    m_cumulative_ticks[pblock->GetHash()] = pblock->cumulative_tick;
