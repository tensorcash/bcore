...
BOOST_AUTO_TEST_CASE(headers_message_writes_zero_txcount)
{
    CBlockHeader header;
    header.nVersion = 2;
    header.nTime = 0x5f5e100;
    header.nBits = 0x1d00ffff;
    header.nAdjBits = header.nBits;
    header.nNonce = 0x12345678;
    header.flags = 0x5a;

    HeadersMessage message({header});

    DataStream serialized;
    serialized << message;

    const size_t expected_header_size = ::GetSerializeSize(header);
    BOOST_CHECK_EQUAL(serialized.size(), 1 + expected_header_size + 1);

    std::vector<unsigned char> bytes;
    bytes.reserve(serialized.size());
    for (std::byte b : serialized) {
        bytes.push_back(std::to_integer<unsigned char>(b));
    }
    BOOST_REQUIRE(!bytes.empty());
    BOOST_CHECK_EQUAL(bytes.front(), 1); // CompactSize count
    BOOST_CHECK_EQUAL(bytes.back(), 0);  // Trailing txcount

    HeadersMessage roundtrip;
    DataStream deserialized{std::span<const unsigned char>(bytes)};
    deserialized >> roundtrip;

    BOOST_REQUIRE_EQUAL(roundtrip.headers.size(), 1U);
    const CBlockHeader& decoded = roundtrip.headers[0];
    ...
    BOOST_CHECK_EQUAL(decoded.nAdjBits, header.nAdjBits);
    BOOST_CHECK_EQUAL(decoded.nNonce, header.nNonce);
    BOOST_CHECK(decoded.hashPoW == header.hashPoW);
    BOOST_CHECK_EQUAL(decoded.flags, header.flags);
}
