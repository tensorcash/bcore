...
struct BuildChainTestingSetup : public TestChain100Setup {
    // Cache synthetic blocks by hash so we can recover VDF metadata for parents
    std::map<uint256, CBlock> m_cached_blocks;
    ...
    const CScript& scriptPubKey)
{
    ...
    const int next_height = prev->nHeight + 1;

    // Update VDF proof to match the synthetic parent and ensure tick is non-zero when required
    ...
    // Derive cumulative tick from the parent block (cached synthetic chain or disk)
    uint64_t prev_cumulative_tick = 0;
    bool have_prev_tick = false;
    if (auto it = m_cached_blocks.find(block.hashPrevBlock); it != m_cached_blocks.end()) {
        prev_cumulative_tick = it->second.cumulative_tick;
        have_prev_tick = true;
    } else {
        CBlock prev_block;
        if (m_node.chainman->m_blockman.ReadBlock(prev_block, *prev)) {
    ...
    while (!CheckProofOfWork(block.GetShortHash(), block.nAdjBits ? block.nAdjBits : block.nBits, m_node.chainman->GetConsensus())) ++block.nNonce;
    ...
    std::vector<std::shared_ptr<CBlock>>& chain)
{
    ...
    for (auto& block : chain) {
        ...
        m_cached_blocks.emplace(block->GetHash(), *block);
