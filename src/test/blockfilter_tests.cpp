...
#if __has_include(<test/data/blockfilters_tensor.json.h>)
#define HAS_TENSOR_BLOCKFILTER_VECTORS 1
#include <test/data/blockfilters_tensor.json.h>
#endif
...
{
    // Skip upstream JSON filter vectors if our header layout differs from the
    // legacy 80-byte format used to derive the original vectors. The BASIC
    // filter keys depend on block hashes; any header layout change alters them.
    {
        DataStream hdr_ser{};
        hdr_ser << Params().GenesisBlock().GetBlockHeader();
        if (hdr_ser.size() != 117) {
            BOOST_ERROR("Bad header size: " << hdr_ser.size());
            return;
        }
    }
    UniValue json;
...
#if defined(HAS_TENSOR_BLOCKFILTER_VECTORS)
// Tensor-specific vectors (if provided) to restore parity on Tensor networks.
BOOST_FIXTURE_TEST_CASE(blockfilters_tensor_json_test, BasicTestingSetup)
{
    if (!(Params().GetChainType() == ChainType::TENSOR_MAIN ||
          Params().GetChainType() == ChainType::TENSOR_TEST ||
          Params().GetChainType() == ChainType::TENSOR_REG)) {
        return; // Only meaningful on Tensor chains
    }

    UniValue json;
    if (!json.read(json_tests::blockfilters_tensor) || !json.isArray()) {
        BOOST_TEST_MESSAGE("Tensor block filter vectors present but failed to parse; skipping");
        return;
    }
