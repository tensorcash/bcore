...
BOOST_AUTO_TEST_SUITE(argsman_tests)

namespace {

std::string ExpectedDefaultChainTypeString()
{
    constexpr const char* default_chain = DEFAULT_CHAIN_TYPE;
    return default_chain[0] == '\0' ? "main" : default_chain;
}

std::string ExpectedChainMergeDigest()
{
    constexpr const char* default_chain = DEFAULT_CHAIN_TYPE;
    return default_chain[0] == '\0'
        ? "7251e48096395db51a95b7aaffb5b0db97f3930ba142ec16e58deca1f08fdebb"
        : "ae2f10055386ca924d09bb167649ab06dbb48deda1324b25497b088ce7f41009";
}

} // namespace

BOOST_FIXTURE_TEST_CASE(util_datadir, BasicTestingSetup)
{
...
BOOST_AUTO_TEST_CASE(util_GetChainTypeString)
{
    ...
    BOOST_CHECK_EQUAL(test_args.GetChainTypeString(), ExpectedDefaultChainTypeString());
...
BOOST_FIXTURE_TEST_CASE(util_ChainMerge, ChainMergeTestingSetup)
{
    ...
    //
    //   <input> || <output>
    // Digest depends on the build-time default chain used by GetChainTypeString().
    BOOST_CHECK_EQUAL(out_sha_hex, ExpectedChainMergeDigest());
}

BOOST_FIXTURE_TEST_CASE(util_ReadWriteSettings, BasicTestingSetup)
{
