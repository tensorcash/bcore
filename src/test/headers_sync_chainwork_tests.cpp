...
{
    while (!CheckProofOfWork(starting_header.GetShortHash(), starting_header.nAdjBits ? starting_header.nAdjBits : starting_header.nBits, Params().GetConsensus())) {
        ...
        const uint256& merkle_root, const uint32_t nBits)
{
    ...
    while (headers.size() < count) {
        ...
        InitializeTensorHeader(next_header);
