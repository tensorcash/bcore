...
static CBlock BuildBlockTestCase(FastRandomContext& ctx) {
    ...
    assert(!mutated);
    while (!CheckProofOfWork(block.GetShortHash(), block.nAdjBits ? block.nAdjBits : block.nBits, Params().GetConsensus())) ++block.nNonce;
    return block;
}
...
BOOST_AUTO_TEST_CASE(CompactBlockPreservesTensorMetadata)
{
    CTxMemPool& pool = *Assert(m_node.mempool);
    TestMemPoolEntryHelper entry;
    auto rand_ctx(FastRandomContext(uint256{137})); // Changed from 1337 to 137 to fit in uint8_t
    CBlock block(BuildBlockTestCase(rand_ctx));

    // Manually attach TensorCash metadata that must round-trip through compact blocks.
    block.pow.tick = 777;
    block.pow.timestamp = 424242;
    block.pow.vdf = {0x01, 0x02, 0x03, 0x04};
    block.pow.model_identifier = "testmodel@deadbeef";
    block.cumulative_tick = 1337;

    LOCK2(cs_main, pool.cs);
    AddToMempool(pool, entry.FromTx(block.vtx[2]));

    const CBlockHeaderAndShortTxIDs cmpct_block(block, rand_ctx.rand64());
    PartiallyDownloadedBlock partial_block(&pool);
    BOOST_CHECK(partial_block.InitData(cmpct_block, empty_extra_txn) == READ_STATUS_OK);

    CBlock reconstructed;
    BOOST_CHECK(partial_block.FillBlock(reconstructed, {block.vtx[1]}) == READ_STATUS_OK);

    BOOST_CHECK_EQUAL(reconstructed.pow.tick, block.pow.tick);
    BOOST_CHECK_EQUAL(reconstructed.cumulative_tick, block.cumulative_tick);
    BOOST_CHECK(reconstructed.pow.vdf == block.pow.vdf);
    BOOST_CHECK_EQUAL(reconstructed.pow.model_identifier, block.pow.model_identifier);
...
public:
    ...
    SERIALIZE_METHODS(TestHeaderAndShortIDs, obj) { READWRITE(obj.header, obj.nonce, obj.pow, obj.cumulative_tick, Using<VectorFormatter<CustomUintFormatter<CBlockHeaderAndShortTxIDs::SHORTTXIDS_LENGTH>>>(obj.shorttxids), obj.prefilledtxn); }
};
