...
{
    while (!CheckProofOfWork(header.GetShortHash(), header.nAdjBits, chainman.GetParams().GetConsensus())) {
        ++(header.nNonce);
    }
}
    ...
    LIMITED_WHILE(fuzzed_data_provider.ConsumeBool(), 100)
    {
        ...
        CallOneOf(
            ...
            [&]() NO_THREAD_SAFETY_ANALYSIS {
                ...
                blocks.reserve(FUZZ_MAX_HEADERS_RESULTS);
                std::vector<CBlockHeader> headers;
                headers.reserve(FUZZ_MAX_HEADERS_RESULTS);
                for (size_t i = 0; i < FUZZ_MAX_HEADERS_RESULTS; ++i) {
                    CBlock block = ConsumeHeader(fuzzed_data_provider, base.GetHash(), base.nBits);
                    FinalizeHeader(block, chainman);
                    base = block;
                ...
                all_headers.insert(all_headers.end(), blocks.begin(), blocks.end());

                auto headers_msg = NetMsg::Make(NetMsgType::HEADERS, std::move(headers));
