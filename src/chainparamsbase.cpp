...
void SetupChainParamsBaseOptions(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-tensor", "Use the TensorCash mainnet chain. Equivalent to -chain=tensor.", ArgsManager::ALLOW_ANY, OptionsCategory::CHAINPARAMS);
    argsman.AddArg("-tensortest", "Use the TensorCash testnet chain. Equivalent to -chain=tensor-test.", ArgsManager::ALLOW_ANY, OptionsCategory::CHAINPARAMS);
    argsman.AddArg("-tensorreg", "Use the TensorCash regtest chain. Equivalent to -chain=tensor-reg.", ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::CHAINPARAMS);
}
...
std::unique_ptr<CBaseChainParams> CreateBaseChainParams(const ChainType chain)
{
    ...
    case ChainType::TENSOR_MAIN:
        return std::make_unique<CBaseChainParams>("tensor", 39242);
    case ChainType::TENSOR_TEST:
        return std::make_unique<CBaseChainParams>("tensor-test", 29240);
    case ChainType::TENSOR_REG:
        return std::make_unique<CBaseChainParams>("tensor-reg", 19242);
    }
    assert(false);
}
