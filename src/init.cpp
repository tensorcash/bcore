...
void SetupServerArgs(ArgsManager& argsman, bool can_listen_ipc)
{
    ...
    argsman.AddArg("-reorgadvisorydepth=<n>", strprintf("Minimum reorg depth to trigger advisory logging (default: %d)", ADVISORY_DEPTH_THRESHOLD), ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);
    ...
    argsman.AddArg("-reorgadvisorygating", "Enable operator gating for deep reorgs - the node holds the reorg (excluded from fork choice in 'mask' mode, activation parked in 'block' mode; see -reorgadvisorygatingmode) and awaits the 'submitreorgdecision' RPC or the decision timeout before switching chains (default: 1, regtest: 0)", ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);
    argsman.AddArg("-reorgadvisorygatingdepth=<n>", strprintf("Minimum reorg depth to trigger gating when enabled (default: %d)", ADVISORY_DEPTH_THRESHOLD), ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);
    ...
    argsman.AddArg("-reorgadvisoryvetottl=<n>", "How long in seconds a rejected reorg stays vetoed before the branch may prompt again (default: 3600, min: 60, max: 86400). A veto also expires early if the branch extends by -reorgadvisoryvetogrowth blocks or its raw-work margin over the local tip grows by more than that many blocks' worth of work", ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);
    argsman.AddArg("-reorgadvisoryvetogrowth=<n>", "Number of blocks a vetoed branch must extend beyond its length at veto time to escape the veto early and re-prompt once; also sizes the raw-work margin escape quantum (default: 6, min: 1)", ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);
