...
bool BCLog::Logger::StartLogging()
{
    ...
    // If logging already started (not buffering), treat this as a no-op.
    // This can happen in unit tests when a previous test aborted before
    // teardown and the next test tries to initialize logging again.
    if (!m_buffering) {
        return true;
    }
