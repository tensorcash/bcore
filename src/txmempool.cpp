...
void CTxMemPool::addNewTransaction(CTxMemPool::txiter newit, CTxMemPool::setEntries& setAncestors)
{
    ...
    // Index any asset registrations this tx carries so later admissions can detect
    // duplicate-registration / ticker collisions against it (registration-conflict overlay).
    AddAssetRegs(tx);
...
void CTxMemPool::removeUnchecked(txiter it, MemPoolRemovalReason reason)
{
    ...
    // Drop any asset registrations this tx owns from the registration-conflict overlay.
    RemoveAssetRegs(it->GetTx());
...
void CTxMemPool::AddAssetRegs(const CTransaction& tx)
{
    AssertLockHeld(cs);
    const Txid txid = tx.GetHash();
    for (unsigned int o = 0; o < tx.vout.size(); ++o) {
        if (tx.vout[o].vExt.empty()) continue;
        const auto reg = assets::ParseIssuerReg(tx.vout[o].vExt);
        if (!reg) continue;
        // Admission permits at most one mempool registration per asset, so this records (or
        // overwrites with the latest, e.g. after a reorg re-add) the single owning entry.
        m_asset_regs[reg->asset_id] = MempoolAssetReg{COutPoint(txid, o), reg->ticker, reg->decimals, txid};
    }
}

void CTxMemPool::RemoveAssetRegs(const CTransaction& tx)
{
    AssertLockHeld(cs);
    const Txid txid = tx.GetHash();
    for (unsigned int o = 0; o < tx.vout.size(); ++o) {
        ...
        const auto reg = assets::ParseIssuerReg(tx.vout[o].vExt);
        if (!reg) continue;
        // Erase only the entry this tx owns (matched by txid), so a stale overwrite from another
        // tx is never dropped on our behalf.
        if (auto it = m_asset_regs.find(reg->asset_id); it != m_asset_regs.end() && it->second.txid == txid) {
            m_asset_regs.erase(it);
        }
    }
}

std::optional<MempoolAssetReg> CTxMemPool::GetMempoolAssetReg(const uint256& asset_id) const
{
    AssertLockHeld(cs);
    if (auto it = m_asset_regs.find(asset_id); it != m_asset_regs.end()) return it->second;
    return std::nullopt;
}

std::optional<std::pair<uint256, Txid>> CTxMemPool::GetMempoolTickerBinding(const std::string& ticker) const
{
    AssertLockHeld(cs);
    if (ticker.empty()) return std::nullopt;
    // Ticker bindings are derived from the asset registrations (single source of truth). Admission
    // rejects same-ticker collisions, so at most one registration matches.
    for (const auto& [aid, reg] : m_asset_regs) {
        if (reg.ticker == ticker) return std::make_pair(aid, reg.txid);
    }
    return std::nullopt;
}

void CTxMemPool::removeStaleAssetRegistrations(const CCoinsView& confirmed)
{
    AssertLockHeld(cs);
    // Collect first, then remove: removeRecursive() mutates m_asset_regs via removeUnchecked().
    std::vector<Txid> stale;
    for (const auto& [aid, reg] : m_asset_regs) {
        bool is_stale = false;
        AssetRegistryEntry cur;
        if (confirmed.ReadAssetPolicy(aid, cur)) {
            // Asset is now confirmed: this mempool registration is unmineable unless it is a
            // rotation spending the current confirmed ICU.
            bool spends_icu = false;
            if (auto it = GetIter(reg.txid)) {
                for (const CTxIn& in : (*it)->GetTx().vin) {
                    if (in.prevout == cur.icu_outpoint) { spends_icu = true; break; }
                }
            }
            if (!spends_icu) is_stale = true;
        }
        if (!is_stale && !reg.ticker.empty()) {
            uint256 bound;
            if (confirmed.ReadTickerBinding(reg.ticker, bound) && bound != aid) {
                is_stale = true; // ticker is now confirmed-bound to a different asset
            }
        }
        if (is_stale) stale.push_back(reg.txid);
    }
    for (const Txid& txid : stale) {
        if (auto it = GetIter(txid)) { // skip ones already gone via a prior cascade
    ...
    for (const auto& it : GetSortedDepthAndScore()) {
        ...
        for (const CTxIn &txin : tx.vin) {
            ...
            if (!mempoolDuplicate.HaveCoin(txin.prevout)) {
                LogError("Mempool check: missing input %s for tx %s; purging tx and descendants\n",
                         txin.prevout.ToString(), tx.GetHash().ToString());
                removeRecursive(tx, MemPoolRemovalReason::REORG);
                // Restart the consistency sweep after cleanup to avoid using stale iterators.
                return check(active_coins_tip, spendheight);
            }
            // Check whether its inputs are marked in mapNextTx.
            auto it3 = mapNextTx.find(txin.prevout);
            if (it3 == mapNextTx.end() || *it3->first != txin.prevout || it3->second != &tx) {
                LogError("Mempool check: mapNextTx mismatch for input %s in tx %s; purging tx and descendants\n",
                         txin.prevout.ToString(), tx.GetHash().ToString());
                removeRecursive(tx, MemPoolRemovalReason::REORG);
                return check(active_coins_tip, spendheight);
            }
        ...
        if (!Consensus::CheckTxInputs(tx, dummy_state, mempoolDuplicate, spendheight, txfee, /*skip_expensive_zk_verification=*/true)) {
            LogPrintf("MEMPOOL CONSISTENCY FAILURE: tx %s failed re-validation in mempool check\n", tx.GetHash().ToString());
            LogPrintf("  Reason: %s (reject_reason=%s)\n", dummy_state.ToString(), dummy_state.GetRejectReason());
            LogPrintf("  This indicates non-deterministic validation - a transaction that passed initial validation is now failing\n");
            LogPrintf("  This is a CRITICAL BUG that could cause consensus splits\n");
        }
        assert(Consensus::CheckTxInputs(tx, dummy_state, mempoolDuplicate, spendheight, txfee, /*skip_expensive_zk_verification=*/true));
...
size_t CTxMemPool::DynamicMemoryUsage() const {
    ...
    size_t asset_reg_usage = memusage::DynamicUsage(m_asset_regs);
    for (const auto& [aid, reg] : m_asset_regs) asset_reg_usage += memusage::DynamicUsage(reg.ticker);
    return memusage::MallocUsage(sizeof(CTxMemPoolEntry) + 15 * sizeof(void*)) * mapTx.size() + memusage::DynamicUsage(mapNextTx) + memusage::DynamicUsage(mapDeltas) + memusage::DynamicUsage(txns_randomized) + cachedInnerUsage + asset_reg_usage;
}
