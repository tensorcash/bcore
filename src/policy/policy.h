...
/** The maximum weight for governance rotation transactions (default 400 KB) */
static constexpr int32_t DEFAULT_MAX_ROTATION_TX_WEIGHT{1600000};
...
/** The maximum size in bytes of each witness stack item in a standard BIP 342 script (Taproot v1, leaf version 0xc0) */
static constexpr unsigned int MAX_STANDARD_TAPSCRIPT_STACK_ITEM_SIZE{80};
/** The maximum size in bytes of each witness stack item in a Taproot v2 (PQ-enabled) script
 *  Increased to 6144 bytes (6 KB) to accommodate ML-DSA-87 signatures (≈4627 bytes) */
static constexpr unsigned int MAX_STANDARD_TAPSCRIPT_V2_STACK_ITEM_SIZE{6144};
...
/** Maximum number of outputs allowed for covenant transactions (policy). */
static constexpr unsigned int MAX_COVENANT_TX_OUTPUTS{128};
/** Maximum number of OP_OUTPUTMATCH_* invocations permitted per input (policy). */
static constexpr unsigned int MAX_OUTPUTMATCH_PER_INPUT{8};
/** Maximum total bytes of post-quantum signatures per input (policy).
 *  Set to 8 KB to allow one ML-DSA-87 signature (≈4627 bytes) plus overhead,
 *  while preventing DoS via excessive PQ witness data. */
static constexpr unsigned int MAX_STANDARD_PQ_BYTES_PER_INPUT{8192};
 ...
 */
static constexpr unsigned int MODEL_REGISTER_DEPOSIT_TX_VERSION{Consensus::MODEL_REGISTER_DEPOSIT_TX_VERSION};
static constexpr unsigned int MODEL_REGISTER_COMMIT_TX_VERSION{Consensus::MODEL_REGISTER_COMMIT_TX_VERSION};
static constexpr unsigned int MODEL_REGISTER_TX_VERSION{MODEL_REGISTER_DEPOSIT_TX_VERSION};
static const unsigned int MAX_OP_RETURN_COUNT = 8;  // Increased for model deposits (up to 7 OP_RETURNs)
/**
 * An extra transaction can be added to a package, as long as it only has one
 * ancestor and is no larger than this. Not really any reason to make this
 * configurable as it doesn't materially change DoS parameters.
...
static constexpr unsigned int MANDATORY_SCRIPT_VERIFY_FLAGS{SCRIPT_VERIFY_P2SH |
                                                             ...
                                                             SCRIPT_VERIFY_TAPROOT_SCRIPT_ONLY};
...
static constexpr decltype(CTransaction::version) TX_MAX_STANDARD_VERSION{Consensus::MODEL_CHALLENGE_COMMIT_TX_VERSION};
