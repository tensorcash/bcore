...
/** Maximum time (seconds) to wait for the configured proxy to become reachable
 *  before falling back to clearnet HTTP seed bootstrap. */
static constexpr int PROXY_READY_MAX_WAIT_SECS = 20;

/** Number of retry attempts for the HTTP seed API (in addition to the first try). */
static constexpr int HTTP_SEED_MAX_RETRIES = 2;

/** Delay between HTTP seed API retry attempts. */
static constexpr std::chrono::seconds HTTP_SEED_RETRY_DELAY{3};

/** Minimum seednode grace period once the proxy becomes reachable. */
static constexpr std::chrono::seconds SEEDNODE_MIN_GRACE{10};
...
/** Return true if a SOCKS proxy is configured on a network that seed-fetch
 *  connections actually route through (NET_ONION, NET_IPV4, or the name
 *  proxy set by -proxy=).  Deliberately excludes NET_I2P and NET_CJDNS so
 *  that an -i2psam-only config does not trigger the proxy-readiness gate. */
static bool IsSeedFetchProxyConfigured()
{
    Proxy p;
    if (HaveNameProxy()) return true;
    if (GetProxy(NET_ONION, p)) return true;
    if (GetProxy(NET_IPV4, p)) return true;
    return false;
}

/** Try a fast connect to the configured SOCKS proxy to see if it is
 *  accepting connections yet.  Handles both TCP (IP) and Unix-domain socket
 *  proxies.  Returns true if a connection succeeds within ~1 second, false
 *  otherwise.  Pure-socket check — no Qt dependency. */
static bool IsProxyReachable()
{
    Proxy proxy;
    // Check the onion proxy first (most common for Tor-first networks),
    // then fall back to the generic IPv4 proxy, then the name proxy.
    if (!GetProxy(NET_ONION, proxy) && !GetProxy(NET_IPV4, proxy)) {
        if (!GetNameProxy(proxy)) return false;
    }

#ifdef HAVE_SOCKADDR_UN
    // Unix socket proxy — probe with a blocking AF_UNIX connect.
    if (proxy.m_is_unix_socket) {
        ...
        memcpy(addr.sun_path, path.c_str(), std::min(sizeof(addr.sun_path) - 1, path.length()));
    ...
    // TCP/IP proxy — async connect with a 1-second deadline.
    try {
        ...
        const auto& sa = proxy.proxy;
        tcp::endpoint ep(
            boost::asio::ip::make_address(sa.ToStringAddr()),
            sa.GetPort());
...
int CConnman::FetchHttpSeeds(int max_retries)
{
    const auto& api_urls = m_params.HttpSeedApiUrls();
    if (api_urls.empty()) return 0;

    FastRandomContext api_rng;
    std::vector<std::string> urls(api_urls.begin(), api_urls.end());
    std::shuffle(urls.begin(), urls.end(), api_rng);

    int total_added = 0;

    for (const auto& url : urls) {
        if (interruptNet) return total_added;

        for (int attempt = 0; attempt <= max_retries; ++attempt) {
            if (interruptNet) return total_added;
            if (attempt > 0) {
                LogPrintf("HTTP seed API: retry %d/%d for %s\n", attempt, max_retries, url);
                if (!interruptNet.sleep_for(HTTP_SEED_RETRY_DELAY)) return total_added;
            }

            LogPrintf("Querying HTTP seed API: %s\n", url);
            try {
                ...
                // Parse URL (expects https://host/path)
                std::string host, path;
                std::string port = "443";
                if (url.substr(0, 8) == "https://") {
                    auto rest = url.substr(8);
                    auto slash = rest.find('/');
                    host = (slash != std::string::npos) ? rest.substr(0, slash) : rest;
                    ...
                    break; // Skip non-HTTPS URLs entirely
                }
                ...
                if (!SSL_set_tlsext_host_name(stream.native_handle(), host.c_str())) continue;
                ...
                req.set(http::field::user_agent, "tensorcash/net");
                ...
                // Parse JSON response: {"peers":[{"address":"...","port":N},...]}
                UniValue root;
                if (!root.read(res.body())) {
                    LogPrintf("HTTP seed API: failed to parse JSON from %s\n", url);
                    continue; // retry
                }
                const UniValue& peers = root.find_value("peers");
                if (!peers.isArray()) continue;
                ...
                for (const auto& peer : peers.getValues()) {
                    const UniValue& addr_val = peer.find_value("address");
                    const UniValue& port_val = peer.find_value("port");
                    if (!addr_val.isStr()) continue;
                    uint16_t peer_port = port_val.isNum()
                        ? static_cast<uint16_t>(port_val.getInt<int>())
                        : m_params.GetDefaultPort();

                    auto results = Lookup(addr_val.get_str(), peer_port, false, 1);
                    if (!results.empty()) {
                        CAddress addr(results.front(), SeedsServiceFlags());
                        addr.nTime = api_rng.rand_uniform_delay(Now<NodeSeconds>() - 3 * 24h, -4 * 24h);
                        vAdd.push_back(addr);
                    }
                }
                ...
                if (!vAdd.empty()) {
                    ...
                    total_added += vAdd.size();
                    LogPrintf("HTTP seed API: added %d addresses from %s\n", vAdd.size(), url);
                    return total_added; // Got addresses, done
                } else {
                    LogPrintf("HTTP seed API: no usable addresses from %s\n", url);
                    break; // Parsed OK but no peers — retrying same URL won't help
                }
            } catch (const std::exception& e) {
                LogPrintf("HTTP seed API: error querying %s: %s\n", url, e.what());
...
void CConnman::ThreadDNSAddressSeed()
{
    ...
    if (!gArgs.GetArgs("-seednode").empty()) {
        const bool proxy_configured = IsSeedFetchProxyConfigured();
        const auto phase_start = NodeClock::now();

        // --- Phase 1: wait for the proxy to become reachable ---
        // The packaged config sets proxy= before TorManager has started Tor.
        // Instead of burning 30s trying seed nodes against a dead proxy, we
        // first wait for the proxy to accept TCP connections.
        bool proxy_ready = false;
        if (proxy_configured) {
            LogPrintf("-seednode enabled. Waiting up to %ds for proxy to become reachable.\n", PROXY_READY_MAX_WAIT_SECS);
            for (int elapsed = 0; elapsed < PROXY_READY_MAX_WAIT_SECS && !interruptNet; ++elapsed) {
                if (IsProxyReachable()) {
                    proxy_ready = true;
                    LogPrintf("Proxy is reachable after ~%ds.\n", elapsed);
                    break;
                }
                if (!interruptNet.sleep_for(std::chrono::seconds(1))) return;
            }

            // If proxy still not ready and addrman is empty, bootstrap via
            // clearnet HTTP seed API so the node gets addresses immediately.
            // This is scoped: once Tor is up, all future connections go through
            // the proxy as normal.
            if (!proxy_ready && addrman.Size() == 0) {
                LogPrintf("Proxy not reachable and addrman empty — trying early HTTP seed bootstrap.\n");
                int added = FetchHttpSeeds(HTTP_SEED_MAX_RETRIES);
                if (added > 0) {
                    LogPrintf("Early HTTP seed bootstrap: %d addresses added to addrman.\n", added);
                }
            }
        }

        // --- Phase 2: give seed nodes a fair chance ---
        // If the proxy came up, give seed nodes time to connect.  If it
        // never came up, skip straight to DNS seeds.
        if (proxy_ready || !proxy_configured) {
            // Budget: remaining time up to 30s total from phase_start,
            // but at least SEEDNODE_MIN_GRACE so seed nodes get a fair shot.
            constexpr std::chrono::seconds SEEDNODE_TOTAL_BUDGET = 30s;
            auto elapsed_so_far = std::chrono::duration_cast<std::chrono::seconds>(NodeClock::now() - phase_start);
            auto remaining = SEEDNODE_TOTAL_BUDGET - elapsed_so_far;
            if (remaining < SEEDNODE_MIN_GRACE) remaining = SEEDNODE_MIN_GRACE;

            LogPrintf("Trying seed nodes for %d seconds.\n", remaining.count());
            auto seednode_deadline = NodeClock::now() + remaining;

            while (!interruptNet) {
                if (!interruptNet.sleep_for(std::chrono::milliseconds(500)))
                    return;

                if (NodeClock::now() > seednode_deadline) {
            ...
            LogPrintf("Proxy not reachable after %ds — skipping seednode wait, proceeding to DNS seeds.\n", PROXY_READY_MAX_WAIT_SECS);
        }
    }
    ...
    if (outbound_connection_count < SEED_OUTBOUND_CONNECTION_THRESHOLD || seeds_right_now) {
        ...
        // HTTP seed API fallback — if DNS seeds found nothing, query the
        // HTTP seed API endpoints.  This path also fires when the early
        // bootstrap already populated addrman but DNS seeds (which are
        // proxy-dependent) still returned nothing.
        if (found == 0) {
            FetchHttpSeeds(HTTP_SEED_MAX_RETRIES);
        }
    } else {
