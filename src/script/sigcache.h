...
public:
    CachingTransactionSignatureChecker(const CTransaction* txToIn, unsigned int nInIn, const CAmount& amountIn, bool storeIn, SignatureCache& signature_cache, PrecomputedTransactionData& txdataIn, const FixingContext* fixing = nullptr, const ScalarFixingSnapshot* scalar_snapshot = nullptr, std::optional<CovenantInputAsset> input_asset = std::nullopt) : TransactionSignatureChecker(txToIn, nInIn, amountIn, txdataIn, MissingDataBehavior::ASSERT_FAIL, fixing, scalar_snapshot, input_asset), store(storeIn), m_signature_cache(signature_cache)  {}
