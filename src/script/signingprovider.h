...
public:
    ...
    // ML-DSA (Post-Quantum) witness v2 methods
    virtual bool GetMLDSAKey(const XOnlyPubKey& output_key, class CMLDSAKey& key) const { return false; }
    virtual bool GetMLDSATaprootSpendData(const XOnlyPubKey& output_key, TaprootSpendData& spenddata, uint8_t& parity) const { return false; }

    virtual bool GetKeyByXOnly(const XOnlyPubKey& pubkey, CKey& key) const
    {
...
public:
    ...
    bool GetKeyByXOnly(const XOnlyPubKey& pubkey, CKey& key) const override;
    bool GetTaprootSpendData(const XOnlyPubKey& output_key, TaprootSpendData& spenddata) const override;
    bool GetTaprootBuilder(const XOnlyPubKey& output_key, TaprootBuilder& builder) const override;
    bool GetMLDSAKey(const XOnlyPubKey& output_key, CMLDSAKey& key) const override;
    bool GetMLDSATaprootSpendData(const XOnlyPubKey& output_key, TaprootSpendData& spenddata, uint8_t& parity) const override;
};
...
struct FlatSigningProvider final : public SigningProvider
{
    ...
    std::map<XOnlyPubKey, TaprootSpendData> tr_spenddata; /** Map from output key to Taproot spend data (direct storage for complex trees) */
    struct XOnlyComparator {
        bool operator()(const XOnlyPubKey& a, const XOnlyPubKey& b) const
        {
            return std::lexicographical_compare(a.begin(), a.end(), b.begin(), b.end());
        }
    };
    std::map<XOnlyPubKey, CKey, XOnlyComparator> tr_xonly_keys; /** Direct x-only private key cache */
    std::set<std::pair<std::vector<unsigned char>, int>> tr_signable_leaves; /** Leaf scripts we can sign */

    // ML-DSA (Post-Quantum) witness v2 data
    std::map<XOnlyPubKey, class CMLDSAKey, XOnlyComparator> mldsa_keys; /** Map from output key to ML-DSA key */
    std::map<XOnlyPubKey, std::pair<TaprootSpendData, uint8_t>, XOnlyComparator> mldsa_taproot_data; /** Map from output key to (spend data, parity) */
