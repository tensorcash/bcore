...
bool HidingSigningProvider::GetKeyByXOnly(const XOnlyPubKey& pubkey, CKey& key) const
{
    if (m_hide_secret) return false;
    return m_provider->GetKeyByXOnly(pubkey, key);
}
...
bool HidingSigningProvider::GetMLDSAKey(const XOnlyPubKey& output_key, CMLDSAKey& key) const
{
    if (m_hide_secret) return false;
    return m_provider->GetMLDSAKey(output_key, key);
}

bool HidingSigningProvider::GetMLDSATaprootSpendData(const XOnlyPubKey& output_key, TaprootSpendData& spenddata, uint8_t& parity) const
{
    return m_provider->GetMLDSATaprootSpendData(output_key, spenddata, parity);
}
...
{
    if (LookupHelper(keys, keyid, key)) {
        return true;
    }

    // Attempt to derive from cached x-only keys before failing.
    for (const auto& [xonly, stored_key] : tr_xonly_keys) {
    ...
    LogDebug(BCLog::RPC, "FlatSigningProvider::GetKey miss keyid=%s size=%zu (after x-only fallback)", HexStr(keyid), keys.size());
    return false;
}
bool FlatSigningProvider::GetTaprootSpendData(const XOnlyPubKey& output_key, TaprootSpendData& spenddata) const
{
    // Check direct spenddata storage first (used for complex covenant trees)
    if (LookupHelper(tr_spenddata, output_key, spenddata)) {
        return true;
    }
    // Fall back to extracting from builder (used for regular taproot)
...
bool FlatSigningProvider::GetKeyByXOnly(const XOnlyPubKey& pubkey, CKey& key) const
{
    if (LookupHelper(tr_xonly_keys, pubkey, key)) {
        LogDebug(BCLog::RPC,
                 "FlatSigningProvider::GetKeyByXOnly hit cache for %s (tr_xonly_keys=%zu)",
                 HexStr(pubkey),
                 tr_xonly_keys.size());
        return true;
    }
    LogDebug(BCLog::RPC,
             "FlatSigningProvider::GetKeyByXOnly fallback for %s (tr_xonly_keys=%zu)",
             HexStr(pubkey),
             tr_xonly_keys.size());
    return SigningProvider::GetKeyByXOnly(pubkey, key);
}

bool FlatSigningProvider::GetMLDSAKey(const XOnlyPubKey& output_key, CMLDSAKey& key) const
{
    return LookupHelper(mldsa_keys, output_key, key);
}

bool FlatSigningProvider::GetMLDSATaprootSpendData(const XOnlyPubKey& output_key, TaprootSpendData& spenddata, uint8_t& parity) const
{
    auto it = mldsa_taproot_data.find(output_key);
    if (it != mldsa_taproot_data.end()) {
        spenddata = it->second.first;
        parity = it->second.second;
        return true;
    }
    return false;
}
...
FlatSigningProvider& FlatSigningProvider::Merge(FlatSigningProvider&& b)
{
    ...
    tr_spenddata.merge(b.tr_spenddata);
    tr_xonly_keys.merge(b.tr_xonly_keys);
    tr_signable_leaves.insert(b.tr_signable_leaves.begin(), b.tr_signable_leaves.end());
    mldsa_keys.merge(b.mldsa_keys);
    mldsa_taproot_data.merge(b.mldsa_taproot_data);
    return *this;
}
