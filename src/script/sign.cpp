...
bool MutableTransactionSignatureCreator::CreateSchnorrSig(const SigningProvider& provider, std::vector<unsigned char>& sig, const XOnlyPubKey& pubkey, const uint256* leaf_hash, const uint256* merkle_root, SigVersion sigversion) const
{
    ...
    CKey key;
    if (!provider.GetKeyByXOnly(pubkey, key)) {
        LogDebug(BCLog::RPC, "CreateSchnorrSig: missing key for %s", HexStr(pubkey));
        return false;
    }
    ...
    if (!m_txdata || !m_txdata->m_bip341_taproot_ready || !m_txdata->m_spent_outputs_ready) {
        LogDebug(BCLog::RPC,
                 "CreateSchnorrSig: precomputed data missing (txdata=%d taproot_ready=%d spent_ready=%d)",
                 m_txdata != nullptr,
                 m_txdata && m_txdata->m_bip341_taproot_ready,
                 m_txdata && m_txdata->m_spent_outputs_ready);
        return false;
    }
    ...
    if (sigversion == SigVersion::TAPSCRIPT) {
        ...
        if (!leaf_hash) {
            LogDebug(BCLog::RPC, "CreateSchnorrSig: missing leaf hash for tapscript signing");
            return false; // BIP342 signing needs leaf hash.
        }
        execdata.m_tapleaf_hash_init = true;
        execdata.m_tapleaf_hash = *leaf_hash;
    }
    uint256 hash;
    if (!SignatureHashSchnorr(hash, execdata, m_txto, nIn, nHashType, sigversion, *m_txdata, MissingDataBehavior::FAIL)) {
        LogDebug(BCLog::RPC, "CreateSchnorrSig: SignatureHashSchnorr failed for input %u", nIn);
        return false;
    }
    sig.resize(64);
    // Use uint256{} as aux_rnd for now.
    if (!key.SignSchnorr(hash, sig, merkle_root, {})) {
        LogDebug(BCLog::RPC, "CreateSchnorrSig: key.SignSchnorr failed for input %u", nIn);
        return false;
    }
...
static bool SignTaprootScript(const SigningProvider& provider, const BaseSignatureCreator& creator, SignatureData& sigdata, int leaf_version, std::span<const unsigned char> script_bytes, std::vector<valtype>& result)
{
    ...
    if (ms && ms->Satisfy(ms_satisfier, result) == miniscript::Availability::YES) {
        return true;
    }

    // Fallback: detect covenant scripts ending in <xonly> OP_CHECKSIG
    ...
    while (script.GetOp(it, opcode, pushdata)) {
        ops.emplace_back(opcode, pushdata);
    }
    if (ops.size() >= 2) {
        const auto& [last_opcode, last_push] = ops.back();
        const auto& [prev_opcode, prev_push] = ops[ops.size() - 2];
        if (last_opcode == OP_CHECKSIG && prev_opcode <= OP_PUSHDATA4 && prev_push.size() == XOnlyPubKey::size()) {
            XOnlyPubKey tap_key;
            std::copy(prev_push.begin(), prev_push.end(), tap_key.begin());
            if (tap_key.IsFullyValid()) {
                std::vector<unsigned char> sig;
                if (creator.CreateSchnorrSig(provider, sig, tap_key, &leaf_hash, &sigdata.tr_spenddata.merkle_root, SigVersion::TAPSCRIPT)) {
                    result.emplace_back(std::move(sig));
                    return true;
                } else {
                    LogDebug(BCLog::RPC, "SignTaprootScript fallback failed to produce Schnorr signature for leaf %s", HexStr(tap_key));
                }
            } else {
                LogDebug(BCLog::RPC, "SignTaprootScript fallback invalid xonly key in script");
            }
        }
    }

    LogDebug(BCLog::RPC, "SignTaprootScript unable to satisfy tapscript");
    return false;
}
                     ...
                     std::vector<valtype>& ret, TxoutType& whichTypeRet, SigVersion sigversion, SignatureData& sigdata)
{
    ...
    case TxoutType::WITNESS_V2_TAPROOT:
        // Witness v2 requires ML-DSA signatures, not supported by standard signing
        // Use signmldsatransaction RPC instead
        return false;
...
bool SignTransaction(CMutableTransaction& mtx, const SigningProvider* keystore, const std::map<COutPoint, Coin>& coins, int nHashType, std::map<int, bilingual_str>& input_errors)
{
    ...
    for (unsigned int i = 0; i < mtx.vin.size(); ++i) {
        ...
        // Prepare signature data for ECDSA inputs (ML-DSA uses PSBT flow)
        SignatureData sigdata;

        // Check if this is a witness v2 (ML-DSA) input: OP_2 <32 bytes>
        bool is_witness_v2 = (prevPubKey.size() == 34 && prevPubKey[0] == OP_2 && prevPubKey[1] == 0x20);

        if (is_witness_v2) {
            // Use PSBT-based ML-DSA signing for witness v2 inputs
            // Create a temporary PSBT for this transaction
            PartiallySignedTransaction psbt(mtx);

            // Set the UTXO for this input
            psbt.inputs[i].witness_utxo = coin->second.out;

            // Try to update and sign with ML-DSA
            bool updated = node::UpdatePSBTInputMLDSA(*keystore, psbt, i);
            bool signed_ok = false;
            if (updated) {
                signed_ok = node::SignPSBTInputMLDSA(*keystore, psbt, i, &txdata, nHashType);
            }

            if (signed_ok && node::FinalizePSBTInputMLDSA(psbt.inputs[i], nHashType)) {
                // Copy the finalized witness to the transaction
                mtx.vin[i].scriptWitness = psbt.inputs[i].final_script_witness;
            } else {
                input_errors[i] = _("Unable to sign ML-DSA input");
            }
        } else {
            // Standard ECDSA signing
            sigdata = DataFromTransaction(mtx, i, coin->second.out);
