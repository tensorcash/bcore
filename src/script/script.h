...
enum opcodetype
{
    ...
    OP_NOP6 = 0xb5,
    OP_NOP7 = 0xb6,
    // Reassign former NOP slots to covenant opcodes (remain outside OP_SUCCESS range)
    OP_OUTPUTMATCH_NATIVE = 0xb7,
    OP_OUTPUTMATCH_ASSET  = 0xb8,
    OP_NOP10 = 0xb9,
    // Scalar-derivative settlement covenant (CFD_GENERALISATION.md §2.1): repurposes the last
    // NOP slot, gated by the flag-height SCRIPT_VERIFY_SCALAR_CFD activation. Defined as the
    // SAME value as OP_NOP10 so the canonical leaf parser (consensus/scalar_cfd_leaf.cpp) and
    // the interpreter share one symbol and cannot drift. Pre-activation (and for any pre-existing
    // OP_NOP10 use) 0xb9 stays an inert NOP and still prints as "OP_NOP10".
    OP_SCALAR_CFD_SETTLE = OP_NOP10,

    // Opcode added by BIP 342 (Tapscript)
    OP_CHECKSIGADD = 0xba,

    // Post-quantum ML-DSA signature verification (Tapscript only)
    OP_CHECKMLSIG = 0xbb,
    OP_CHECKMLSIGVERIFY = 0xbc,

    // Difficulty-derivative covenant opcodes (v1 Tapscript only, genesis-active;
    // removed from the OP_SUCCESS range). See DIFFICULTY_DERIVATIVE.md.
    OP_NBITS_AT = 0xbd,        // <height_le4> -> <nBits_le4 @ ancestor height>
    OP_DIFFCFD_SETTLE = 0xbe,  // verify a vault's two committed-formula payouts

    OP_INVALIDOPCODE = 0xff,
};

// Maximum value that an opcode can be
static const unsigned int MAX_OPCODE = OP_DIFFCFD_SETTLE;
