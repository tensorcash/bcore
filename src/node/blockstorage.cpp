    ...
    while (pcursor->Valid()) {
        ...
        if (pcursor->GetKey(key) && key.first == DB_BLOCK_INDEX) {
            ...
            if (pcursor->GetValue(diskindex)) {
                ...
                pindexNew->nAdjBits       = diskindex.nAdjBits;
                pindexNew->hashPoW        = diskindex.hashPoW;
                pindexNew->flags          = diskindex.flags;
                pindexNew->nStatus        = diskindex.nStatus;
                pindexNew->nTx            = diskindex.nTx;

                // Backwards-compatibility: if adjusted bits were not persisted, mirror nBits
                if (pindexNew->nAdjBits == 0) pindexNew->nAdjBits = pindexNew->nBits;

                if (!CheckProofOfWork(pindexNew->GetBlockShortHash(), pindexNew->nAdjBits, consensusParams)) {
                    LogError("%s: CheckProofOfWork failed: %s\n", __func__, pindexNew->ToString());
                    std::cout << "indexNew->GetBlockShortHash().ToString() : " << pindexNew->GetBlockShortHash().ToString() << std::endl;
                    std::cout << "pindexNew->nAdjBits : " << pindexNew->nAdjBits << std::endl; 

                    // DEBUG: Calculate target manually to see what's wrong
                    auto target = DeriveTarget(pindexNew->nAdjBits, consensusParams.powLimit);
                    if (target) {
                        std::cout << "Calculated target: " << target->ToString() << std::endl;
                        std::cout << "powLimit: " << consensusParams.powLimit.ToString() << std::endl;
                        
                        arith_uint256 hashArith = UintToArith256(pindexNew->GetBlockShortHash());
                        std::cout << "Hash as arith_uint256: " << hashArith.ToString() << std::endl;
                        std::cout << "Hash > Target: " << (hashArith > *target) << std::endl;
                        std::cout << "Hash <= Target: " << (hashArith <= *target) << std::endl;
                    } else {
                        std::cout << "DeriveTarget returned nullopt!" << std::endl;
                    }                                       
                    return false;
                }
...
namespace {
int CompareEffectiveWork(const CBlockIndex* pa, const CBlockIndex* pb)
{
    const bool a_nonnegative{pa->nChainWork >= pa->nChainPenalty};
    const bool b_nonnegative{pb->nChainWork >= pb->nChainPenalty};

    if (a_nonnegative != b_nonnegative) {
        return a_nonnegative ? 1 : -1;
    }

    if (a_nonnegative) {
        const arith_uint256 a_effective{pa->nChainWork - pa->nChainPenalty};
        const arith_uint256 b_effective{pb->nChainWork - pb->nChainPenalty};
        if (a_effective > b_effective) return 1;
        if (a_effective < b_effective) return -1;
        return 0;
    }

    const arith_uint256 a_debt{pa->nChainPenalty - pa->nChainWork};
    const arith_uint256 b_debt{pb->nChainPenalty - pb->nChainWork};
    if (a_debt < b_debt) return 1;
    if (a_debt > b_debt) return -1;
    return 0;
}
} // namespace
...
bool CBlockIndexWorkComparator::operator()(const CBlockIndex* pa, const CBlockIndex* pb) const
{
    ...
    // Zero-work blocks should not become the build tip on equal chainwork due
    // to sequence-id or pointer tie-breaks after restart.
    const bool pa_zero_work{(pa->nStatus & (BLOCK_FULL_RED_LOCAL | BLOCK_MODEL_CHALLENGE_ZERO_WORK)) != 0};
    const bool pb_zero_work{(pb->nStatus & (BLOCK_FULL_RED_LOCAL | BLOCK_MODEL_CHALLENGE_ZERO_WORK)) != 0};
    if (pa_zero_work != pb_zero_work) return pa_zero_work;
...
bool CBlockIndexPolicyComparator::operator()(const CBlockIndex* pa, const CBlockIndex* pb) const
{
    // First sort by most local policy-effective work: nChainWork - nChainPenalty.
    const int effective_cmp{CompareEffectiveWork(pa, pb)};
    if (effective_cmp > 0) return false;
    if (effective_cmp < 0) return true;

    // Prefer the chain with less full-red debt when effective work is tied.
    if (pa->nChainPenalty < pb->nChainPenalty) return false;
    if (pa->nChainPenalty > pb->nChainPenalty) return true;

    // Zero-work blocks should not become the build tip on equal effective work
    // due to sequence-id or pointer tie-breaks after restart.
    const bool pa_zero_work{(pa->nStatus & (BLOCK_FULL_RED_LOCAL | BLOCK_MODEL_CHALLENGE_ZERO_WORK)) != 0};
    const bool pb_zero_work{(pb->nStatus & (BLOCK_FULL_RED_LOCAL | BLOCK_MODEL_CHALLENGE_ZERO_WORK)) != 0};
    if (pa_zero_work != pb_zero_work) return pa_zero_work;
    ...
    ChainstateManager& chainman)
{
    ...
    // Prune block metadata for blocks we're discarding to prevent database bloat.
    // This is safe since we're already inside the prune lock.
    if (m_block_meta_db && count > 0) {
        size_t meta_pruned = m_block_meta_db->PruneBefore(last_block_can_prune);
        if (meta_pruned > 0) {
            LogDebug(BCLog::PRUNE, "[%s] Pruned %zu block metadata entries (height <= %d)\n",
                chain.GetRole(), meta_pruned, last_block_can_prune);
        }
    }
}
...
bool BlockManager::WriteBlockIndexDB()
{
    ...
    // Flush block metadata DB (first_seen timestamps) in sync with block index.
    if (m_block_meta_db) {
        m_block_meta_db->Flush();
    }
...
bool BlockManager::ReadBlock(CBlock& block, const FlatFilePos& pos) const
{
    ...
    // Backwards-compatibility: if adjusted bits were not serialized, mirror nBits
    if (block.nAdjBits == 0) block.nAdjBits = block.nBits;

    // Check the header
    if (!CheckProofOfWork(block.GetShortHash(), block.nAdjBits, GetConsensus())) {
...
FlatFilePos BlockManager::WriteBlock(const CBlock& block, int nHeight)
{
    ...
    LogPrintf("WriteBlock block_size %d %d %d %d", block_size, STORAGE_HEADER_BYTES, nHeight, block.GetBlockTime());
      ...
      m_interrupt{interrupt}
{
    ...
    // Initialize block metadata DB for reorg advisory (first_seen_ts tracking).
    // Creates a separate "blockmeta" subdirectory under the blocks index path.
    m_block_meta_db = std::make_unique<BlockMetaDB>(
        m_opts.block_tree_db_params.path,
        /*cache_size_bytes=*/2 << 20,  // 2 MiB
        m_opts.block_tree_db_params.memory_only,
        m_opts.block_tree_db_params.wipe_data
    );
...
void ImportBlocks(ChainstateManager& chainman, std::span<const fs::path> import_paths)
{
    ...
    // Startup full-validation rescan is DISABLED.
    //
    // Cached Full_Red/Full_Green status is already replayed and chainwork
    // recomputed at boot by ChainstateManager::LoadBlockIndex()
    // (validation.cpp ~8995-9021), and ActivateBestChain() ran just above, so
    // chain selection is already correct on startup. The live/non-live
    // tip-window policy is enforced incrementally on body-backed blocks by the
    // connection path (ShouldRequireFullValidationForBlock, validation.cpp:8094).
    //
    // The pass below was redundant with that and actively harmful for a node
    // catching up: it seeded the window from m_best_header / setBlockIndexCandidates
    // (header-only tips whose bodies are not yet downloaded -> ReadBlock failures),
    // sent fresh Full requests, hard-waited on the Full queue
    // (while (!IsFullQueueEmpty())), and InvalidateBlock()'d any tip-window block
    // lacking a green Full result. On a half-synced node that combination freezes
    // tip advancement (observed: 1600 "failed to read block", 0 UpdateTip).
    // Set need_rescan = chainman.GetConsensus().external_api to restore it.
    {
        bool need_rescan = false;
        // When using the in-process mock backend, skip mass revalidation/invalidation
        // to avoid destabilizing functional tests unless explicitly requested via RPC.
        if (need_rescan) {
            if (g_ValidationApi && g_ValidationApi->UsesRequestStatusForBlockProcessing()) {
                need_rescan = false;
            }
            if (need_rescan && g_ValidationApi->ShouldDeferMissingFullValidation()) {
                LogPrintf("%s: skipping startup full revalidation in public-status mode\n", __func__);
                need_rescan = false;
            }
        }
        while (need_rescan) {
            need_rescan = false;
            std::vector<const CBlockIndex*> last_blocks;
            const int full_validation_tip_window = GetFullValidationTipWindow();
            {
                LOCK(::cs_main);
                auto add_tip_window = [&](const CBlockIndex* pindex) {
                    for (int i = 0; pindex != nullptr && (full_validation_tip_window == 0 || i < full_validation_tip_window); ++i) {
                        if (std::find(last_blocks.begin(), last_blocks.end(), pindex) == last_blocks.end()) {
                            last_blocks.push_back(pindex);
                        }
                        pindex = pindex->pprev;
                    }
                };
                add_tip_window(chainman.ActiveChain().Tip());
                add_tip_window(chainman.m_best_header);
                for (const CBlockIndex* candidate : chainman.ActiveChainstate().setBlockIndexCandidates) {
                    add_tip_window(candidate);
                }
            }
            std::sort(last_blocks.begin(), last_blocks.end(), [](const CBlockIndex* a, const CBlockIndex* b) {
                return a->nHeight < b->nHeight;
            });
            ValidationResponseValue status;
            for (const CBlockIndex* idx : last_blocks) {
                CBlock block;
                if (!chainman.m_blockman.ReadBlock(block, *idx)) {
                    LogPrintf("FullValidation(tip-window): failed to read block at height %d, hash=%s\n", idx->nHeight, idx->GetBlockHash().ToString());
                    continue;
                }
            
                status = static_cast<ValidationResponseValue>(g_ValidationApi->GetOwnFullStatus(block.GetHash()));
            ...
            while (!g_ValidationApi->IsFullQueueEmpty()){
                if (chainman.m_interrupt) {
                    LogPrintf("Interrupt requested. Exit %s\n", __func__);
                    return;
                }
            ...
            for (const CBlockIndex* idx : last_blocks) {
                ...
                LogPrintf("TEST3 %s block=%s\n", __func__, block.GetHash().ToString());
            
                status = static_cast<ValidationResponseValue>(g_ValidationApi->GetOwnFullStatus(block.GetHash()));
                const bool have_full_status{status != ValidationResponseValue::Not_Checked};
                if (!need_rescan && have_full_status && status == ValidationResponseValue::Full_Red) {
                    if (idx->nHeight == 0) {
                        chainman.GetNotifications().fatalError(
                            strprintf(_("Genesis block failed external validation (status=%d)."),
                                      static_cast<int>(status)));
                        return;
                    }
                    LogPrintf("%s: Full %s is RED; replaying zero local work contribution\n", __func__, block.GetHash().ToString());
                    bool changed{false};
                    {
                        LOCK(::cs_main);
                        CBlockIndex* pindex{chainman.SetBlockFullValidationRedStatus(block.GetHash(), true, &changed)};
                        if (changed) {
                            chainman.RecalculateBlockIndexWorkForFullValidation(pindex);
                        }
                    }
                    if (changed) {
                        BlockValidationState red_state;
                        if (!chainman.ActiveChainstate().ActivateBestChain(red_state)) {
                            LogError("%s: ActivateBestChain after Full RED chainwork replay FAILED (%s)\n", __func__, red_state.ToString());
                        }
                    }
                    continue;
                }
                if (!need_rescan && have_full_status && status == ValidationResponseValue::Full_Green) {
                    bool changed{false};
                    {
                        LOCK(::cs_main);
                        CBlockIndex* pindex{chainman.SetBlockFullValidationRedStatus(block.GetHash(), false, &changed)};
                        if (changed) {
                            chainman.RecalculateBlockIndexWorkForFullValidation(pindex);
                        }
                    }
                    if (changed) {
                        BlockValidationState green_state;
                        if (!chainman.ActiveChainstate().ActivateBestChain(green_state)) {
                            LogError("%s: ActivateBestChain after Full GREEN chainwork replay FAILED (%s)\n", __func__, green_state.ToString());
                        }
                    }
                }
                if (!have_full_status ||
    ...
    LogPrintf("TEST1 %s blockcount=%d\n", __func__, WITH_LOCK(::cs_main, return chainman.ActiveChain().Height()));
...
// =============================================================================
// BlockManager - Block metadata methods for reorg advisory
// =============================================================================

bool BlockManager::RecordBlockFirstSeen(const uint256& block_hash, int32_t tip_height)
{
    if (!m_block_meta_db) {
        return false;
    }
    return m_block_meta_db->WriteFirstSeen(block_hash, GetTime(), tip_height);
}

int64_t BlockManager::GetBlockFirstSeenTs(const uint256& block_hash) const
{
    if (!m_block_meta_db) {
        return 0;
    }
    return m_block_meta_db->GetFirstSeenTs(block_hash);
}

std::optional<BlockMeta> BlockManager::GetBlockMeta(const uint256& block_hash) const
{
    if (!m_block_meta_db) {
        return std::nullopt;
    }
    return m_block_meta_db->Read(block_hash);
}
