...
Warnings::Warnings()
{
    // Pre-release build warning — shown only on non-production chains.
    // Suppressed on the production mainnets (BTC main + TensorCash TENSOR_MAIN),
    // where "do not use for mining or merchant applications" is inappropriate
    // (the tensor mainnet wallet exists precisely to mine). NOTE: we cannot use
    // CChainParams::IsTestChain() here — it is `m_chain_type != ChainType::MAIN`,
    // so it returns true for TENSOR_MAIN and would keep the banner on mainnet.
    if (!CLIENT_VERSION_IS_RELEASE) {
        const ChainType chain_type{Params().GetChainType()};
        const bool is_production_mainnet{chain_type == ChainType::MAIN || chain_type == ChainType::TENSOR_MAIN};
