...
struct CBlockIndexPolicyComparator {
    bool operator()(const CBlockIndex* pa, const CBlockIndex* pb) const;
};
...
public:
    ...
    //! Database for per-block metadata (first_seen_ts, etc.) for reorg advisory.
    //! Thread-safe via internal locking, does not require cs_main.
    std::unique_ptr<BlockMetaDB> m_block_meta_db;
    ...
    //! Block metadata (first_seen_ts) access for reorg advisory.
    //! @{

    /** Record first-seen timestamp for a block (no-op if already recorded). */
    bool RecordBlockFirstSeen(const uint256& block_hash, int32_t tip_height);

    /** Get first-seen timestamp for a block, or 0 if unknown. */
    int64_t GetBlockFirstSeenTs(const uint256& block_hash) const;

    /** Get full block metadata, or nullopt if not recorded. */
    std::optional<BlockMeta> GetBlockMeta(const uint256& block_hash) const;

    /** Direct access to BlockMetaDB (may be null during early init). */
    BlockMetaDB* GetBlockMetaDB() { return m_block_meta_db.get(); }
    const BlockMetaDB* GetBlockMetaDB() const { return m_block_meta_db.get(); }
