...
public:
    ...
    UniValue executeRpc(const std::string& command, const UniValue& params, const std::string& uri) override
    {
        ...
        // Wallet RPC routing requires the URI to start with "/wallet/<name>" so
        // GetWalletForJSONRPCRequest can extract the wallet name. GUI callers
        // historically pass a bare wallet name (e.g. "testnet_basic"); promote
        // those to the canonical form so multi-wallet setups don't fall through
        // to the RPC_WALLET_NOT_SPECIFIED (-19) path.
...
public:
    ...
    std::optional<AssetRegistryEntry> getAssetRegistryEntry(const uint256& asset_id) override
    {
        LOCK(::cs_main);
        Chainstate& active = chainman().ActiveChainstate();
        AssetRegistryEntry entry;
        if (!active.CoinsTip().ReadAssetPolicy(asset_id, entry)) {
            return std::nullopt;
        }
        return entry;
    }

    std::optional<uint256> getAssetIdByTicker(const std::string& ticker) override
    {
        LOCK(::cs_main);
        Chainstate& active = chainman().ActiveChainstate();
        uint256 asset_id;
...
public:
    ...
    void updateBlockPOW(const CProofBlob &pow, const uint64_t &cumulative_tick, const uint32_t &nAdjBits) override
    {
        m_block_template->block.pow = pow;
        m_block_template->block.cumulative_tick = cumulative_tick;
        m_block_template->block.nAdjBits = nAdjBits;
        // Choose commitment format based on activation height
        const CBlock& blk = m_block_template->block;
        const CBlockIndex* prev_index = WITH_LOCK(::cs_main, return chainman().m_blockman.LookupBlockIndex(blk.hashPrevBlock));
        int height = prev_index ? prev_index->nHeight + 1 : 0;
        bool use_merkle = chainman().GetConsensus().IsVdfSpvActive(height);
        m_block_template->block.hashPoW = pow.GetCommitment(use_merkle);
    }
