...
bool DecodeHexBlockHeader(CBlockHeader& header, const std::string& hex_header)
{
    ...
    // Prefer legacy 80-byte header decode first to handle callers that pass a full block
    // or legacy header layouts. If legacy fails, fall back to the extended Tensor header.
    auto try_legacy = [&]() -> bool {
        ...
        try {
            ...
            ser_header >> nVersion >> hashPrevBlock >> hashMerkleRoot >> nTime >> nBits >> nNonce;
            header.SetNull();
            ...
            header.nTime = nTime;
            header.nBits = nBits;
            header.nAdjBits = nBits; // mirror for legacy
            header.nNonce = nNonce;
            header.hashPoW.SetNull();
            header.flags = 0;
            return true;
        } catch (...) { return false; }
    };

    if (try_legacy()) return true;
...
bool DecodeHexBlk(CBlock& block, const std::string& strHexBlk)
{
    ...
    try {
        ...
        // Fallback: legacy Bitcoin Core block layout without TensorCash extensions
        try {
            // Decode legacy header manually
            int32_t nVersion;
            ...
            ssBlock.Rewind();
            ssBlock >> nVersion;
            ssBlock >> hashPrevBlock;
            ssBlock >> hashMerkleRoot;
            ssBlock >> nTime;
            ssBlock >> nBits;
            ssBlock >> nNonce;
            ...
            block.nTime = nTime;
            block.nBits = nBits;
            block.nAdjBits = nBits;
            block.nNonce = nNonce;
            block.hashPoW.SetNull();
            block.flags = 0;

            // Read transaction count and transactions
            uint64_t tx_count{ReadCompactSize(ssBlock)};
            block.vtx.reserve(tx_count);
            for (uint64_t i = 0; i < tx_count; ++i) {
                CMutableTransaction mtx;
                ssBlock >> TX_WITH_WITNESS(mtx);
...
bool DecodeHexBlockHeader(CBlockHeader& header, const std::vector<unsigned char>& raw)
{
    DataStream ser_header{raw};
    try {
        ser_header >> header;
        return true;
    } catch (const std::exception&) {
        ...
        try {
            int32_t nVersion; uint256 hashPrevBlock; uint256 hashMerkleRoot; uint32_t nTime; uint32_t nBits; uint32_t nNonce;
            ser_header >> nVersion >> hashPrevBlock >> hashMerkleRoot >> nTime >> nBits >> nNonce;
            header.SetNull(); header.nVersion=nVersion; header.hashPrevBlock=hashPrevBlock; header.hashMerkleRoot=hashMerkleRoot; header.nTime=nTime; header.nBits=nBits; header.nAdjBits=nBits; header.nNonce=nNonce; header.hashPoW.SetNull(); header.flags=0; return true;
        } catch (...) { return false; }
    }
}

bool DecodeHexBlk(CBlock& block, const std::vector<unsigned char>& raw)
{
    DataStream ssBlock{raw};
    try {
        ssBlock >> TX_WITH_WITNESS(block);
        return true;
    } catch (const std::exception&) {
        // Fallback: legacy header + txs
        try {
            int32_t nVersion; uint256 hashPrevBlock; uint256 hashMerkleRoot; uint32_t nTime; uint32_t nBits; uint32_t nNonce;
            ssBlock.Rewind();
            ssBlock >> nVersion >> hashPrevBlock >> hashMerkleRoot >> nTime >> nBits >> nNonce;
            block.SetNull(); block.nVersion=nVersion; block.hashPrevBlock=hashPrevBlock; block.hashMerkleRoot=hashMerkleRoot; block.nTime=nTime; block.nBits=nBits; block.nAdjBits=nBits; block.nNonce=nNonce; block.hashPoW.SetNull(); block.flags=0;
            uint64_t tx_count{ReadCompactSize(ssBlock)};
            block.vtx.reserve(tx_count);
            for (uint64_t i=0;i<tx_count;++i){ CMutableTransaction mtx; ssBlock >> TX_WITH_WITNESS(mtx); block.vtx.push_back(MakeTransactionRef(std::move(mtx))); }
            return true;
...
util::Result<int> SighashFromStr(const std::string& sighash)
{
    static const std::map<std::string, int> map_sighash_values = {
        ...
        // Accept common alias used by some clients/UIs
        {std::string("SIGHASH_DEFAULT"), int(SIGHASH_DEFAULT)},
